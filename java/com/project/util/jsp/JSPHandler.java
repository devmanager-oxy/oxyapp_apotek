...
public class JSPHandler implements I_JSPType {
    ...
    private String[] specialCharExc = {"+or+", "+and+", "<", ">", "'", "/*", "\"", "--"};
    private String[] specialCharReplaceExc = {" or ", " and ", "&lt;", "&gt;", "", "", "", ""};
    
    public JSPHandler() {
    }

    /** Creates new FRMHandler */
    public JSPHandler(I_JSPInterface ifrm, HttpServletRequest request) {
    ...
    /** Creates new FRMHandler */
    public JSPHandler(I_JSPInterface ifrm, HttpServletRequest request, int lang) {
        formName = ifrm.getFormName();
        fieldNames = ifrm.getFieldNames();
        fieldTypes = ifrm.getFieldTypes();
...
//                    System.out.println("index : "+index);
    ...
    public void requestParamExcTitikKoma(){
        String value = "";
        paramValues.clear();
        ...
        for(int i = 0; i < fieldNames.length; i++) {
            ...
            switch (fieldTypes[i] & FILTER_TYPE){
                ...
                value = checkValueForInjectionExcTitikKoma(value);
            }
    ...
    public String checkValueForInjectionExcTitikKoma(String value){
        if(value!=null && value.length()>0){
            String specChar = "";
            int index = -1;
            boolean stop = false;
            for(int i=0; i<specialCharExc.length; i++){
                specChar =  specialCharExc[i];
                stop = false;

                while(!stop){
	                index = value.indexOf(specChar);
	                if(index>-1){
                            value = replaceString(index, value, specChar, specialCharReplaceExc[i]);
	                }
                    else{
                        stop = true;
                    }
            ...
            case TYPE_INT:
                if(value.length()<1){
                    if((fieldTypes[idx] & FILTER_ENTRY)==ENTRY_REQUIRED){
                        this.addError(idx, JSPMessage.getErr(language, JSPMessage.ERR_REQUIRED));
                    }
                }else{
                    try{
                        ...
                        this.addError(idx, JSPMessage.getErr(language, JSPMessage.ERR_TYPE));
                    }
                }
                break;
   ...
   /* protected Date getDate(int idx) {
    ...
    protected Date getDate(int idx) {
        ...
        try{
            ...
            //return formatDate(sVal, "dd/MM/yyyy");
     ...
     * @update by Edhy on March 28, 2005
     */    
    public  static String userFormatStringDecimal(double number, int digit) {
        
        String format = "";
        
        switch (digit) {
            case 0 : format = "##"+SYSTEM_DIGIT_GROUP+"###";
            break;
            case 1 : format = "##"+SYSTEM_DIGIT_GROUP+"###"+SYSTEM_DECIMAL_SYMBOL+"#";
            break;
            case 2 : format = "##"+SYSTEM_DIGIT_GROUP+"###"+SYSTEM_DECIMAL_SYMBOL+"##";
            break;
        }  
        ...
        try{
            //StringTokenizer strToken = new StringTokenizer(format, SYSTEM_DIGIT_GROUP);
            StringTokenizer strToken = new StringTokenizer(format, SYSTEM_DECIMAL_SYMBOL);
            String str = "";
            while (strToken.hasMoreTokens()){
                str = strToken.nextToken();
            }
