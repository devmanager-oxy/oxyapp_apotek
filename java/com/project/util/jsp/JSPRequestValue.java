...
public class JSPRequestValue {
    ...
    private static String[] specialCharExc = {"+or+", "+and+", "<", ">", "'", "/*", "\"", "--"};
    private static String[] specialCharReplaceExc = {" or ", " and ", "&lt;", "&gt;", "", "", "", ""};
...
//        System.out.println("--FRMQueryString sParam : " + sParam);
    ...
    public static String requestStringExcTitikKoma(HttpServletRequest req, String paramName) {
        String sParam = (((String)req.getParameter(paramName)) == null) ? "" : ((String)(req.getParameter(paramName)));
        sParam = checkValueForInjectionExcTitikKoma(sParam);
        return sParam;
    }
    
    public static String checkValueForInjectionExcTitikKoma(String value){
        if(value!=null && value.length()>0){
            String specChar = "";
            int index = -1;
            boolean stop = false;
