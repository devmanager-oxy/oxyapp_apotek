...
public class CmdRepackItemMulty extends Control implements I_Language {
    ...
    private JspRepackItemMulty jspRepackItem;
    int language = LANGUAGE_DEFAULT;

    public CmdRepackItemMulty(HttpServletRequest request) {
        msgString = "";
        ...
        jspRepackItem = new JspRepackItemMulty(request, repackItem);
    }
    ...
    public JspRepackItemMulty getForm() {
    ...
    public int action(int cmd, long oidRepackItem, long oidRepack, int qtyStockCode) {
        ...
        switch (cmd) {
            ...
            case JSPCommand.SAVE:
                ...
                //cek jika unit stocknya tidak sama
                Vector temp = DbRepackItem.list(0,1, "repack_id="+oidRepack, "");
                if(temp!=null && temp.size()>0){
                    ...
                    try{
                        ...
                        im2 = DbItemMaster.fetchExc(repackItem.getItemMasterId());
                        if(im.getUomStockId()!=im2.getUomStockId()){
                            jspRepackItem.addError(jspRepackItem.JSP_QTY, "Can't process difference unit item");
                        }
                    }
                    catch(Exception e){
                    }
    ...
    public static void recalculateOutputCogs(long repackId){
        //ambil total input cogs
        String sql = "select sum(qty*cogs) as tot, sum(qty) as qty from pos_repack_item where repack_id="+repackId+" and type=0";
        CONResultSet dbrs = null;
        double amount = 0;
        double qty = 0;
        try {
        ...
        if(temp!=null && temp.size()>0){
            for(int i=0; i<temp.size(); i++){
                ...
                ri.setPercentCogs((ri.getQty()/qty)*100);
                //update output
                try{
