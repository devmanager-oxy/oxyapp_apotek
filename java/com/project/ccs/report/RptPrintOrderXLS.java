...
public class RptPrintOrderXLS extends HttpServlet {
    ...
    */
    ...
    throws ServletException, java.io.IOException {
        ...
        try{
            ...
            vendorId = (Long)session.getValue("KONSTAN");
        }catch(Exception ex){
        ...
        try{
            HttpSession session = request.getSession();
            whereclause = (String)session.getValue("DETAIL");
            if(vendorId!=0){
                vOrder= DbOrder.listByVendor(0, 0, whereclause, "");
            }else{
                vOrder= DbOrder.list(0, 0, whereclause, "");
            }
            if(vOrder.size()>0){
                for(int i=0;i<vOrder.size();i++){
                    Order ord = (Order) vOrder.get(i);
                    RptOrder rpt = new RptOrder();
                    try{
                        ItemMaster im = DbItemMaster.fetchExc(ord.getItemMasterId());
                        Location loc = DbLocation.fetchExc(ord.getLocationId());
                        rpt.setBarcode(im.getBarcode());
                        rpt.setName(im.getName());
                        rpt.setCode(im.getCode());
                        rpt.setLocation(loc.getName());
                        
                        
                    }catch(Exception ex){
                        
                    }
                    rpt.setNumber(ord.getNumber());
                    rpt.setDate(ord.getDate());
                    rpt.setQty_order(ord.getQtyOrder());
                    vDetail.add(rpt);
                }
            }
        ...
        wb.println("<Column ss:AutoFitWidth=\"0\" ss:Width=\"200\"/>");//sku
        wb.println("<Column ss:AutoFitWidth=\"0\" ss:Width=\"80\"/>");//name
        wb.println("<Column ss:AutoFitWidth=\"0\" ss:Width=\"80\"/>");//gol1
        wb.println("<Column ss:AutoFitWidth=\"0\" ss:Width=\"40\"/>");//gol2
        ...
        wb.println("<Cell ss:Index=\"2\" ss:MergeAcross=\"4\" ss:StyleID=\"s43\"><Data ss:Type=\"String\">DAFTAR ORDER BARANG</Data></Cell>");
        ...
        wb.println("<Row ss:AutoFitHeight=\"0\">");
        wb.println("<Cell ss:Index=\"2\" ss:MergeAcross=\"1\" ss:StyleID=\"s42\"><Data ss:Type=\"String\"></Data></Cell>");
        wb.println("<Cell ss:StyleID=\"s23\"/>");
        wb.println("<Cell ss:StyleID=\"s23\"/>");
        wb.println("<Cell ss:StyleID=\"s23\"/>");
        wb.println("<Cell ss:StyleID=\"s23\"/>");
        ...
        RptOrder detail1 = (RptOrder) vDetail.get(0);
        
        wb.println("<Row ss:AutoFitHeight=\"0\">");
        wb.println("<Cell ss:Index=\"2\" ss:MergeAcross=\"1\" ss:StyleID=\"s45\"><Data ss:Type=\"String\">Location</Data></Cell>");
        wb.println("<Cell ss:MergeAcross=\"1\" ss:StyleID=\"s45\"><Data ss:Type=\"String\"> : "+detail1.getLocation()+"</Data></Cell>");
        wb.println("<Cell ss:StyleID=\"s23\"/>");
        wb.println("<Cell ss:StyleID=\"s23\"/>");
        wb.println("<Cell ss:StyleID=\"s23\"/>");
        wb.println("</Row>");
           ...
           wb.println("<Cell ss:StyleID=\"m25175646\"><Data ss:Type=\"String\">Qty Order</Data></Cell>");
        ...
        if(vDetail!=null && vDetail.size()>0){
            for(int i=0;i<vDetail.size();i++){
                ...
                RptOrder detail = (RptOrder) vDetail.get(i);
                wb.println("<Row>");
                ...
                wb.println("<Cell ss:StyleID=\"m25175656\"><Data ss:Type=\"String\">"+detail.getQty_order()+"</Data></Cell>");
