...
import com.project.ccs.session.RptMargin;
 ...
 * @author Roy
 */
public class RptSalesStarDateXLS extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            result = (Vector) session.getValue("REPORT_SALES_BY_START_DATE");
        } catch (Exception e) {
        }

        try {
            rp = (ReportParameter) session.getValue("REPORT_SALES_BY_START_DATE_PARAMETER");
        } catch (Exception e) {
        }
        ...
        wb.println("      <Created>2013-07-29T02:46:36Z</Created>");
        ...
        wb.println("      <Column ss:AutoFitWidth=\"0\" ss:Width=\"90.5\"/>");
        ...
        wb.println("      <Cell ss:StyleID=\"s63\"><Data ss:Type=\"String\">Printed : " + user.getFullName() + ", date : " + JSPFormater.formatDate(new Date(), "dd-MMM-yyyy HH:mm:ss") + "</Data></Cell>");
        wb.println("      </Row>");
        ...
        wb.println("      <Cell ss:StyleID=\"s66\"><Data ss:Type=\"String\">LOCATION</Data></Cell>");
        wb.println("      <Cell ss:StyleID=\"s66\"><Data ss:Type=\"String\">START DATE</Data></Cell>");
        wb.println("      <Cell ss:StyleID=\"s66\"><Data ss:Type=\"String\">OMSET</Data></Cell>");

        wb.println("      </Row>");
        if (result != null && result.size() > 0) {
            double total = 0;
            for (int i = 0; i < result.size(); i++) {
                RptMargin rptMargin = (RptMargin) result.get(i);
                String startDate = "-";
                try {
                    if (rptMargin.getStartDate() != null) {
                        startDate = JSPFormater.formatDate(rptMargin.getStartDate(), "yyyy-MM-dd");
                    }
                } catch (Exception e) {
                    startDate = "";
                }
                total = total + rptMargin.getOmset();
                wb.println("      <Row>");
                wb.println("      <Cell ss:StyleID=\"s68\"><Data ss:Type=\"Number\">" + (i + 1) + "</Data></Cell>");
                wb.println("      <Cell ss:StyleID=\"s67\"><Data ss:Type=\"String\">" + rptMargin.getName() + "</Data></Cell>");
                wb.println("      <Cell ss:StyleID=\"s69\"><Data ss:Type=\"String\">" + startDate + "</Data></Cell>");
                wb.println("      <Cell ss:StyleID=\"s69\"><Data ss:Type=\"Number\">" + rptMargin.getOmset() + "</Data></Cell>");
                wb.println("      </Row>");
            }
            wb.println("      <Row>");
            ...
            wb.println("      <Cell ss:StyleID=\"s69\"><Data ss:Type=\"Number\">" + total + "</Data></Cell>");
            wb.println("      </Row>");
        }
