...
import com.project.ccs.postransaction.opname.DbOpnameItem;
...
public class RptSubOpnameXLS extends HttpServlet {
    ...
    */
    ...
    throws ServletException, java.io.IOException {
        ...
        try{
            ...
            oidSubOpname =  (Long)session.getValue("DETAIL");
            opSubLoc= DbOpnameSubLocation.fetchExc(oidSubOpname);
            subLoc= DbSubLocation.fetchExc(opSubLoc.getSubLocationId());
            loc = DbLocation.fetchExc(subLoc.getLocation_id());
            opname = DbOpname.fetchExc(opSubLoc.getOpnameId());
        }catch(Exception ex){
            System.out.println(ex.toString());
        }
        
        Vector vOpitem= new Vector();
        vOpitem = DbOpnameItem.list(0, 0, DbOpnameItem.colNames[DbOpnameItem.COL_OPNAME_SUB_LOCATION_ID]+ "=" + oidSubOpname, "");
        ...
        wb.println("<Column ss:AutoFitWidth=\"0\" ss:Width=\"49.75\"/>");
        ...
        wb.println("</Row>");
        wb.println("<Row>");
        wb.println("<Cell ss:Index=\"2\" ss:MergeAcross=\"1\" ss:StyleID=\"s24\"><Data ss:Type=\"String\">Form Number</Data></Cell>");
        wb.println("<Cell ss:StyleID=\"s24\"><Data ss:Type=\"String\">: "+opSubLoc.getFormNumber()+"</Data></Cell>");
        wb.println("<Cell ss:StyleID=\"s25\"/>");
        wb.println("<Cell ss:StyleID=\"s25\"/>");
        wb.println("<Cell ss:StyleID=\"s25\"/>");
        wb.println("</Row>");
        ...
        if(vOpitem!=null && vOpitem.size()>0){
            ItemMaster im = new ItemMaster();
            for(int i=0;i<vOpitem.size();i++){
                OpnameItem opi = (OpnameItem)vOpitem.get(i);
                try{
                ...
                wb.println("<Cell ss:StyleID=\"s28\"><Data ss:Type=\"Number\">"+opi.getQtyReal()+"</Data></Cell>");
