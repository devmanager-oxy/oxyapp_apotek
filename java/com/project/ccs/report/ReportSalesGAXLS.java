 ...
 * @author Roy
 */
public class ReportSalesGAXLS extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            result = (Vector) session.getValue("REPORT_SALES_GA");
        } catch (Exception e) {
        }
        ...
        String kepada = "";
        try {
            resultParameter = (Vector) session.getValue("REPORT_SALES_GA_PARAMETER");
            invoice = String.valueOf("" + resultParameter.get(0));
            period = JSPFormater.formatDate(String.valueOf("" + resultParameter.get(4)), "dd/MM/yyyy");
            indukId = Long.parseLong(String.valueOf("" + resultParameter.get(1)));
            customerId = Long.parseLong(String.valueOf("" + resultParameter.get(2)));
        } catch (Exception e) {
        }
        ...
        if(customerId != 0){
            try {
                Customer c = DbCustomer.fetchExc(customerId);
                kepada = c.getName();
            } catch (Exception e) {
            }
        }else{
            if(indukId != 0){
                try {
                    IndukCustomer ic = DbIndukCustomer.fetch(indukId);
                    kepada = ic.getName();
                } catch (Exception e) {
                }
            }
        ...
        wb.println("      <Cell ss:MergeAcross=\"2\" ss:StyleID=\"m83536796\"><Data ss:Type=\"String\">" + invoice + "</Data></Cell>");
        ...
        wb.println("      <Cell ss:MergeAcross=\"2\" ss:StyleID=\"m83537204\"><Data ss:Type=\"String\">" + JSPFormater.formatDate(period, "MMMM yyyy") + "</Data></Cell>");
        ...
        if (result != null && result.size() > 0) {
            ...
            for (int i = 0; i < result.size(); i++) {
                ...
                if (tmpGid != 0 && tmpGid != gId) {
                    wb.println("      <Row ss:AutoFitHeight=\"0\">");
                    wb.println("      <Cell ss:MergeAcross=\"5\" ss:StyleID=\"m83537448\"><Data ss:Type=\"String\">TOTAL PEMAKAIAN " + grpName + "</Data></Cell>");
        ...
        try {
            Approval approval = DbApproval.getListApproval(I_Project.TYPE_APPROVAL_SALES_GA, DbApproval.URUTAN_0);             
            try {
        ...
        try {
            Approval approval2 = DbApproval.getListApproval(I_Project.TYPE_APPROVAL_SALES_GA, DbApproval.URUTAN_1);                         
