 ...
 * @author gwawan
 */
public class ProfitLossDetailPdf extends HttpServlet {

    public ProfitLossDetailPdf() {
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Vector vReport = (Vector) session.getValue("SESS_PROFITLOSSDETAIL_PDF");
    ...
    public static void getReport(Document document, Vector vReport) throws BadElementException, DocumentException {
        ...
        if (vReport != null && vReport.size() > 0) {
            ...
            report.addCell(new Phrase("PERIODE " + JSPFormater.formatDate(reportDate, "MMMM yyyy").toUpperCase(), fontHeaderB));
            ...
            if (vRow != null && vRow.size() > 0) {
                for (int i = 0; i < vRow.size(); i++) {
                    ...
                    if (pnl.isLabel()) {
                        ...
                        report.addCell(new Phrase(pnl.getDescription(), fontContentB));
                        report.addCell(new Phrase("", fontContentB));
                        report.addCell(new Phrase("", fontContentB));
                        n = 0;
                    } else if (pnl.isTotal()) {
                        ...
                        report.setDefaultHorizontalAlignment(Cell.ALIGN_LEFT);
                        report.addCell(new Phrase(pnl.getDescription(), fontContent));
