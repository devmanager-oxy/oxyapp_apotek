 ...
 * @author gwawan
 */
public class BalanceSheetPdf extends HttpServlet {
    
    public BalanceSheetPdf() {
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Vector vReport = (Vector) session.getValue("SESS_BALANCESHEET_PDF");
    ...
    public static void getReport(Document document, Vector vReport) throws BadElementException, DocumentException {
        ...
        int[] colWidth = {4, 40, 14, 14, 14, 7, 7};
        ...
        if(vReport != null && vReport.size() > 0) {
            ...
            report.addCell(new Phrase("ANGGARAN PER ", fontContentB));
            report.addCell(new Phrase("REALISASI PER ", fontContentB));
            report.setDefaultRowspan(1);
            ...
            if(vRow != null && vRow.size() > 0) {
                for(int i=0; i<vRow.size(); i++) {
                    ...
                    if (bs.isBoldText()) {
                        ...
                        report.addCell(new Phrase(toAccountingFormat(bs.getBudgetCY()), fontContentB));
                        report.addCell(new Phrase(toAccountingFormat(bs.getRealCY()), fontContentB));
                        report.setDefaultHorizontalAlignment(Cell.ALIGN_CENTER);
                        report.addCell(new Phrase(toAccountingFormat(bs.getPercentLY()), fontContentB));
                        report.addCell(new Phrase(toAccountingFormat(bs.getPercentBudget()), fontContentB));
                    } else {
                        report.setDefaultHorizontalAlignment(Cell.ALIGN_CENTER);
                        report.addCell(new Phrase(String.valueOf(i + 1), fontContent));
                        report.setDefaultHorizontalAlignment(Cell.ALIGN_LEFT);
                        ...
                        report.addCell(new Phrase(toAccountingFormat(bs.getBudgetCY()), fontContent));
                        report.addCell(new Phrase(toAccountingFormat(bs.getRealCY()), fontContent));
                        report.setDefaultHorizontalAlignment(Cell.ALIGN_CENTER);
                        report.addCell(new Phrase(toAccountingFormat(bs.getPercentLY()), fontContent));
                        report.addCell(new Phrase(toAccountingFormat(bs.getPercentBudget()), fontContent));
                    }
                }
            }
        }
