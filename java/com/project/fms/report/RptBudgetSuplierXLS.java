 ...
 * @author Roy Andika
 */
public class RptBudgetSuplierXLS extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (list != null && list.size() > 0) {
            ...
            reportBudget.setDate(dateFrom);
            reportBudget.setDateEnd(dateTo);
            reportBudget.setCreateDate(new Date());
            reportBudget.setVendorId(vendorId);
            reportBudget.setPkp(pkp);
            reportBudget.setNonPkp(nonpkp);
            reportBudget.setIgnore(ignore);
            reportBudget.setEmployeeId(user.getEmployeeId());
            reportBudget.setUserId(userId);        
            reportBudget.setFullName(emp.getName());
            reportBudget.setNon(non);
            reportBudget.setKonsinyasi(konsinyasi);
            reportBudget.setKomisi(komisi);
            long oidBudget = 0;
            try{
                oidBudget = DbReportBudget.insertExc(reportBudget);
            }catch(Exception e){}                    
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (oid != 0) {
                    ...
                    if (v.equalsIgnoreCase("") || v.compareToIgnoreCase(srbs.getSuplier()) != 0) {
                        ...
                        if(oidBudget != 0){
                            ReportBudgetDetail rd = new ReportBudgetDetail();
                            rd.setReportBudgetId(oidBudget);
                            rd.setSequence(number);
                            rd.setVendorId(srbs.getVendorId());
                            try{
                                DbReportBudgetDetail.insertExc(rd);
                            }catch(Exception e){}
                        }
                        number = number + 1;
                    } else {
