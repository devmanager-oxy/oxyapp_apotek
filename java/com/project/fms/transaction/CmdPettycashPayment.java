    ...
    public int action(int cmd, long oidPettycashPayment) {
        ...
        switch (cmd) {
                ...
                if (pettycashPayment.getOID() == 0) {
                    try {
                        ...
                        if(pettycashPayment.getType() == DbPettycashPayment.STATUS_TYPE_KASBON){
                            formatDocCode = DbSystemDocNumber.getNumberPrefixKasbon(opnPeriode.getOID());
                            counter = DbSystemDocNumber.getNextCounterKasbon(opnPeriode.getOID());  
                        }else{
                            formatDocCode = DbSystemDocNumber.getNumberPrefixBkk(opnPeriode.getOID());
                            counter = DbSystemDocNumber.getNextCounterBkk(opnPeriode.getOID());  
                        }
                        ...
                        if(pettycashPayment.getType() == DbPettycashPayment.STATUS_TYPE_KASBON){
                            systemDocNumber.setType(DbSystemDocCode.typeDocument[DbSystemDocCode.TYPE_DOCUMENT_KASBON]);
                        }else{
                        ...
                        if(pettycashPayment.getType() == DbPettycashPayment.STATUS_TYPE_KASBON){
                            formatDocCode = DbSystemDocNumber.getNextNumberKasbon(pettycashPayment.getJournalCounter(), opnPeriode.getOID());
                        }else{
            ...
            case JSPCommand.SUBMIT:
                ...
                try {
                    objCoa = DbCoa.fetchExc(pettycashPayment.getCoaId());
            ...
            case JSPCommand.POST:
                ...
                if(preClosedPeriod.getOID() != 0){                    
                    wherez = "'" + JSPFormater.formatDate(pettycashPayment.getTransDate(), "yyyy-MM-dd") + "' between start_date and end_date " +
                        " and ( status='" + I_Project.STATUS_PERIOD_OPEN + "' OR status='" + I_Project.STATUS_PERIOD_PRE_CLOSED + "')  and " + DbPeriode.colNames[DbPeriode.COL_PERIODE_ID] + " = " + pettycashPayment.getPeriodeId();
                }else{
                    wherez = "'" + JSPFormater.formatDate(pettycashPayment.getTransDate(), "yyyy-MM-dd") + "' between start_date and end_date " +
                        " and status='" + I_Project.STATUS_PERIOD_OPEN + "'";
                ...
                Vector vz = DbPeriode.list(0, 0, wherez, "");
            ...
            case JSPCommand.RESET:
                if (oidPettycashPayment != 0) {
                    try {
                        ...
                        if (oid != 0) {
                            DbPettycashPaymentDetail.deleteAllDetail(oidPettycashPayment);

                            System.out.println("\n\nin delete pettycash_payment : " + oidPettycashPayment);

                            DbApprovalDoc.deleteApprovalByDocId(oidPettycashPayment);
                        }
