...
public class CmdGlPreClosed extends Control implements I_Language {
    ...
    public CmdGlPreClosed(HttpServletRequest request) {
        msgString = "";
        gl = new Gl();
        try {
            dbGl = new DbGl(0);
        ...
        switch (cmd) {
            case JSPCommand.ADD:
                ...
                String where = "'" + JSPFormater.formatDate(gl.getTransDate(), "yyyy-MM-dd") + "' between start_date and end_date " +
                        " and status='" + I_Project.STATUS_PERIOD_PRE_CLOSED +"'";

                Vector v = DbPeriode.list(0, 0, where, "");
                if (v == null || v.size() == 0) {
                    jspGl.addError(jspGl.JSP_TRANS_DATE, "<br>transaction date out of pre-closed period range");
                }
            ...
            case JSPCommand.SAVE:
                ...
                if (gl.getOID() == 0) {
                    try {
                        ...
                        Periode p = DbPeriode.getPreClosedPeriod();
