 ...
 * @author gwawan
 */
public class InvoiceUPALAllPdf extends HttpServlet {

    public static final int INV_UPAL_SURAT = 0;
    public static final int INV_UPAL_PERHITUNGAN = 1;
    public static final int INV_UPAL_FAKTUR = 2;

    public InvoiceUPALAllPdf() {
    }
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Vector vTemp = (Vector) session.getValue("SESS_INV_UPAL_ALL");

            int invUPAL = Integer.parseInt((String) vTemp.get(0));
            Vector vList = (Vector) vTemp.get(1);
                ...
                switch (invUPAL) {
                    case INV_UPAL_SURAT:
                        for (int i = 0; i < vList.size(); i++) {
                            ...
                            InvoiceUPAL invoiceUPAL = (InvoiceUPAL) vList.get(i);
                            document.add(getSurat(document, invoiceUPAL, logo));
                        }
                        break;
                    case INV_UPAL_PERHITUNGAN:
                        for (int i = 0; i < vList.size(); i++) {
                            if (i != 0) {
                                document.newPage();
                            }
