...
public class DbApprovalDoc extends CONHandler implements I_CONInterface, I_CONType, I_PersintentExc, I_Language { 
	...
	public static void deleteApprovalByDocId(long docId){
		...
		System.out.println("in delete doc_id in approval : "+docId);
	...
	/*public static boolean isInAmountRange(int type, long docId, Approval app){
    	...
    	switch(type){
			...
			case 12 : PettycashPayment ptc = new PettycashPayment();
					  try{
					  	  	ptc = DbPettycashPayment.fetchExc(docId);
						  	if(ptc.getAmount()>=app.getJumlahDari()){
						  		return true;
						  	}
					  }
					  catch(Exception e){
    ...
    */	
	...
	public static boolean isPostingReady(long docId){
		...
		if(temp!=null && temp.size()>0){
			...
			for(int i=0; i<temp.size(); i++){
				...
				if(apd.getStatus()==STATUS_DRAFT){
					
					boolean isApproved = false;
					
					try{						
						Approval app = DbApproval.fetchExc(apd.getApprovalId());
						
						for(int x=0; x<temp.size(); x++){
							ApprovalDoc apdx = (ApprovalDoc)temp.get(x);
							if(apd.getOID()!=apdx.getOID()){							
								Approval appx = DbApproval.fetchExc(apdx.getApprovalId());
								if(app.getUrutanApproval()==appx.getUrutanApproval()){
	...
	public static Vector getDocumentByUser(User user){
		...
		String where = colNames[COL_EMPLOYEE_ID]+"="+user.getEmployeeId()+" and "+colNames[COL_STATUS]+"="+STATUS_DRAFT;
		
		for(int i=0; i<I_Crm.approvalTypeStr.length; i++){
			
			String wherex = where + " and "+colNames[COL_DOC_TYPE]+"="+i;		
				
			System.out.println(wherex);	
				
			Vector temp = list(0,0, wherex, colNames[COL_SEQUENCE]);
    ...
    /*public static boolean isInAmountRange(ApprovalDoc apd, Approval app){
