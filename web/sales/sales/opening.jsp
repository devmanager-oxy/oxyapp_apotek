...
<%!
...

	{
		...
		ctrlist.addHeader("Shift","6%");
		//ctrlist.addHeader("Name","20%");
		ctrlist.addHeader("Cashier number","15%");
		ctrlist.addHeader("Currency","5%");
		ctrlist.addHeader("Rate","10%");
		...
		for (int i = 0; i < objectClass.size(); i++) {
			...
			*/
			...
			rowx.add(shift.getName());
                        ...
                        rowx.add(loc.getName() + " " +  cashMaster.getCashierNumber() );
			...
			rowx.add(curr.getCurrencyCode());			
			rowx.add("<div align=\"right\">"+JSPFormater.formatNumber(cashCashier.getRateOpen(),"#,###")+"</div>");
			rowx.add("<div align=\"right\">"+JSPFormater.formatNumber(cashCashier.getAmountOpen(),"#,###")+"</div>");
			rowx.add("<div align=\"right\">"+JSPFormater.formatNumber((cashCashier.getAmountOpen() * cashCashier.getRateOpen()),"#,###")+"</div>");
...
long oidCashCashier = JSPRequestValue.requestLong(request, "hidden_cash_cashier_id");
...
long currency_id = JSPRequestValue.requestLong(request, "JSP_CURRENCY_ID_OPEN");
...
/*switch statement */
listCashier2 = DbCashCashier.list(0, 10, "cash_master_id=" + cashMasterId + " and status=" + DbCashCashier.status_opening, "");
boolean stillOpen = false;
if(listCashier2.size()>0){
    stillOpen=true;
}
iErrCode = ctrlCashCashier.action(iJSPCommand , oidCashCashier, user, stillOpen);
/* end switch*/
...
int vectSize = DbCashCashier.getCount(whereClause);
...
listCashier = DbCashCashier.list(start,recordToGet, "status=" + DbCashCashier.status_opening , orderClause);
...
function cmdSearch(){
	...
	document.frmsales.action="opening.jsp?menu_idx=<%=menuIdx%>";
...
function cmdRate(){
        ...
        document.frmsales.action="opening.jsp";
        document.frmsales.submit();
    }
                                                              ...
                                                              <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                                                ...
                                                                <tr> 
                                                                  ...
                                                                  <td width="38%" height="14"> 
                                                                    <%
																	Vector Vloc = DbLocation.list(0,0, "", "");
																	%>
                                                                    <select name="<%=JspLocation.colNames[JspLocation.JSP_LOCATION_ID]%>" onChange="javascript:cmdLocation()">
                                                                      
                                                                      <%if(Vloc!=null && Vloc.size()>0){
																	  for(int i=0; i<Vloc.size(); i++){
																	  	 Location us = (Location)Vloc.get(i);
                                                                                                                                                 if(location_id==0){
                                                                                                                                                     location_id= us.getOID();
                                                                                                                                                 }
																	  %>
                                                                      <option value="<%=us.getOID()%>" <%if(us.getOID()==location_id){%>selected<%}%>><%=us.getName()%></option>
                                                                      <%}}%>
                                                                    </select>
                                                                  </td>
                                                                ...
                                                                <tr> 
                                                                  <td width="8%" height="14">Cashier Number  </td>
                                                                  <td width="38%" height="14"> 
                                                                    <%
																	Vector VCashMaster = DbCashMaster.list(0,0,"location_id =" + location_id , "");
																	%>
                                                                    <select name="<%=JspCashMaster.colNames[JspCashMaster.JSP_CASH_MASTER_ID]%>">
                                                                      
                                                                      <%if(VCashMaster!=null && VCashMaster.size()>0){
																	  for(int i=0; i<VCashMaster.size(); i++){
																	  	 CashMaster us = (CashMaster)VCashMaster.get(i);
																	  %>
                                                                      <option value="<%=us.getOID()%>" <%if(us.getOID()==cashMasterId){%>selected<%}%>><%=us.getCashierNumber()%></option>
                                                                      <%}}%>
                                                                    </select>
                                                                  </td>
                                                                  <td width="54%" height="14">&nbsp;</td>
                                                                ...
                                                                <tr>
                                                                  <td width="8%" height="14">Shift </td>
                                                                  <td width="38%" height="14"> 
                                                                    <%
																	Vector Vshift = DbShift.list(0,0, "", "");
																	%>
                                                                    <select name="<%=JspShift.colNames[JspShift.JSP_SHIFT_ID]%>">
                                                                     
                                                                      <%if(Vshift!=null && Vshift.size()>0){
																	  for(int i=0; i<Vshift.size(); i++){
																	  	Shift us = (Shift)Vshift.get(i);
																	  %>
                                                                      <option value="<%=us.getOID()%>" <%if(us.getOID()==shiftId){%>selected<%}%>><%=us.getName()%></option>
                                                                      <%}}%>
                                                                    </select>
                                                                  </td>
                                                                  <td width="54%" height="14">&nbsp;</td>
                                                                ...
                                                                <tr>
                                                                  ...
                                                                  <td width="38%" height="14"> 
																	...
																	%>
                                                                    <select name="<%=JspCashCashier.colNames[JspCashCashier.JSP_CURRENCY_ID_OPEN]%>" onChange="javascript:cmdRate()">
                                                                ...
                                                                <tr> 
                                                                  ...
                                                                  <td width="38%" height="15">
                                                                      <input type="text" class="readOnly" name="<%=JspCashCashier.colNames[JspCashCashier.JSP_RATE_OPEN]%>" value="<%=curr.getRate()%>" size="15" readOnly>
                                                                  </td>
                                                                ...
                                                                <tr> 
                                                                  ...
                                                                  <td width="38%" height="15">
                                                                      <input type="text" name="<%=JspCashCashier.colNames[JspCashCashier.JSP_AMOUNT_OPEN]%>" value="0" size="15">
                                                                  </td>
                                                                  <td width="54%" height="15">&nbsp;</td>
                                                                </tr>
							...
							%>
                                                          <tr align="left" valign="top"> 
                                                            <td height="22" valign="middle" colspan="4"> 
                                                              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                                <tr> 
                                                                  <td class="boxed1"><%= drawList(listCashier,oidCashCashier)%></td>
                                                                </tr>
                                                              </table>
                                                            </td>
                                                          </tr>
