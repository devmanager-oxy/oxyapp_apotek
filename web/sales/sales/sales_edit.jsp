...
<%!
	...
	long itemGroupId, long itemMasterId)
	{
		...
		for (int i = 0; i < objectClass.size(); i++) 
		{
			...
			if(index == i && ((iCommand == JSPCommand.EDIT || iCommand == JSPCommand.ASK) || (frmObject.getErrors().size() > 0 && objSalesDetail .getOID()!=0))){
				//if(sales.getStatus()==I_Crm.PROJECT_STATUS_DRAFT){				
					...
					rowx.add(colCombo2.getCode()+"-"+colCombo2.getName());//JSPCombo.draw(frmObject.colNames[frmObject.JSP_PRODUCT_MASTER_ID],null, ""+objEntity.getProductMasterId(), ItemMaster_value , ItemMaster_key, "onchange=\"javascript:checkProduct()\"", "formElemen")+ frmObject.getErrorMsg(frmObject.JSP_PRODUCT_MASTER_ID));
					...
					rowx.add("<input type=\"hidden\" name=\"EditAmount\" value=\""+JSPFormater.formatNumber(objSalesDetail .getSellingPrice(),"#,###.##")+"\">"+
							 "<input style=\"text-align:right\" type=\"text\" name=\""+frmObject.colNames[frmObject.JSP_SELLING_PRICE] +"\" value=\""+JSPFormater.formatNumber(objSalesDetail .getSellingPrice(),"#,###.##")+"\" class=\"formElemen\" size=\"25\" onClick=\"this.select()\" onBlur=\"javascript:checkNumber2()\">"+ frmObject.getErrorMsg(frmObject.JSP_SELLING_PRICE));				
					rowx.add("<input type=\"hidden\" name=\"EditDiscount\" value=\""+JSPFormater.formatNumber(objSalesDetail .getDiscountAmount(),"#,###.##")+"\">"+
							 "<input style=\"text-align:right\" type=\"text\" name=\""+frmObject.colNames[frmObject.JSP_COGS] +"\" value=\""+JSPFormater.formatNumber(objSalesDetail .getCogs(),"#,###.##")+"\" class=\"formElemen\" size=\"15\" onClick=\"this.select()\" onBlur=\"javascript:checkNumber2()\">"+ frmObject.getErrorMsg(frmObject.JSP_COGS)+"<br><a href=\"javascript:updateCogs('"+objSalesDetail.getOID()+"')\">Save</a>");				
					//rowx.add("<input style=\"text-align:right\" type=\"text\" name=\""+frmObject.colNames[frmObject.JSP_TOTAL] +"\" value=\""+JSPFormater.formatNumber(objSalesDetail .getTotal(),"#,###.##")+"\" class=\"formElemen\" size=\"25\" readonly>"+ frmObject.getErrorMsg(frmObject.JSP_TOTAL));			
					rowx.add("<input style=\"text-align:right\" type=\"text\" name=\""+frmObject.colNames[frmObject.JSP_TOTAL] +"\" value=\""+JSPFormater.formatNumber((colCombo2.getSellingPrice()*objEntity.getQty())-(objSalesDetail .getDiscountAmount()),"#,###.##")+"\" class=\"readonly\" size=\"25\" readonly>"+ frmObject.getErrorMsg(frmObject.JSP_TOTAL));			
				/*}else{
					rowx.add("<input type=\"hidden\" name=\""+frmObject.colNames[frmObject.JSP_SALES_ID] +"\" value=\""+objSalesDetail .getSalesId()+"\">" +
							 "<input type=\"hidden\" name=\""+frmObject.colNames[frmObject.JSP_CURRENCY_ID] +"\" value=\""+objSalesDetail .getCurrencyId()+"\">" +
			...
			{				
				...
				rowx.add("<div align=\"right\">"+JSPFormater.formatNumber(objSalesDetail.getCogs(), "#,###.##")+"</div>");
...
<%	
	...
	iErrCode = cmdSales.action(iCommand , oidSales, sysCompany.getOID());
	...
	if(iCommand!=JSPCommand.LIST && itemGroupId==0 && itemGroups!=null && itemGroups.size()>0){
		ItemGroup ig = (ItemGroup)itemGroups.get(0);
		itemGroupId = ig.getOID();
		long uniUsahaId = JSPRequestValue.requestLong(request, jspSales.colNames[jspSales.JSP_UNIT_USAHA_ID]);
		sales.setUnitUsahaId(uniUsahaId);
	}
	else{
		itemGroupId = sales.getItemGroupId();
	}
	...
	iErrCode = cmdSalesDetail.action(iCommand , oidSalesDetail, sales.getOID());
	...
	*/
	...
	if (iCommand==JSPCommand.SUBMIT){
		...
		out.println("oidSalesDetail : "+oidSalesDetail);
		try{
			SalesDetail sd = DbSalesDetail.fetchExc(oidSalesDetail);
			double cogs = JSPRequestValue.requestDouble(request, JspSalesDetail.colNames[JspSalesDetail.JSP_COGS]);
			sd.setCogs(cogs);
...
<script language="JavaScript">
	...
	function updateCogs(oidSalesDetail){
	...
	function cmdAdd(){
		...
		document.frmsalesproductdetail.prev_command.value="<%=prevCommand%>";
		document.frmsalesproductdetail.action="sales_edit.jsp?menu_idx=<%=menuIdx%>&oid=<%=oidSales%>";
		document.frmsalesproductdetail.submit();
	}
                                                          ...
                                                          <tr align="left" valign="top"> 
                                                            <td height="8"  colspan="3"> 
                                                              <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                                                ...
                                                                <tr> 
                                                                  <td height="18" width="10%">Item 
                                                                    Group</td>
                                                                  <td width="30%"> 
                                                                    <select name="<%=jspSales.colNames[jspSales.JSP_ITEM_GROUP_ID] %>" onChange="javascript:checkProduct()">
                                                                      <%if(itemGroups!=null && itemGroups.size()>0){
																	  for(int i=0; i<itemGroups.size(); i++){
																	  	ItemGroup us = (ItemGroup)itemGroups.get(i);
																	  %>
                                                                      <option value="<%=us.getOID()%>" <%if(us.getOID()==itemGroupId){%>selected<%}%>><%=us.getName()%></option>
                                                                      <%}}%>
                                                                    </select>
                                                                  </td>
                                                                  <td width="6%">&nbsp;</td>
		   ...
		   %>
                                                                <tr align="left" valign="top"> 
                                                                  <td height="22" valign="middle" colspan="4"> 
                                                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                                      <tr> 
                                                                        <td class="boxed1"><%= drawList(iCommand,jspSalesDetail , salesProductDetail,listSalesDetail,oidSalesDetail,sales, itemGroupId, itemMasterId)%></td>
                                                                      </tr>
                                                                    </table>
                                                                  </td>
                                                                </tr>
                                                                      ...
                                                                      <tr> 
                                                                        <td width="97%"> 
                                                                          <table width="40%" border="0" cellspacing="1" cellpadding="1">
                                                                            <tr> 
                                                                              <td width="33%"><a href="javascript:cmdUpdateDoc()"  onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('save211','','../images/savedoc2.gif',1)"><img src="../images/savedoc.gif" name="save211" border="0"></a></td>
                                                                              <td width="34%"><%if(sales.getStatus()==0){%><a href="javascript:cmdDeleteDoc('<%=oidSales%>')"><img src="../images/deletedoc.gif" width="120" height="22" border="0"></a><%}%></td>
                                                                              <td width="33%"> 
