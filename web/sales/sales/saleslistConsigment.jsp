...

	{
		...
		for (int i = 0; i < objectClass.size(); i++) {
			...
			*/
			...
			rowx.add("<div align=\"center\">"+((sales.getStatus()==0) ? "-" : "Posted")+"</div>");
...
//Where Clause
 ...
 if (whereClause != "") whereClause = whereClause + " and ";
	whereClause = whereClause + " type="+stype;
 if (whereClause != "") whereClause = whereClause + " and ";
...
function targetPage(oidSales){
	...
	<%}
		...
		window.location="sales_consigment.jsp?menu_idx=<%=menuIdx%>&command=<%=JSPCommand.LIST%>&hidden_sales_id="+oidSales;
	<%}%>
}

function cmdSearch(){
	document.frmsales.command.value="<%=JSPCommand.SUBMIT%>";
	document.frmsales.action="saleslistConsigment.jsp?menu_idx=<%=menuIdx%>";
...
function cmdEdit(oidSales, oidProposal){
	...
	document.frmsales.prev_command.value="<%=prevJSPCommand%>";
	document.frmsales.action="sales_consigment.jsp";
	document.frmsales.submit();
}
                                                          ...
                                                          <tr align="left" valign="top"> 
                                                            <td height="8" valign="middle" colspan="3"> 
                                                              <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                                                ...
                                                                <tr> 
                                                                  <td width="8%" height="14">Sales 
                                                                    Type </td>
                                                                  <td width="38%" height="14"> 
                                                                    <select name="sales_type">
                                                                      <option value="0" <%if(stype==0){%>selected<%}%>>CASH</option>
                                                                      <option value="1" <%if(stype==1){%>selected<%}%>>CREDIT</option>
                                                                    </select>
                                                                  </td>
                                                                  <td width="54%" height="14">&nbsp;</td>
                                                                </tr>
