...
<%!
	...
	public Vector addNewDetail(Vector listBankNonpoActivity, TransactionActivityDetail activityDetail){
		try{
			boolean found = false;
			//diblok sementara
			if(listBankNonpoActivity!=null && listBankNonpoActivity.size()>0){
				for(int i=0; i<listBankNonpoActivity.size(); i++){
					TransactionActivityDetail cr = (TransactionActivityDetail)listBankNonpoActivity.get(i);
					if(activityDetail.getIsDirect()==cr.getIsDirect() && cr.getType()==activityDetail.getType() && cr.getModuleId()==activityDetail.getModuleId() && cr.getDonorId()==activityDetail.getDonorId()){
			...
			if(!found){
				listBankNonpoActivity.add(activityDetail);
			}
		}
		catch(Exception e){
		...
		return listBankNonpoActivity;
	}

	public Vector concatVector(Vector listBankNonpoActivity, Vector vctNewActivityDetail, double maxAmount, double maxTrx, double maxActAmountFA, double maxActAmountLog, double maxActAmountMod){
			...
			if(listBankNonpoActivity!=null && listBankNonpoActivity.size()>0){
				...
				if(vctNewActivityDetail!=null && vctNewActivityDetail.size()>0){		
					...
					for(int i=0; i<vctNewActivityDetail.size(); i++){
						...
						for(int x=0; x<listBankNonpoActivity.size(); x++){
						
							TransactionActivityDetail activityDetailx = (TransactionActivityDetail)listBankNonpoActivity.get(x);
							...
							if(cr.getType()==activityDetailx.getType() && cr.getType()==DbTransactionActivityDetail.ACTIVITY_TYPE_FA){
								...
								listBankNonpoActivity.remove(x);
								break;
					...
					listBankNonpoActivity.addAll(vctNewActivityDetail);
				}
			}
			else{
				listBankNonpoActivity = new Vector();
				listBankNonpoActivity.addAll(vctNewActivityDetail);
			}
...
//int start = JSPRequestValue.requestInt(request, "start");
//int prevJSPCommand = JSPRequestValue.requestInt(request, "prev_command");
long oidActivityDetail = JSPRequestValue.requestLong(request, "hidden_banknonpo_activity_id");
...
//JSPLine ctrLine = new JSPLine();
Vector listBankNonpoActivity = new Vector(1,1);


//iErrCode = ctrlTransactionActivityDetail.action(iJSPCommand , oidActivityDetail);
...
if(session.getValue("BNPOACTIVITY_DETAILX")!=null){
	listBankNonpoActivity = (Vector)session.getValue("BNPOACTIVITY_DETAILX");
	
	//out.println("in meer ...");
	
	if(iJSPCommand!=JSPCommand.SAVE && listBankNonpoActivity!=null && listBankNonpoActivity.size()>0){
		for(int i=0; i<listBankNonpoActivity.size(); i++){
		
			//out.println("<br>in meer ..2.");
			
			TransactionActivityDetail xx = (TransactionActivityDetail)listBankNonpoActivity.get(i);
			//if(xx.getIsDirect()==0){
			if(xx.getIsDirect()==0 && xx.getType()==DbTransactionActivityDetail.ACTIVITY_TYPE_MODULE){
				//out.println("<br>in meer ..3.");
				
				listBankNonpoActivity.remove(i);
				i=i-1;
			}else if(xx.getIsDirect()==1 && xx.getType()==DbTransactionActivityDetail.ACTIVITY_TYPE_MODULE){
				Vector activityPredefined = DbBanknonpoPaymentDetail.getActivityPredefined(bankNonpoPayment.getOID());
				...
				if(activityPredefined!=null && activityPredefined.size()>0){
					for(int iz=0; iz<activityPredefined.size(); iz++){
						...
						BanknonpoPaymentDetail ppd = (BanknonpoPaymentDetail)v.get(1);
	...
	//load max fa+log trx
	if(listBankNonpoActivity!=null && listBankNonpoActivity.size()>0){
		for(int ix=0; ix<listBankNonpoActivity.size(); ix++){
			TransactionActivityDetail ppdxx = (TransactionActivityDetail)listBankNonpoActivity.get(ix);
...
//out.println("listBankNonpoActivity : "+listBankNonpoActivity);

double totalDetail = 0;//getTotalDetail(listBankNonpoActivity);
String where = "";
...
if(cntActivity>0){
	listBankNonpoActivity = DbTransactionActivityDetail.list(0,0, "transaction_id="+bankNonpoPayment.getOID(), "");
}
...
if(iJSPCommand==JSPCommand.SAVE){
	...
	Vector listBankNonpoActivityX = DbBanknonpoPaymentDetail.list(0,0, "banknonpo_payment_id="+bankNonpoPayment.getOID(), "");	
	if(listBankNonpoActivityX!=null && listBankNonpoActivityX.size()>0){
		for(int ix=0; ix<listBankNonpoActivityX.size(); ix++){
			BanknonpoPaymentDetail ppdx = (BanknonpoPaymentDetail)listBankNonpoActivityX.get(ix);
	...
	if(activityPredefined!=null && activityPredefined.size()>0){
		for(int i=0; i<activityPredefined.size(); i++){
			...
			if(faAmount>0){				
                ta.setTransactionId(oidBanknonpoPayment );
			...
			//DbTransactionActivityDetail.insertNonModularActivityDetail(oidBanknonpoPayment , DbTransactionActivityDetail.ACTIVITY_TYPE_FA, faAmount, faPercent);
			...
			if(logAmount>0){				
                ta1.setTransactionId(oidBanknonpoPayment );
				...
				//listBankNonpoActivity = addNewDetail(listBankNonpoActivity, ta1);
				actSubmit = addNewDetail(actSubmit, ta1);
				//actSubmit.add(ta1);
			}
			
			//DbTransactionActivityDetail.insertNonModularActivityDetail(oidBanknonpoPayment , DbTransactionActivityDetail.ACTIVITY_TYPE_LOG, logAmount, logPercent);
...
listBankNonpoActivity = concatVector(listBankNonpoActivity, actSubmit, maxAmount, maxTrx, maxActAmountFA, maxActAmountLog, maxActAmountMod);
//listBankNonpoActivity = concatVector(listBankNonpoActivity, actSubmit);
...
/*if(!isOK){
	...
	if(listBankNonpoActivity!=null && listBankNonpoActivity.size()>0){
		for(int i=0; i<listBankNonpoActivity.size(); i++){
			TransactionActivityDetail tadx = (TransactionActivityDetail)listBankNonpoActivity.get(i);
...
session.putValue("BNPOACTIVITY_DETAILX", listBankNonpoActivity);

//out.println("<br>listBankNonpoActivity : "+listBankNonpoActivity);
...
<%if(iJSPCommand==JSPCommand.SAVE && isOK){%>
	window.location="banknonpoactivityclosing.jsp?hidden_banknonpo_payment_id=<%=oidBanknonpoPayment %>&command=<%=JSPCommand.ADD%>&select_idx=-1";
<%}%>

function cmdPrintJournal(){	 
	window.open("<%=printroot%>.report.RptPCPaymentPDF?oid=<%=appSessUser.getLoginId()%>&pcPayment_id=<%=bankNonpoPayment.getOID()%>");
}

function cmdClickIt(){
	document.frmbanknonpodetail.<%=jspTransactionActivityDetail.colNames[jspTransactionActivityDetail.JSP_AMOUNT]%>.select();
}

function cmdGetBalance(){
	
	var x = document.frmbanknonpodetail.<%=jspBanknonpoPayment .colNames[jspBanknonpoPayment .JSP_COA_ID]%>.value;
	//alert(x);
	...
	%>
		if(x=='<%=c.getOID()%>'){
			document.frmbanknonpodetail.<%=jspBanknonpoPayment .colNames[jspBanknonpoPayment .JSP_ACCOUNT_BALANCE]%>.value="<%=JSPFormater.formatNumber(c.getOpeningBalance(),"###.##")%>";
			...
			document.frmbanknonpodetail.pcash_balance.value="<%=JSPFormater.formatNumber(c.getOpeningBalance(),"###.##")%>";
...
function cmdFixing(){	
	document.frmbanknonpodetail.command.value="<%=JSPCommand.POST%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();	
}

function cmdNewJournal(){	
	document.frmbanknonpodetail.command.value="<%=JSPCommand.NONE%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();	
}
...
function checkNumber(){
	var st = document.frmbanknonpodetail.<%=jspBanknonpoPayment .colNames[jspBanknonpoPayment .JSP_AMOUNT]%>.value;		
	var ab = document.frmbanknonpodetail.<%=jspBanknonpoPayment .colNames[jspBanknonpoPayment .JSP_ACCOUNT_BALANCE]%>.value;		
	...
	document.frmbanknonpodetail.<%=jspBanknonpoPayment .colNames[jspBanknonpoPayment .JSP_AMOUNT]%>.value = formatFloat(result, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace); 
}

function checkNumber2(){
	var main = document.frmbanknonpodetail.<%=jspBanknonpoPayment .colNames[jspBanknonpoPayment .JSP_AMOUNT]%>.value;		
	main = cleanNumberFloat(main, sysDecSymbol, usrDigitGroup, usrDecSymbol);
	var currTotal = document.frmbanknonpodetail.total_detail.value;
	currTotal = cleanNumberFloat(currTotal, sysDecSymbol, usrDigitGroup, usrDecSymbol);				
	var idx = document.frmbanknonpodetail.select_idx.value;
	
	var maxtransaction = document.frmbanknonpodetail.max_pcash_transaction.value;
	var pbalanace = document.frmbanknonpodetail.pcash_balance.value;
	var limit = parseFloat(maxtransaction);
	if(limit > parseFloat(pbalanace)){
		limit = parseFloat(pbalanace);
	}		
	
	var st = document.frmbanknonpodetail.<%=jspTransactionActivityDetail.colNames[jspTransactionActivityDetail.JSP_AMOUNT]%>.value;		
	result = removeChar(st);	
	result = cleanNumberFloat(result, sysDecSymbol, usrDigitGroup, usrDecSymbol);
	...
	if(parseFloat(idx)<0){
		...
		document.frmbanknonpodetail.<%=jspBanknonpoPayment .colNames[jspBanknonpoPayment .JSP_AMOUNT]%>.value = formatFloat(amount, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace); 	
		
	}
	//edit
	else{
		var editAmount =  document.frmbanknonpodetail.edit_amount.value;
	...
	document.frmbanknonpodetail.<%=jspTransactionActivityDetail.colNames[jspTransactionActivityDetail.JSP_AMOUNT]%>.value = formatFloat(result, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace); 	
}

function cmdSubmitCommand(){
	document.frmbanknonpodetail.command.value="<%=JSPCommand.SAVE%>";
	document.frmbanknonpodetail.prev_command.value="<%=prevJSPCommand%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();
}
...
function cmdBanknonpoPayment (oid){
	<%if(cntActivity>0){%>
		document.frmbanknonpodetail.hidden_bankarchive.value=oid;
		document.frmbanknonpodetail.command.value="<%=JSPCommand.EDIT%>";
		document.frmbanknonpodetail.prev_command.value="<%=prevJSPCommand%>";
		document.frmbanknonpodetail.action="banknonpodetailprint.jsp";
		document.frmbanknonpodetail.submit();
	<%}else{%>
...
function cmdAdd(){
	document.frmbanknonpodetail.select_idx.value="-1";
	document.frmbanknonpodetail.hidden_banknonpo_payment_id.value="0";
	document.frmbanknonpodetail.hidden_banknonpo_activity_id.value="0";
	document.frmbanknonpodetail.command.value="<%=JSPCommand.ADD%>";
	document.frmbanknonpodetail.prev_command.value="<%=prevJSPCommand%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();
}
...
function cmdAsk(idx){
	document.frmbanknonpodetail.select_idx.value=idx;
	document.frmbanknonpodetail.hidden_banknonpo_activity_id.value=0;
	document.frmbanknonpodetail.command.value="<%=JSPCommand.ASK%>";
	document.frmbanknonpodetail.prev_command.value="<%=prevJSPCommand%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();
}

function cmdConfirmDelete(oidActivityDetail){
	document.frmbanknonpodetail.hidden_banknonpo_activity_id.value=oidActivityDetail;
	document.frmbanknonpodetail.command.value="<%=JSPCommand.DELETE%>";
	document.frmbanknonpodetail.prev_command.value="<%=prevJSPCommand%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();
}
...
function cmdSave(){
	document.frmbanknonpodetail.command.value="<%=JSPCommand.SUBMIT%>";
	document.frmbanknonpodetail.prev_command.value="<%=prevJSPCommand%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();
}
...
function cmdEdit(idxx){
	document.frmbanknonpodetail.select_idx.value=idxx;
	document.frmbanknonpodetail.hidden_banknonpo_activity_id.value=0;
	document.frmbanknonpodetail.command.value="<%=JSPCommand.EDIT%>";
	document.frmbanknonpodetail.prev_command.value="<%=prevJSPCommand%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
...
function cmdCancel(oidActivityDetail){
	document.frmbanknonpodetail.hidden_banknonpo_activity_id.value=oidActivityDetail;
	document.frmbanknonpodetail.command.value="<%=JSPCommand.EDIT%>";
	document.frmbanknonpodetail.prev_command.value="<%=prevJSPCommand%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
...
function cmdBack(oid){
	document.frmbanknonpodetail.hidden_banknonpo_payment_id.value=oid;
	document.frmbanknonpodetail.command.value="<%=JSPCommand.ADD%>";
	document.frmbanknonpodetail.action="banknonpoactivity.jsp";
	document.frmbanknonpodetail.submit();
}

function cmdListFirst(){
	document.frmbanknonpodetail.command.value="<%=JSPCommand.FIRST%>";
	document.frmbanknonpodetail.prev_command.value="<%=JSPCommand.FIRST%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();
}

function cmdListPrev(){
	document.frmbanknonpodetail.command.value="<%=JSPCommand.PREV%>";
	document.frmbanknonpodetail.prev_command.value="<%=JSPCommand.PREV%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();
}

function cmdListNext(){
	document.frmbanknonpodetail.command.value="<%=JSPCommand.NEXT%>";
	document.frmbanknonpodetail.prev_command.value="<%=JSPCommand.NEXT%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();
}

function cmdListLast(){
	document.frmbanknonpodetail.command.value="<%=JSPCommand.LAST%>";
	document.frmbanknonpodetail.prev_command.value="<%=JSPCommand.LAST%>";
	document.frmbanknonpodetail.action="banknonpoactivityclosingpredefined.jsp";
	document.frmbanknonpodetail.submit();
}
...
function cmdDelPict(oidActivityDetail){
	document.frmimage.hidden_banknonpo_activity_id.value=oidActivityDetail;
	document.frmimage.command.value="<%=JSPCommand.POST%>";
	document.frmimage.action="banknonpoactivityclosingpredefined.jsp";
              ...
              <tr> 
                ...
                <td width="100%" valign="top"> 
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
                      <td class="title"><span class="level1">Closing Period
                        </span> &raquo; <span class="level1">Bank Non PO Payment Activity
                        </span> &raquo; 
                        <span class="level2">Predefined Activity <br>
                        </span></td>
                    </tr>
                    <tr> 
                    ...
                    <tr> 
                      <td> 
                        <form name="frmbanknonpodetail" method ="post" action="">
                          <input type="hidden" name="command" value="<%=iJSPCommand%>">
                          <input type="hidden" name="vectSize" value="<%=vectSize%>">
                          <input type="hidden" name="start" value="<%=start%>">
                          <input type="hidden" name="prev_command" value="<%=prevJSPCommand%>">
                          <input type="hidden" name="hidden_banknonpo_activity_id" value="<%=oidActivityDetail%>">
                                                          ...
                                                          <tr> 
                                                            <td width="100%" class="page"> 
                                                              <table width="100%" border="0" cellpadding="1" height="20" cellspacing="1">
                                                                <tr> 
                                                                  <td class="tablehdr" height="20" width="27%"> 
                                                                    <div align="center"><b><font color="#FFFFFF">Expense</font></b></div>
                                                                  </td>
                                                                  <td width="50%" class="tablehdr" height="20"> 
																...
																%>
                                                                <tr> 
                                                                  <td width="27%" class="tablecell" height="17" nowrap valign="top"> 
                                                                    <b><%=coa.getCode()+" - "+coa.getName()%> 
                                                                    </b><br>
                                                                    <br>
                                                                    <p>&nbsp;</p>
                                                                    <p>&nbsp;</p>
                                                                  </td>
                                                                  <td width="50%" class="tablecell" height="17"> 
                                                                    <div align="center"> 
                                                                      <table width="57%" border="0" cellspacing="1" cellpadding="0">
                                                                        <tr> 
                                                                        ...
                                                                        <tr> 
                                                                          ...
                                                                          <td width="28%" height="19"><b>Module</b></td>
                                                                ...
                                                                <tr> 
                                                                  <td width="27%" class="tablecell1" height="17"> 
                                                                    <div align="right">&nbsp;&nbsp;</div>
                                                                  </td>
                                                                  <td width="50%" class="tablecell1" height="17">&nbsp; 
                                                                  </td>
                                                                </tr>
                                                                <%}}%>
                                                              </table>
                                                ...
                                                <td colspan="5"> 
                                                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                    ...
                                                    <tr> 
                                                      ...
                                                      <td width="43%"><a href="javascript:cmdBack('<%=oidBanknonpoPayment %>')" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('post1','','../images/cancel2.gif',1)"><img src="../images/cancel.gif" name="post1" height="23" border="0" width="63"></a></td>
                                                      <td width="46%"> 
                                                        <%if(!isOK){%>
					...
					out.println(e.toString());
                          ...
                          <script language="JavaScript">
				//cmdGetBalance();
				//document.frmbanknonpodetail.<%=jspBanknonpoPayment .colNames[jspBanknonpoPayment .JSP_AMOUNT]%>.value= formatFloat('<%=totalDetail%>', '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace); 
