...
<%
            ...
            boolean privAnggaran = true;//appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MN_CASH, AppMenu.M2_MN_BKK_ANGGARAN, AppMenu.PRIV_VIEW);
%>
<!-- Jsp Block -->
<%!
...
//================================================= item detail
            ...
            String[] langNav = {"Home", "BKK Approval", "Date", "Required", "SEARCHING", "DISBUSHMENT FORM", "ADVANCE FORM"};
            ...
            if (lang == LANG_ID) {
                ...
                langCT = langID;
                String[] navID = {"Home", "Approval BKK", "Tanggal", "Harus diisi", "PENCARIAN", "PENGAKUAN BIAYA", "PEMBERIAN KASBON"};
                langNav = navID;
...
<td valign="top"> 
    ...
    if (isLoad) {
        nom_jur = pettycashPayment.getJournalNumber();
                                        ...
                                        %>
                                        <tr> 
                                            <td colspan="3" height="23"><font face="arial"><b><u><%=(pettycashPayment.getType() != DbPettycashPayment.STATUS_TYPE_KASBON) ? langNav[5] : langNav[6]%></font></u></b></td>
        ...
        for (int iC = 0; iC < vCustomer.size(); iC++) {
            Customer objCustomer = (Customer) vCustomer.get(iC);
                                        ...
                                        <tr> 
                                            ...
                                            <%
    //out.println("pettycashPayment.getType() : "+pettycashPayment.getType());

    Employee emp = new Employee();
    if (pettycashPayment.getType() == DbPettycashPayment.STATUS_TYPE_KASBON) {
        try {
            emp = DbEmployee.fetchExc(pettycashPayment.getEmployeeId());
        } catch (Exception e) {
        }
    }

                                            %>
                                            <td width="13%" height="16"><%if (pettycashPayment.getType() == DbPettycashPayment.STATUS_TYPE_KASBON) {%>
                                                Pegawai 
                                            <%}%></td>
                                            <td width="38%" height="16"><%if (pettycashPayment.getType() == DbPettycashPayment.STATUS_TYPE_KASBON) {
    ...
    int totCancel = 0;
    try {
        totCancel = DbApprovalDoc.listTotalDocCancel(oidPettycashPayment);
    } catch (Exception e) {
    }

    ApprovalDoc appX = new ApprovalDoc();
    try {
        appX = DbApprovalDoc.listPersonCancel(oidPettycashPayment);
    } catch (Exception e) {
    }

    ApprovalDoc appLast = new ApprovalDoc();
    try {
        appLast = DbApprovalDoc.lastApp(oidPettycashPayment);
    } catch (Exception e) {
    }
                ...
                <table width="700" border="0" cellspacing="1" cellpadding="1">
                    ...
                    <tr> 
                        ...
                        <td width="13%" height="20" bgcolor="#F3F3F3"><b><font size="1"><%=langApp[3]%></font></b></td>
                        <td width="20%" height="20" bgcolor="#F3F3F3"><b><font size="1"><%=langApp[2]%></font></b></td>
