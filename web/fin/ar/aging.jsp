
<%@ page language="java"%>
<%@ page import = "java.util.*" %>
<%@ page import = "java.sql.*" %>
<%@ page import = "com.project.util.*" %>
<%@ page import = "com.project.util.jsp.*" %>
<%@ page import = "com.project.system.*" %>
<%@ page import = "com.project.crm.project.*" %>
<%@ page import = "com.project.crm.*" %>
<%@ page import = "com.project.fms.master.*" %>
<%@ page import = "com.project.fms.ar.*" %>
<%@ page import = "com.project.general.*" %>
<%@ page import = "com.project.main.db.*" %>
<%@ page import = "java.util.Date" %>
<%@ include file="../main/javainit.jsp"%>
<%@ include file="../main/check.jsp"%>
<%@ include file="../calendar/calendarframe.jsp"%>
<%@ page import = "com.project.I_Project" %>
<%
            boolean priv = QrUserSession.isHavePriviledge(appSessUser.getUserOID(), AppMenu.M1_MENU_AP, AppMenu.M2_MENU_AA);
            boolean privView = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_AP, AppMenu.M2_MENU_AA, AppMenu.PRIV_VIEW);
            boolean privUpdate = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_AP, AppMenu.M2_MENU_AA, AppMenu.PRIV_UPDATE);
            boolean privAdd = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_AP, AppMenu.M2_MENU_AA, AppMenu.PRIV_ADD);
            boolean privDelete = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_AP, AppMenu.M2_MENU_AA, AppMenu.PRIV_DELETE);
%>
<%
            if (session.getValue("AGE_ANALYSIS") != null) {
                session.removeValue("AGE_ANALYSIS");
            }

            int iJSPCommand = JSPRequestValue.requestCommand(request);
            long unitUsahaId = JSPRequestValue.requestLong(request, "src_unit_usaha_id");
            long oidCustomer = JSPRequestValue.requestLong(request, "customer_id");
            int ageDate = JSPRequestValue.requestInt(request, "age_date");            
            Vector vPayList = new Vector();

            String cstName = "";
            Customer cs = new Customer();
            try {
                cs = DbCustomer.fetchExc(oidCustomer);
                cstName = cs.getName();
            } catch (Exception e) {
            }

            if (iJSPCommand == JSPCommand.SUBMIT) {
                //vPayList = QrAR.list(oidCustomer, ageDate, ageRange, unitUsahaId);(long oidCustomer, int ageDate)
                vPayList = QrAR.listAgingAR(oidCustomer, ageDate, formNumbComp);
            }

            session.putValue("AGE_ANALYSIS", vPayList);
            session.putValue("AGE_ANALYSIS_UNIT_USAHA", "" + unitUsahaId);

            /*** LANG ***/
            String[] langAR = {"Customer", "Aged by", "Code", "Last Payment", "Current", //0-4
                "Over 30 days", "Over 60 days", "Over 90 days", "Over 120 days", "Balance", "Amount", "Date"}; //5-11

            String[] langNav = {"Account Receiveable", "Aging Analysis", "Data not found", "Searching Form :", "Click search button to seraching the data"};

            if (lang == LANG_ID) {
                String[] langID = {"Konsumen", "Dihitung dari", "Kode", "Pembayaran Terakhir", "Terkini", //0-4
                    "Lebih dari 30 hari", "Lebih dari 60 hari", "Lebih dari 90 hari", "Lebih dari 120 hari", "Saldo", "Jumlah", "Tanggal"}; //5-11
                langAR = langID;

                String[] navID = {"Piutang", "Aging Analysis", "Data tidak ditemukan", "Form Pencarian :", "Klik tombol search untuk mencari data"};
                langNav = navID;
            }
%>
<html >
    <!-- #BeginTemplate "/Templates/index.dwt" --> 
    <head>
        <!-- #BeginEditable "javascript" --> 
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
        <title><%=systemTitle%></title>
        <link href="../css/css.css" rel="stylesheet" type="text/css" />
        <script type="text/javascript">
            <%if (!priv || !privView) {%>
            window.location="<%=approot%>/nopriv.jsp";
            <%}%>
            
            function cmdPrint(){	 
                window.open("<%=printroot%>.report.RptAccReceivableAgingXLS?oid=<%=sysCompany.getOID()%>&age=<%=ageDate%>");
                }
                
                function cmdSearchCustomer(){
                    var cstName = document.frmaging.cst_name.value;  
                    window.open("srckonsumen.jsp?cst_name=\'"+cstName+"'&command=<%=JSPCommand.SEARCH%>&frm_cst_id=customer_id&form_name=frmaging", null, "height=400,width=700, status=yes,toolbar=no,menubar=no,location=no, scrollbars=yes");
                    }  
                    
                    function cmdSearch(){
                        document.frmaging.command.value="<%=JSPCommand.SUBMIT%>";
                        document.frmaging.action="aging.jsp";
                        document.frmaging.submit();
                    }
                    
                    function MM_swapImgRestore() { //v3.0
                        var i,x,a=document.MM_sr; for(i=0;a&&i<a.length&&(x=a[i])&&x.oSrc;i++) x.src=x.oSrc;
                    }
                    function MM_preloadImages() { //v3.0
                        var d=document; if(d.images){ if(!d.MM_p) d.MM_p=new Array();
                            var i,j=d.MM_p.length,a=MM_preloadImages.arguments; for(i=0; i<a.length; i++)
                            if (a[i].indexOf("#")!=0){ d.MM_p[j]=new Image; d.MM_p[j++].src=a[i];}}
                    }
                    
                    function MM_findObj(n, d) { //v4.01
                        var p,i,x;  if(!d) d=document; if((p=n.indexOf("?"))>0&&parent.frames.length) {
                            d=parent.frames[n.substring(p+1)].document; n=n.substring(0,p);}
                        if(!(x=d[n])&&d.all) x=d.all[n]; for (i=0;!x&&i<d.forms.length;i++) x=d.forms[i][n];
                        for(i=0;!x&&d.layers&&i<d.layers.length;i++) x=MM_findObj(n,d.layers[i].document);
                        if(!x && d.getElementById) x=d.getElementById(n); return x;
                    }
                    
                    function MM_swapImage() { //v3.0
                        var i,j=0,x,a=MM_swapImage.arguments; document.MM_sr=new Array; for(i=0;i<(a.length-2);i+=3)
                        if ((x=MM_findObj(a[i]))!=null){document.MM_sr[j++]=x; if(!x.oSrc) x.oSrc=x.src; x.src=a[i+2];}
                    }
                    //-->
        </script>
        <!-- #EndEditable --> 
    </head>
    <body onLoad="MM_preloadImages('<%=approot%>/images/home2.gif','<%=approot%>/images/logout2.gif','../images/search2.gif')">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" height="100%">
            <tr> 
                <td valign="top"> 
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%">
                        <tr> 
                            <td height="96"> <!-- #BeginEditable "header" --> 
            <%@ include file="../main/hmenu.jsp"%>
                            <!-- #EndEditable --> </td>
                        </tr>
                        <tr> 
                            <td valign="top"> 
                                <table width="100%" border="0" cellpadding="0" cellspacing="0" height="100%">
                                    <!--DWLayoutTable-->
                                    <tr> 
                                        <td width="165" height="100%" valign="top" style="background:url(<%=approot%>/images/leftmenu-bg.gif) repeat-y"> 
                                            <!-- #BeginEditable "menu" --> 
                  <%@ include file="../main/menu.jsp"%>
                                        <!-- #EndEditable --> </td>
                                        <td width="100%" valign="top"> 
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                <tr> 
                                                    <td class="title"><!-- #BeginEditable "title" -->
                                           <%
            String navigator = "<font class=\"lvl1\">" + langNav[0] + "</font><font class=\"tit1\">&nbsp;&raquo;&nbsp;<span class=\"lvl2\">" + langNav[1] + "</span></font>";
                                           %>
                                           <%@ include file="../main/navigator.jsp"%>
                                                    <!-- #EndEditable --></td>
                                                </tr>
                                                <!--tr> 
                      <td><img src="<%=approot%>/images/title-sp.gif" width="584" height="1"></td> 
                                                </tr-->
                                                <tr> 
                                                    <td><!-- #BeginEditable "content" --> 
                                                        <table width="100%" border="0" cellpadding="0" cellspacing="0" height="100%">
                                                            <tr> 
                                                                <td valign="top"> 
                                                                    <table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%">
                                                                        <tr> 
                                                                            <td valign="top"> 
                                                                                <table width="100%" border="0" cellpadding="0" cellspacing="0" height="100%">
                                                                                    <!--DWLayoutTable-->
                                                                                    <tr> 
                                                                                        <td width="100%" valign="top"> 
                                                                                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                                                                <tr> 
                                                                                                    <td> 
                                                                                                        <form id="form1" name="frmaging" method="post" action="">
                                                                                                            <input type="hidden" name="command">
                                                                                                            <input type="hidden" name="menu_idx" value="<%=menuIdx%>">     
                                                                                                            <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                                                                                                <tr> 
                                                                                                                    <td class="container"> 
                                                                                                                        <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                                                                                                            <tr> 
                                                                                                                                <td > 
                                                                                                                                    <table border="0" cellpadding="1" cellspacing="1" width="300">                                                                                                                                        
                                                                                                                                        <tr>                                                                                                                                            
                                                                                                                                            <td class="tablecell1" > 
                                                                                                                                                <table width="100%" border="0" style="border:1px solid #ABA8A8" cellspacing="2" cellpadding="1">
                                                                                                                                                    <tr> 
                                                                                                                                                        <td >&nbsp;</td>
                                                                                                                                                        <td colspan="2" class="fontarial"><i><b><%=langNav[3]%></b></i></td>
                                                                                                                                                    </tr>    
                                                                                                                                                    <tr>
                                                                                                                                                        <td width="5">&nbsp;</td>
                                                                                                                                                        <td width="80" class="fontarial"><%=langAR[0]%></td>
                                                                                                                                                        <td > 
                                                                                                                                                            <input type="hidden" name="customer_id" value="<%=oidCustomer%>">
                                                                                                                                                            <table border="0" cellpadding="0" cellspacing="0">
                                                                                                                                                                <tr>
                                                                                                                                                                    <td>
                                                                                                                                                                        <input type="text" name="cst_name" size="20" value="<%=cstName%>">
                                                                                                                                                                    </td>
                                                                                                                                                                    <td>
                                                                                                                                                                        &nbsp;<a href="javascript:cmdSearchCustomer()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('new21','','../images/search.jpg',1)"><img src="../images/search2.jpg" name="new211" height="17" border="0" style="padding:0px"></a> 
                                                                                                                                                                    </td>
                                                                                                                                                                </tr>
                                                                                                                                                            </table>   
                                                                                                                                                        </td>
                                                                                                                                                    </tr>
                                                                                                                                                    <tr> 
                                                                                                                                                        <td >&nbsp;</td>
                                                                                                                                                        <td class="fontarial"><%=langAR[1]%></td>
                                                                                                                                                        <td > 
                                                                                                                                                            <%try {%>
                                                                                                                                                            <select name="age_date">
                                                                                                                                                                <%for (int i = 0; i < I_Project.ageDateStr.length; i++) {%>
                                                                                                                                                                <option value="<%=i%>" <%if (i == ageDate) {%>selected<%}%>><%=I_Project.ageDateStr[i]%></option>
                                                                                                                                                                <%}%>
                                                                                                                                                            </select>
                                                                                                                                                            <%} catch (Exception e) {
                out.println(e.toString());
            }

                                                                                                                                                            %>                                                                  
                                                                                                                                                        </td>                                                                                                                                                        
                                                                                                                                                    </tr>
                                                                                                                                                    <tr> 
                                                                                                                                                        <td height="10" colspan="3"></td>                                                                                                                                                        
                                                                                                                                                    </tr>
                                                                                                                                                </table>
                                                                                                                                            </td>
                                                                                                                                        </tr>
                                                                                                                                    </table>
                                                                                                                                </td>
                                                                                                                            </tr>
                                                                                                                            <tr> 
                                                                                                                                <td ><a href="javascript:cmdSearch()"  onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('new2','','../images/search2.gif',1)"><img src="../images/search.gif" name="new2" border="0"></a></td>
                                                                                                                            </tr>
                                                                                                                            <tr> 
                                                                                                                                <td height="30">&nbsp;</td>
                                                                                                                            </tr>                                                                                                                
                                                                                                                            <tr> 
                                                                                                                                <td> 
                                                                                                                                    <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                                                                                                                        <tr> 
                                                                                                                                            <td rowspan="2" class="tablehdr" height="35" width="25">No</td>
                                                                                                                                            <td rowspan="2" class="tablehdr" height="35" width="10%"><%=langAR[2]%></td>
                                                                                                                                            <td rowspan="2" class="tablehdr" height="35" width="18%"><%=langAR[0]%></td>
                                                                                                                                            <td colspan="2" class="tablehdr" height="17"><%=langAR[3]%></td>
                                                                                                                                            <td rowspan="2" class="tablehdr" height="35" width="9%"><%=langAR[4]%></td>
                                                                                                                                            <td rowspan="2" class="tablehdr" height="35" width="8%"><%=langAR[5]%></td>
                                                                                                                                            <td rowspan="2" class="tablehdr" height="35" width="9%"><%=langAR[6]%></td>
                                                                                                                                            <td rowspan="2" class="tablehdr" height="35" width="8%"><%=langAR[7]%></td>
                                                                                                                                            <td rowspan="2" class="tablehdr" height="35" width="9%"><%=langAR[8]%></td>
                                                                                                                                            <td rowspan="2" class="tablehdr" height="35" ><%=langAR[9]%></td>
                                                                                                                                        </tr>
                                                                                                                                        <tr> 
                                                                                                                                            <td width="8%" class="tablehdr" height="17"><%=langAR[10]%></td>
                                                                                                                                            <td width="7%" class="tablehdr" height="17"><%=langAR[11]%></td>
                                                                                                                                        </tr>
                                                                                                                                        <%
            if (iJSPCommand == JSPCommand.SUBMIT) {

                double totalAmount = 0;
                double totalCurrent = 0;
                double totalOver30 = 0;
                double totalOver60 = 0;
                double totalOver90 = 0;
                double totalOver120 = 0;

                if (vPayList != null && vPayList.size() > 0) {
                    for (int i = 0; i < vPayList.size(); i++) {
                        SesAgingAnalysis vAgingAnalysis = (SesAgingAnalysis) vPayList.get(i);

                        String tableClass = "tablecell1";
                        if (i % 2 != 0) {
                            tableClass = "tablecell";
                        }

                        totalAmount = totalAmount + vAgingAnalysis.getLastPaymentAmount();
                        totalCurrent = totalCurrent + vAgingAnalysis.getAgeCurrent();
                        totalOver30 = totalOver30 + vAgingAnalysis.getAgeOver30();
                        totalOver60 = totalOver60 + vAgingAnalysis.getAgeOver60();
                        totalOver90 = totalOver90 + vAgingAnalysis.getAgeOver90();
                        totalOver120 = totalOver120 + vAgingAnalysis.getAgeOver120();
                                                                                                                                        %>
                                                                                                                                        <tr height="22"> 
                                                                                                                                            <td class="<%=tableClass%>"> 
                                                                                                                                                <div align="center"><%=i + 1%></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="<%=tableClass%>" nowrap><%=(vAgingAnalysis.getCustomerCode() == null) ? "" : vAgingAnalysis.getCustomerCode()%></td>
                                                                                                                                            <td class="<%=tableClass%>"><%=vAgingAnalysis.getCustomerName()%></td>
                                                                                                                                            <td class="<%=tableClass%>"> 
                                                                                                                                                <div align="right"><%=(vAgingAnalysis.getLastPaymentAmount() == 0) ? "" : JSPFormater.formatNumber(vAgingAnalysis.getLastPaymentAmount(), "#,###")%></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="<%=tableClass%>"> 
                                                                                                                                                <div align="center"><%=(vAgingAnalysis.getLastPaymentDate() == null) ? "" : JSPFormater.formatDate(vAgingAnalysis.getLastPaymentDate(), "dd MMM yyyy")%></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="<%=tableClass%>"> 
                                                                                                                                                <div align="right"><%=(vAgingAnalysis.getAgeCurrent() == 0) ? "" : JSPFormater.formatNumber(vAgingAnalysis.getAgeCurrent(), "#,###")%></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="<%=tableClass%>"> 
                                                                                                                                                <div align="right"><%=(vAgingAnalysis.getAgeOver30() == 0) ? "" : JSPFormater.formatNumber(vAgingAnalysis.getAgeOver30(), "#,###")%></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="<%=tableClass%>"> 
                                                                                                                                                <div align="right"><%=(vAgingAnalysis.getAgeOver60() == 0) ? "" : JSPFormater.formatNumber(vAgingAnalysis.getAgeOver60(), "#,###")%></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="<%=tableClass%>"> 
                                                                                                                                                <div align="right"><%=(vAgingAnalysis.getAgeOver90() == 0) ? "" : JSPFormater.formatNumber(vAgingAnalysis.getAgeOver90(), "#,###")%></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="<%=tableClass%>"> 
                                                                                                                                                <div align="right"><%=(vAgingAnalysis.getAgeOver120() == 0) ? "" : JSPFormater.formatNumber(vAgingAnalysis.getAgeOver120(), "#,###")%></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="<%=tableClass%>">                                                                                                                                                  
                                                                                                                                                    <%
                                                                                                                                                    double total = 0;
                                                                                                                                                    total = vAgingAnalysis.getAgeCurrent() + vAgingAnalysis.getAgeOver30() + vAgingAnalysis.getAgeOver60() + vAgingAnalysis.getAgeOver90() + vAgingAnalysis.getAgeOver120();
                                                                                                                                                    %>                                                                                                                                                
                                                                                                                                                <div align="right"><%=(total == 0) ? "" : JSPFormater.formatNumber(total, "#,###")%></div>
                                                                                                                                            </td>
                                                                                                                                        </tr>
                                                                                                                                        <%}%>
                                                                                                                                        <tr bgcolor="#CCCCCC" height="22"> 
                                                                                                                                            <td ><font size="1"></font></td>
                                                                                                                                            <td ><font size="1"></font></td>
                                                                                                                                            <td > 
                                                                                                                                                <div align="center" class="fontarial"><b>T O T A L : </b></div>
                                                                                                                                            </td>
                                                                                                                                            <td >&nbsp;</td>
                                                                                                                                            <td >&nbsp;</td>
                                                                                                                                            <td class="fontarial"> 
                                                                                                                                                <div align="right"><b><%=(totalCurrent == 0) ? "" : JSPFormater.formatNumber(totalCurrent, "#,###")%></b></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="fontarial"> 
                                                                                                                                                <div align="right"><font size="1"><b><%=(totalOver30 == 0) ? "" : JSPFormater.formatNumber(totalOver30, "#,###")%></b></font></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="fontarial"> 
                                                                                                                                                <div align="right"><b><%=(totalOver60 == 0) ? "" : JSPFormater.formatNumber(totalOver60, "#,###")%></b></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="fontarial"> 
                                                                                                                                                <div align="right"><b><%=(totalOver90 == 0) ? "" : JSPFormater.formatNumber(totalOver90, "#,###")%></b></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="fontarial"> 
                                                                                                                                                <div align="right"><b><%=(totalOver120 == 0) ? "" : JSPFormater.formatNumber(totalOver120, "#,###")%></b></div>
                                                                                                                                            </td>
                                                                                                                                            <td class="fontarial"> 
                                                                                                                                                    <%
                                                                                                                                                double grandTotal = 0;
                                                                                                                                                grandTotal = totalCurrent + totalOver30 + totalOver60 + totalOver90 + totalOver120;
                                                                                                                                                    %>                                                                                                                                                
                                                                                                                                                <div align="right"><b><%=(grandTotal == 0) ? "" : JSPFormater.formatNumber(grandTotal, "#,###")%></b></div>
                                                                                                                                            </td>
                                                                                                                                        </tr>
                                                                                                                                        <tr>
                                                                                                                                            <td colspan="11">&nbsp;</td>
                                                                                                                                        </tr>
                                                                                                                                        <tr>
                                                                                                                                            <td colspan="11">
                                                                                                                                                <a href="javascript:cmdPrint()"  onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('new21','','../images/printxls2.gif',1)"><img src="../images/printxls.gif" name="new21" border="0"></a>
                                                                                                                                            </td>
                                                                                                                                        </tr>
                                                                                                                                        <%}%>
                                                                                                                                        <%} else {%>
                                                                                                                                        <tr> 
                                                                                                                                            <td colspan="11" class="tablecell1"><b><i><%=langNav[4]%></i></b></td>
                                                                                                                                        </tr>
                                                                                                                                        <%}%>
                                                                                                                                    </table>
                                                                                                                                </td>
                                                                                                                            </tr>
                                                                                                                            <tr> 
                                                                                                                                <td>&nbsp;</td>
                                                                                                                            </tr>
                                                                                                                        </table>
                                                                                                                    </td>
                                                                                                                </tr>
                                                                                                            </table>
                                                                                                        </form>
                                                                                                    </td>
                                                                                                </tr>
                                                                                                <tr> 
                                                                                                    <td>&nbsp;</td>
                                                                                                </tr>
                                                                                            </table>
                                                                                        </td>
                                                                                    </tr>
                                                                                </table>
                                                                            </td>
                                                                        </tr>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                        </table>
                                                    <!-- #EndEditable --> </td>
                                                </tr>
                                                <tr> 
                                                    <td>&nbsp;</td>
                                                </tr>
                                            </table>
                                        </td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                        <tr> 
                            <td height="25"> <!-- #BeginEditable "footer" --> 
            <%@ include file="../main/footer.jsp"%>
                            <!-- #EndEditable --> </td>
                        </tr>
                    </table>
                </td>
            </tr>
        </table>
    </body>
    <!-- #EndTemplate --> 
</html>

