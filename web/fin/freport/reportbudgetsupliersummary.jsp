    ...
    <head>
        ...
        function cmdPrintJournal(){	                       
            ...
            function cmdPrintJournalXLS(){	                       
                ...
                function cmdPrintJournalBg(vendorId, tot){
                ...
                //return false;
                window.open("<%=printroot%>.report.PrintBgVBudgetXLS?user_id=<%=appSessUser.getUserOID()%>&vendorId="+vendorId+"&lang=<%=lang%>&materai="+materai+"&tot="+tot+"&datefrom=<%=JSPFormater.formatDate(dateFrom, "dd-MM-yyyy")%>&dateto=<%=JSPFormater.formatDate(dateTo, "dd-MM-yyyy")%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                }
                function cmdPrintJournalCheck(vendorId, tot){
                    var confirmMaterai = confirm("===Print BG===\nInclude materai?");
                    if(confirmMaterai==true){
                        var materai='1';
                    }else{
                ...
                //return false;
                window.open("<%=printroot%>.report.PrintCheckBudgetXLS?user_id=<%=appSessUser.getUserOID()%>&vendorId="+vendorId+"&lang=<%=lang%>&materai="+materai+"&tot="+tot+"&datefrom=<%=JSPFormater.formatDate(dateFrom, "dd-MM-yyyy")%>&dateto=<%=JSPFormater.formatDate(dateTo, "dd-MM-yyyy")%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                }
                
                // select Vendor
                function cmdPrintJournalBg1(){
                    tot=document.form1.total.value;
                    tot=tot.replace(",","");
                    if (tot==0){
                        alert ("Anda belum memilih budget");
                    }else{
                    var confirmMaterai = confirm("===Print BG===\nInclude materai?");
                    if(confirmMaterai==true){
                        var materai='1';
            ...
            function cmdPrintJournalCheck1(){
                tot=document.form1.total.value;
                tot=tot.replace(",","");
                if (tot==0){
                    alert ("Anda belum memilih budget");
        ...
        function cmdSelectAll(){
            var val = document.form1.bg_checkall.checked;
            var bg1 = document.form1.bg;
            if (bg1.length>0){
                for (ij = 0; ij < bg1.length; ij++) {
                    bg1[ij].checked=val;
                }
            }else{
            document.form1.bg.checked=val;
        }
        
        cmdSumChecked();
    }
    
    function cmdSumChecked(){
        var bgx = document.form1.bg;
        var totalbg = 0;
        var i;
        if (bgx.length>0){
            for (i = 0; i < bgx.length; i++) {
                if (bgx[i].checked) {
                    totalbg = eval(totalbg) + eval(bgx[i].value);
                }
            }
        }else{
        if (document.form1.bg.checked) {
            totalbg = eval(document.form1.bg.value);
        }
    }
    document.form1.total.value=formatNum(Math.round(totalbg));
}
//-->
        </script>
        <!-- #EndEditable --> 
    ...
    for (int ix = 0; ix < result.size(); ix++) {
        Vendor vendor = (Vendor) result.get(ix);
                                                                            ...
                                                                            <tr> 
                                                                                <td class="tablecell">&nbsp;&nbsp;Transaction Date</td>
                                                                                <td >
                                                                            ...
                                                                            <tr> 
                                                                                <td class="tablecell">&nbsp;&nbsp;Tipe Suplier</td>
                                                                                <td align="left">
                                                                                    <table border="0" cellpadding="0" cellspacing="0">
                                                                                        <tr>
                                                                            ...
                                                                            <tr> 
                                                                                <td class="tablecell1">&nbsp;&nbsp;History Print</td>
                                                                                <td>
                                                                                ...
                                                                                <td colspan="2">
                                                                                    <table width="1000" cellpadding="0" cellspacing="1">                                                                                        
                                                                                        <tr height="25">
                                                                                            ...
                                                                                            <td class="tablehdr" width="10%">PRINT</td>
                                                                                            <td class="tablehdr" width="10%">Select<br/><input name="bg_checkall" value="1" type="checkbox" onChange="cmdSelectAll()">
                                                                                            </td>
                                                                                        </tr>
                                                                                        <%
    double totAmount = 0;
        ...
        try {
            idx = Integer.parseInt(String.valueOf(vendorIdx.get("" + srbs.getVendorId())));
                                                                                        ...
                                                                                        <tr height="20">
                                                                                            ...
                                                                                            <td  class="tablecell" align="center">
                                                                                                <a href="javascript:cmdPrintJournalCheck('<%=srbs.getVendorId()%>','<%=JSPFormater.formatNumber(srbs.getValue(), "####")%>')">Cheque</a> |
                                                                                                <a href="javascript:cmdPrintJournalBg('<%=srbs.getVendorId()%>','<%=JSPFormater.formatNumber(srbs.getValue(), "####")%>')">BG</a>
                                                                                            </td>
                                                                                            <td  class="tablecell" align="center"><input name="bg" value="<%=srbs.getValue()%>" type="checkbox" onchange="cmdSumChecked()"></td>
                                                                                        </tr>                                                                                                                                                                                
                                                                                        <%
    }
                                                                                        %>
                                                                                ...
                                                                                <td colspan="2">
                                                                                    <table border="0" cellpadding="0" cellspacing="0" width="1000">
                                                                                        <tr>
                                                                                            ...
                                                                                            <td width="10"><a href="javascript:cmdGenerate()"><img src="../images/search2.jpg" width="22" height="22" border="0"></a></td>
                                                                                            <td width="150"><a href="javascript:cmdGenerate()">&nbsp;Generate</a></td>
                                                                                            <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                                                                            <td>
                                                                                                <table align="right" height="10" cellpadding="0" cellspacing="0">
                                                                                                    <tr>
                                                                                                        <td class="tablehdr" width="300">Total selected amount <input type="text" name="total" value="0" disabled="true" length="15" style="text-align:right; font-weight:bold;"></td>
                                                                                                        <td class="tablehdr">
                                                                                                            &nbsp;
                                                                                                            <a href="javascript:cmdPrintJournalBg1()" style="color:#ffffff;text-decoration:none;">PRINT BG</a>
                                                                                                            &nbsp; | &nbsp;
                                                                                                            <a href="javascript:cmdPrintJournalCheck1()" style="color:#ffffff;text-decoration:none;">PRINT CHEQUE</a>
                                                                                                        </td>
                                                                                                    </tr>
                                                                                                </table>
                                                                                            </td>
