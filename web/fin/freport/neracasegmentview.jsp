...
<%-- 
    Document   : neracasegmentview
    Created on : Jul 15, 2013, 9:34:57 AM
    Author     : Roy Andika
--%>
...
<html >
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function cmdGO(){                
                ...
                document.form1.action="neracasegmentview.jsp?menu_idx=<%=menuIdx%>";
    ...
    for (int x = 0; x < I_Project.accGroup.length; x++) {
        ...
        if (I_Project.accGroup[x].equals(I_Project.ACC_GROUP_LIQUID_ASSET) || I_Project.accGroup[x].equals(I_Project.ACC_GROUP_FIXED_ASSET) || I_Project.accGroup[x].equals(I_Project.ACC_GROUP_OTHER_ASSET)) {
            tempCoas = DbCoa.listCoaNeraca(DbCoa.colNames[DbCoa.COL_ACCOUNT_GROUP] + "='" + I_Project.accGroup[x] + "' and  "+DbCoa.colNames[DbCoa.COL_LEVEL]+" = 1 ", DbCoa.colNames[DbCoa.COL_CODE]);
        }
        ...
        if (tempCoas != null && tempCoas.size() > 0) {            
            for (int i = 0; i < tempCoas.size(); i++) {
                ...
                //    amountLY = amountLY + DbCoaOpeningBalanceLocation.getOpeningBalance(lastPeriod, coa.getCoaId(), 0);
                ...
                //amountTY = amountTY + SessViewReport.getAmountInPeriod(period.getOID(), coa.getCoaId(), coa.getLevel(), coa.getAccountGroup());
        ...
        if (tempCoas != null && tempCoas.size() > 0) {
            ...
            for (int i = 0; i < tempCoas.size(); i++) {
                ...
                if (coaLabaBerjalan.getCode().equals(coa.getCode())) {
                    if (lastPeriod.getOID() != 0) {                        
                        amountTY = amountTY + DbCoaOpeningBalanceLocation.getOpeningBalance(lastPeriod, coa.getCoaId(), 0);
                ...
                if (isOpen) {
                    seq = seq + 1;
                                                                                        ...
                                                                                        <tr> 
                                                                                            <td colspan=4>
                                                                                                <% out.print("<a href=\"../freport/neraca_penjelasan_print_priview.jsp?rpt_format_id=" + oidRptFormat + "&period_id=" + periodId + "&preview_type=" + previewType + "\"  onclick=\"return hs.htmlExpand(this,{objectType: 'ajax'})\"><img src=\"../images/print.gif\" name=\"print\" height=\"22\" border=\"0\"></a></div>");%>
                                                                                            </td>
                                                                                        </tr>
