...
%>
<%
	if(session.getValue("PROFIT1")!=null){
		session.removeValue("PROFIT1");
	}
	...
	if(vDep!=null && vDep.size()>0){
		...
		colSpan = loopingDep+loopingSec;
	}
	
	Vector vSesDep = new Vector();

/*** LANG ***/
String[] langFR = {"Show List", "Account With Transaction", "All", "PROFIT & LOSS STATEMENT", "PERIOD", //0-4
"Description", "P&L Prev. Period", "Total Corporate", "Net Income"}; //5-8
String[] langNav = {"Financial Report", "Profit & Loss"};

if(lang == LANG_ID) {
	String[] langID = {"Tampilkan Daftar", "Perkiraan Dengan Transaksi", "Semua", "LAPORAN LABA RUGI", "PERIODE", //0-4
	"Keterangan", "Laba Rugi Periode Sebelumnya", "Total", "Pendapatan Bersih"}; //5-8
	langFR = langID;
...
function cmdChangeList(){
	document.frmcoa.action="profitloss1.jsp";
	document.frmcoa.submit();
}

function cmdPrintJournal(){	 
	window.open("<%=printroot%>.report.RptProfitLoss1XLS?oid=<%=appSessUser.getLoginId()%>");
}
                                  ...
                                  <tr align="left" valign="top"> 
                                    <td height="22" valign="middle" colspan="3"> 
                                      <table width="100%" border="0" cellpadding="0" height="20" cellspacing="1">                                        
                                        <tr> 
                                          <td width="400" class="tablehdr" height="22" rowspan="2" nowrap align="center"><div align="center"><b><font color="#FFFFFF"><%=langFR[5]%></font></b></div></td>
										  <td width="150" class="tablehdr" height="22" rowspan="2" nowrap><%=langFR[6]%></td>
										  <%
											  	if(vDep!=null && vDep.size()>0){
													for(int i=0; i<vDep.size(); i++){
														Department dep = (Department)vDep.get(i);												
														Vector vSec = DbDepartment.list(0,0,"level=1 and ref_id="+dep.getOID(),"code");
										  %>
										  <td width="150" class="tablehdr" height="22" colspan="<%=vSec.size()%>" nowrap><div align="center"><%=dep.getName()%></div></td>
										  <%	}	}	%>
										  <td width="150" class="tablehdr" height="22" rowspan="2" nowrap><%=langFR[7]%></td>
                                        </tr>
										<tr>											
										  <%
											  	if(vDep!=null && vDep.size()>0){
													for(int i=0; i<vDep.size(); i++){
														...
														if(vSec!=null && vSec.size()>0){
															for(int ia=0; ia<vSec.size(); ia++){
																Department depSec = (Department)vSec.get(ia);												
										  %>
													<td width="140" class="tablehdr" height="20" nowrap><div align="center"><b><font color="#FFFFFF"><%=depSec.getName()%></font></b></div></td>										  
										  <%				}	
										  				} else
														{
										  %>
...
<!--level ACC_GROUP_REVENUE-->
                                        ...
                                        <tr> 
                                          ...
                                          <td class="tablecell" colspan="<%=colSpan+2%>"></td>
                                        </tr>									
										<% 	} %>
					<%
											  	...
											  	if(vDep!=null && vDep.size()>0){
													for(int ix=0; ix<vDep.size(); ix++){
														...
														if(vSec!=null && vSec.size()>0){
															for(int ia=0; ia<vSec.size(); ia++){
																Department depSec = (Department)vSec.get(ia);												
																amount = DbCoa.getCoaBalance(coa.getOID(),dep.getOID(),depSec.getOID(),"CD");
											  	...
											  	if(vDep!=null && vDep.size()>0){
													for(int ix=0; ix<vDep.size(); ix++){
														...
														if(vSec!=null && vSec.size()>0){
															for(int ia=0; ia<vSec.size(); ia++){
																Department depSec = (Department)vSec.get(ia);												
																double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_REVENUE,"CD",dep.getOID(),depSec.getOID());																
					...
					%>
                                        <tr> 
                                          ...
                                          <td class="tablecell1" colspan="<%=colSpan+2%>"></td>
										</tr>
										<%	}	%>
<!--level 2-->
													...
													for(int ix=0; ix<vDep.size(); ix++){
														...
														if(vSec!=null && vSec.size()>0){
															for(int ia=0; ia<vSec.size(); ia++){
																...
																//System.out.println("depsec.getOID() = "+depSec.getOID());														
																//System.out.println("amount = "+amount);													
																amount = DbCoa.getCoaBalance(coa.getOID(),dep.getOID(),depSec.getOID(),"DC");
																//System.out.println("amount = "+amount);
																displayStr1 = strDisplay(amount, coa.getStatus());
																vSesDep.add(""+amount);
																//total by section
											  	...
											  	if(vDep!=null && vDep.size()>0){
													for(int ix=0; ix<vDep.size(); ix++){
														...
														if(vSec!=null && vSec.size()>0){
															for(int ia=0; ia<vSec.size(); ia++){
																Department depSec = (Department)vSec.get(ia);												
																double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_COST_OF_SALES,"DC",dep.getOID(),depSec.getOID());																
																displayStr1 = strDisplay(amount, "");
																vSesDep.add(""+amount);
																//total by section
										  %>
											  	...
											  	if(vDep!=null && vDep.size()>0){
													for(int ix=0; ix<vDep.size(); ix++){
														...
														if(vSec!=null && vSec.size()>0){
															for(int ia=0; ia<vSec.size(); ia++){
																Department depSec = (Department)vSec.get(ia);												
																double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_EXPENSE,"DC",dep.getOID(),depSec.getOID());																
																displayStr1 = strDisplay(amount, "");
																vSesDep.add(""+amount);
																//total by section
										  %>
											  	...
											  	if(vDep!=null && vDep.size()>0){
													for(int ix=0; ix<vDep.size(); ix++){
														...
														if(vSec!=null && vSec.size()>0){
															for(int ia=0; ia<vSec.size(); ia++){
																Department depSec = (Department)vSec.get(ia);												
																double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_OTHER_REVENUE,"CD",dep.getOID(),depSec.getOID());																
																displayStr1 = strDisplay(amount, "");
																vSesDep.add(""+amount);
																//total by section
										  %>
											  	...
											  	if(vDep!=null && vDep.size()>0){
													for(int ix=0; ix<vDep.size(); ix++){
														...
														if(vSec!=null && vSec.size()>0){
															for(int ia=0; ia<vSec.size(); ia++){
																Department depSec = (Department)vSec.get(ia);												
																double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_OTHER_EXPENSE,"DC",dep.getOID(),depSec.getOID());																
																displayStr1 = strDisplay(amount, "");
																vSesDep.add(""+amount);
																//total by section
										  %>
					...
					%>
                                        ...
                                        <tr> 
										  ...
										  <%
												...
												String displayStrNet = strDisplay(amountNet, "");
										  %>
                                          <td class="tablecell2" align="right">
											<table width="100%" border="0" cellpadding="0" cellspacing="1">
											  <tr>
											  	<td width="5%"></td>
												<td width="90%" class="tablecell2"><div align="right"><b><%=displayStrNet%></b></div></td>
												<td width="5%"></td>
											  </tr>
											</table>
										  </td>
													...
													for(int ix=0; ix<vDep.size(); ix++){
														...
														if(vSec!=null && vSec.size()>0){
															for(int ia=0; ia<vSec.size(); ia++){
																...
																//double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_OTHER_EXPENSE,"DC",dep.getOID(),depSec.getOID());																
																double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_REVENUE,"CD",dep.getOID(),depSec.getOID())-
																		 DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_COST_OF_SALES,"DC",dep.getOID(),depSec.getOID())-
																		 DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_EXPENSE,"DC",dep.getOID(),depSec.getOID())+
																		 DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_OTHER_REVENUE,"CD",dep.getOID(),depSec.getOID())-
																		 DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_OTHER_EXPENSE,"DC",dep.getOID(),depSec.getOID());																
																//System.out.println("amount = "+amount);
																displayStr1 = strDisplay(amount, "");
																vSesDep.add(""+amount);
																//total by section
                                          ...
                                          <td class="tablecell2" align="right">
											<table width="100%" border="0" cellpadding="0" cellspacing="1">
											  <tr>
											  	<td width="5%"></td>
												<td width="90%" class="tablecell2"><div align="right"><b><%=displayStr1%><%displayStr1="";%><%displayStr1="";%></b></div></td>
												<td width="5%"></td>
											  </tr>
											</table>
										  </td>
							...
							<%
								session.putValue("PROFIT1", listReport);							
