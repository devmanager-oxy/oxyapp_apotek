...
<%-- 
    Document   : print_neraca_rpt
    Created on : Aug 22, 2011, 10:36:04 AM
    Author     : Roy Andika
--%>
            ...
            <%if (rptDetails != null && rptDetails.size() > 0) {
                ...
                for (int i = 0; i < rptDetails.size(); i++) {
                    ...
                    /*if (rpd.getType() == DbRptFormatDetail.RPT_TYPE_LABA_TAHUN_BERJALAN) {
                        ...
                        amountLY = DbGlDetail.getCurrentYearEarningLastYear(rpd.getOID(), period);
                        budgetTY = 0;//DbCoaBudget.getRealisasiCurrentYear(rpd.getOID(), period);
                        amountTY = DbGlDetail.getCurrentYearEarningCurrentYear(rpd.getOID(), period);

                    } else if (rpd.getType() == DbRptFormatDetail.RPT_TYPE_LABA_TAHUN_LALU) {

                        //out.println("in laba tahun lalu");	
                        amountLY = DbGlDetail.getLastYearEarningLastYear(rpd.getOID(), period);
                        budgetTY = 0;
                        amountTY = DbGlDetail.getLastYearEarningCurrentYear(rpd.getOID(), period);
                    ...
                    if (rpd.getType() == DbRptFormatDetail.RPT_TYPE_TOTAL) {
                       ...
                       // amountLY = totalAmountLY;
                        //budgetTY = totalBudgetTY;
                        //amountTY = totalAmountTY;
