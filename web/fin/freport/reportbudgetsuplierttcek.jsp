...
<%-- 
    Document   : reportbudgetsuplierttcek
    Created on : Oct 21, 2015, 3:40:21 PM
    Author     : Roy
--%>
...
<%
            ...
            dtTrans2.setTransDate(dateFrom);
            dateTrans.add(dtTrans1);
            dateTrans.add(dtTrans2);

            session.removeValue("PRINT_REPORT_BUDGET_TT");
            ...
            String[] langNav = {"Report", "Budget Report Suplier (CHECK & BG)"};
            ...
            if (lang == LANG_ID) {
                ...
                langRp = langID;
                String[] navID = {"Laporan", "Laporan Budget Suplier (CHECK & BG)"};
                langNav = navID;
            }
...
<html ><!-- #BeginTemplate "/Templates/index.dwt" -->
    <head>
        ...
        <script type="text/javascript">
            ...
            function cmdPrintJournalXLS(){	                       
                window.open("<%=printroot%>.report.PrintTandaTerimaBGXLS?vendorId=<%=vendorId%>&dateFrom=<%=dateFrom%>&dateTo=<%=dateFrom%>&ignore=<%=ignore%>&pkp=<%=pkp%>&nonpkp=<%=nonpkp%>&non=<%=non%>&konsinyasi=<%=konsinyasi%>&komisi=<%=komisi%>&user_id=<%=appSessUser.getUserOID()%>&payment_type=<%=paymentType%>&lang=<%=lang%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                }
                
                function cmdSearch(){                                
                    document.form1.command.value="<%=JSPCommand.LIST%>";
                    document.form1.action="reportbudgetsuplierttcek.jsp";
    ...
    for (int ix = 0; ix < result.size(); ix++) {
        Vendor vendor = (Vendor) result.get(ix);
                                                                            ...
                                                                            <tr> 
                                                                                <td class="tablecell" style=padding:3px;>Type Pembayaran</td>
                                                                                <td>
                                                                                ...
                                                                                <td colspan="2">
                                                                                    <table width="1100" cellpadding="0" cellspacing="1">                                                                                        
                                                                                        <tr height="25">
                                                                                            ...
                                                                                            <td class="tablehdr" width="80">NUMBER</td>
    ...
    for (int i = 0; i < list.size(); i++) {
        ...
        try {
            ...
            TandaTerimaBG ttDetail = (TandaTerimaBG) numbs.get(0);
            TandaTerimaBGMain ttMain = DbTandaTerimaBGMain.fetchExc(ttDetail.getTandaTerimaBgMainId());
            numberx = ttMain.getNumber();
        } catch (Exception e) {
        }
                                                                                        %>
                                                                                        <tr height="20">
    ...
    if (counter == srbs.getCounter()) {
        ...
        v = srbs.getSuplier();
                                                                            ...
                                                                            <%
    session.putValue("PRINT_REPORT_BUDGET_TT", print);
                                                                            %>
                                                                            <tr>
                                                                                <td colspan="2">
                                                                                    <table border="0" cellpadding="0" cellspacing="0">
