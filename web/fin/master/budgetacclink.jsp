...
<%-- 
    Document   : budgetacclink
    Created on : Mar 1, 2016, 11:00:12 AM
    Author     : Roy
--%>
...
<%
            boolean priv = QrUserSession.isHavePriviledge(appSessUser.getUserOID(), AppMenu.M1_MENU_BUDGET, AppMenu.M2_MENU_BUDGET_ACC_LINK);
            boolean privView = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_BUDGET, AppMenu.M2_MENU_BUDGET_ACC_LINK, AppMenu.PRIV_VIEW);
            boolean privUpdate = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_BUDGET, AppMenu.M2_MENU_BUDGET_ACC_LINK, AppMenu.PRIV_UPDATE);
            boolean privAdd = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_BUDGET, AppMenu.M2_MENU_BUDGET_ACC_LINK, AppMenu.PRIV_ADD);
            boolean privDelete = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_BUDGET, AppMenu.M2_MENU_BUDGET_ACC_LINK, AppMenu.PRIV_DELETE);
%>
<!-- Jsp Block -->
<%!
...
// Combobox Type
        ...
        for (int i = 0; i < I_Project.budgetAccLinks.length; i++) {
            type_key.add(I_Project.budgetAccLinks[i]);
            type_value.add(I_Project.budgetAccLinks[i]);
        }
...
// variable declaration
            ...
            if (grpType.equals("All")) {
                for (int i = 0; i < I_Project.budgetAccLinks.length; i++) {
                    whereClause = whereClause + " (" + DbAccLink.colNames[DbAccLink.COL_TYPE] + " = '" + I_Project.budgetAccLinks[i] + "') or";
                }
...
<html ><!-- #BeginTemplate "/Templates/index.dwt" -->
        ...
        <script language="JavaScript">
            ...
            function cmdResetSaja(){
                if(confirm("Warning !! \nthis action will delete previows setup data, and update to the new setup, \nare you seure ?")){
                    ...
                    document.frmacclink.action="budgetacclink.jsp";
    ...
    <body onLoad="MM_preloadImages('<%=approot%>/images/home2.gif','<%=approot%>/images/logout2.gif','../images/new2.gif')">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" height="100%">
            ...
            for (int i = 0; i < I_Project.budgetAccLinks.length; i++) {
                                                                                        %>
                                                                                        <option value="<%=I_Project.budgetAccLinks[i]%>" <%if (I_Project.budgetAccLinks[i].equals(grpType)) {%>selected<%}%>><%=I_Project.budgetAccLinks[i]%></option>
                                                                                        <%
            }
                                                                                        %>
                                                                                    </select>
...
%>
                                                                            <tr align="left" valign="top"> 
                                                                                ...
                                                                                <td height="22" valign="middle" colspan="3" width="99%">
                                                                                    <%= drawList(iCommand, jspAccLink, accLink, listAccLink, oidAccLink, isPostableOnly, langBT)%>                                                                                     
                                                                                </td>
                                                                            </tr>
                                                                            <%
            //								}
