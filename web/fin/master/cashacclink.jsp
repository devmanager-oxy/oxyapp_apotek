...
<%
            boolean priv = QrUserSession.isHavePriviledge(appSessUser.getUserOID(), AppMenu.M1_MN_CASH, AppMenu.M2_MN_CA);
            boolean privView = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MN_CASH, AppMenu.M2_MN_CA, AppMenu.PRIV_VIEW);
            boolean privUpdate = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MN_CASH, AppMenu.M2_MN_CA, AppMenu.PRIV_UPDATE);
            boolean privAdd = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MN_CASH, AppMenu.M2_MN_CA, AppMenu.PRIV_ADD);
            boolean privDelete = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MN_CASH, AppMenu.M2_MN_CA, AppMenu.PRIV_DELETE);
%>

<!-- Jsp Block -->
<%!
    public String drawList(int iCommand, JspAccLink frmObject, AccLink objEntity, Vector objectClass,
            long accLinkId, long depID, String typeAccLink, boolean isPostableOnly, boolean cashPrivUpdate, String[] lang){
...
// Combobox Type
        ...
        for (int i = 0; i < I_Project.cashAccLinks.length; i++) {
            type_key.add(I_Project.cashAccLinks[i]);
            type_value.add(I_Project.cashAccLinks[i]);
        }
        ...
        if (iCommand == JSPCommand.ADD || (iCommand == JSPCommand.SAVE && frmObject.errorSize() > 0 && accLinkId == 0)) {            
            rowx.add(JSPCombo.draw(frmObject.colNames[frmObject.JSP_TYPE], null, "" + typeAccLink, type_value, type_key, "formElemen", ""));            
...
<%
            ...
            int x_edit = JSPRequestValue.requestInt(request, "xx_edit");
            int iCommand = JSPRequestValue.requestCommand(request);
            long depID = JSPRequestValue.requestLong(request, "x_department_id");
            String typeAccLink = JSPRequestValue.requestString(request, "x_type");
...
// variable declaration
            ...
            if (grpType.equals("All")) {
                whereClause = " type = '" + I_Project.ACC_LINK_GROUP_CASH + "' OR type = '" + I_Project.ACC_LINK_GROUP_CASH_CREDIT + "'" +
                        " OR type = '" + I_Project.ACC_LINK_GROUP_PETTY_CASH + "' OR type = '" + I_Project.ACC_LINK_GROUP_PETTY_CASH_CREDIT + "'" +
                        " OR type = '" + I_Project.ACC_LINK_GROUP_PETTY_CASH_PAYMENT_SUSPENSE_ACCOUNT + "'" +
                        " OR type = '" + I_Project.ACC_LINK_GROUP_CASH_SALES + "'" +
                        " OR type = '" + I_Project.ACC_LINK_CASH_BACK_CREDIT + "'" +
                        " OR type = '" + I_Project.ACC_LINK_CASH_BACK_DEBET + "'" +
                        " OR type = '" + I_Project.ACC_LINK_GROUP_PETTY_CASH_PAYMENT_KASBON + "'";
            } else{
...
// handle condition if size of record to display = 0 and start > 0 	after delete
            ...
            /*** LANG ***/
            String[] langCT = {"Account Type", "Cash Account", "Location", //0-2
            ...
            String[] langNav = {"Cash Transaction", "Account Link", "Date"};

            if (lang == LANG_ID) {
                String[] langID = {"Tipe Perkiraan", "Perkiraan Tunai", "Lokasi", //0-2
                ...
                String[] navID = {"Transaksi Tunai", "Setup Perkiraan", "Tanggal"};
                langNav = navID;
            }
        ...
        <script language="JavaScript">
            ...
            function cmdResetSaja(){
                if(confirm("Warning !! \nthis action will delete previows setup data, and update to the new setup, \nare you seure ?")){
                    ...
                    document.frmacclink.action="cashacclink.jsp";
            ...
            function cmdEdit(oidAccLink){
                <%if(privUpdate){%>
                document.frmacclink.xx_edit.value=1;
                document.frmacclink.hidden_acclink.value=oidAccLink;
            ...
            function cmdDept(){
                ...
                document.frmacclink.xx_edit.value=0;
                document.frmacclink.submit();
            }
                                                ...
                                                <tr> 
                                                    <td><!-- #BeginEditable "content" --> 
                                                        <form name="frmacclink" method ="post" action="">
                                                            ...
                                                            <input type="hidden" name="xx_edit" value="">
                                                                    ...
                                                                    <td height="8"  colspan="4" valign="top"> 
                                                                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                                            <tr align="left" valign="top"> 
                                                                                <td height="22" valign="middle" width="1%" class="comment">&nbsp;</td>
                                                                                <td height="22" valign="middle" colspan="3" class="comment" width="99%">&nbsp;<%=langCT[0]%> 
                                                                                    <select name="groupType" onChange="javascript:cmdAccGroup()">
                                                                                        <option value="All" <%if (grpType.equals("All")) {%>selected<%}%>>All</option>
                                                                                        <%
            for (int i = 0; i < I_Project.cashAccLinks.length; i++) {
                                                                                        %>
                                                                                        <option value="<%=I_Project.cashAccLinks[i]%>" <%if (I_Project.cashAccLinks[i].equals(grpType)) {%>selected<%}%>><%=I_Project.cashAccLinks[i]%></option>
                                                                                        <%
            }
                                                                                        %>
                                                                                    </select>
...
%>
                                                                            <tr align="left" valign="top"> 
                                                                                ...
                                                                                <td height="22" valign="middle" colspan="3" width="99%">
                                                                                    <%= drawList(iCommand, jspAccLink, accLink, listAccLink, oidAccLink, depID, typeAccLink, isPostableOnly, privUpdate, langCT)%>
                                                                                </td>
                                                                            </tr>
                                                                            <%
            //								}
                                                                            ...
                                                                            %>
                                                                            <tr align="left" valign="top"> 
                                                                                <td height="8" align="left" width="1%" class="command">&nbsp;</td>
            ...
            jspLine.setLastOnMouseOver("MM_swapImage('Image26x','','" + approot + "/images/last2.gif',1)");
                                                                            ...
                                                                            <%
            if (privUpdate && (iCommand != JSPCommand.EDIT && iCommand != JSPCommand.ADD && iCommand != JSPCommand.ASK && iErrCode == 0)) {
                                                                            %>
                                                                            <tr align="left" valign="top"> 
                                                                                <td height="22" valign="middle" width="1%">&nbsp;</td>
                                                                                <td height="22" valign="middle" colspan="3" width="99%"> 
            ...
            if (privAdd == false) {
                jspLine.setAddCaption("");
                                                                            ...
                                                                            <tr align="left" valign="top"> 
                                                                                <td height="22" valign="middle" width="1%">&nbsp;</td>
                                                                                <td height="22" valign="middle" colspan="3" width="99%"><font color="#FF0000"><%=langCT[3]%></font> </td>
                                                                            </tr>
                                                                            <tr align="left" valign="top"> 
                                                                                <td height="22" valign="middle" width="1%">&nbsp;</td>
                                                                                <td height="22" valign="middle" colspan="3" width="99%"> 
            ...
            for (int i = 0; i < I_Project.cashAccLinks.length; i++) {
                                                                                            ...
                                                                                            <td width="18%"> 
                                                                                                <div align="center"><a href="javascript:cmdResetSaja()"><%=langCT[5]%></a></div>
                                                                                            </td>
                                                                                            <td width="3%">&nbsp;</td>
                                                                                        </tr>
                                                                                    </table>
