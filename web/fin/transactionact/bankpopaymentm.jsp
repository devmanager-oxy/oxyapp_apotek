...
<%!
    ...
    public BankpoPaymentDetail getBankpoPaymentDetail(ArapMemo am, Vector bankpoPaymentDetail) {
        BankpoPaymentDetail bpd = new BankpoPaymentDetail();
        try {
            if (bankpoPaymentDetail != null && bankpoPaymentDetail.size() > 0) {
                for (int i = 0; i < bankpoPaymentDetail.size(); i++) {
                    BankpoPaymentDetail x = (BankpoPaymentDetail) bankpoPaymentDetail.get(i);
                    if (x.getArapMemoId() == am.getOID()) {
                        return x;
                    }
                }
            }
...
%>
<%
            String[] langCT = {"Receipt to Account", "Customer", "Payment Method", "Cheque/Transfer Number", "Memo", "Journal Number", "Trans. Date", "Amount", "Period", "Searching", "Journal number"}; //0 - 10
            String[] langNav = {"Bank", "PO Payment", "Date", "Invoice", "Vendor", "Inv. Currency", "Location", "Balance",
                "Rate", "Payment", "Payment by Invoice", "Description", "DO Number", "Currency", "Deduction"};

            if (lang == LANG_ID) {
                String[] langID = {"Perkiraan", "Sarana", "Cara Pelunasan", "Cek/nomor Transfer", "Memo", "Nomor Jurnal", "Tgl. Transfer", "Jumlah", "Periode", "Pencarian", "Nomor Jurnal"}; //0 - 10
                langCT = langID;
                String[] navID = {"Bank", "Order Pembelian", "Tanggal", "Tagihan", "Suplier", "Mata uang Inv.", "Lokasi", "Balance",
                    "Rate", "Pembayaran", "Pembayaran Dg.", "Keterangan", "Nomor DO", "Mata Uang Inv", "Deduction"};
                langNav = navID;
            }
            ...
            if(iJSPCommand == JSPCommand.REFRESH) {                
                oidBankpoPayment = JSPRequestValue.requestLong(request, "bankpo_id");
            }
            ...
            if(iJSPCommand == JSPCommand.REFRESH && bankpoPayment.getOID() != 0) {                
                invoices = QrInvoice.getOpenInvoiceByVendor(0, invSrc,bankpoPayment);
            }else{
                invoices = QrInvoice.getOpenInvoiceByVendor(0, invSrc);
            }
            ...
            Vector vListDed = new Vector();
            if (invoices != null && invoices.size() > 0) {
                long tVendorId = 0;
                for (int ix = 0; ix < invoices.size(); ix++) {
                    Vector v = (Vector) invoices.get(ix);
                    ...
                    if (tVendorId != vxxx.getOID()) {
                        if (tVendorId != 0) {
                            String where = DbArapMemo.fieldNames[DbArapMemo.COL_VENDOR_ID] + " = " + tVendorId +  " and " + DbArapMemo.fieldNames[DbArapMemo.COL_TYPE] + " = " + DbArapMemo.TYPE_AP;
                            if(iJSPCommand != JSPCommand.REFRESH){
                                where = where +" and " + DbArapMemo.fieldNames[DbArapMemo.COL_PAYMENT_STATUS] + " != " + DbArapMemo.MEMO_STATUS_FULL_PAID;
                            }
                            
                            Vector apMemo = DbArapMemo.list(0, 0, where, DbArapMemo.fieldNames[DbArapMemo.COL_NUMBER]);

                            for (int x = 0; x < apMemo.size(); x++) {
                                ArapMemo am = (ArapMemo) apMemo.get(x);
                                vListDed.add(am);
                            }
                        }
                    }
                    tVendorId = vxxx.getOID();
                }
...
<%
            ...
            if(iJSPCommand == JSPCommand.REFRESH){
                ...
                vBPD = DbBankpoPaymentDetail.list(0, 0, DbBankpoPaymentDetail.colNames[DbBankpoPaymentDetail.COL_BANKPO_PAYMENT_ID]+" = "+oidBankpoPayment, null);
                ...
                for (int i = 0; i < invoices.size(); i++){
                    ...
                    if (chk == 1){
                        ...
                        long vendId = JSPRequestValue.requestLong(request, "vendor_" + pi.getOID());
                ...
                if (vListDed != null && vListDed.size() > 0) {

                    if (vListDed != null && vListDed.size() > 0) {
                        for (int i = 0; i < vListDed.size(); i++) {

                            ArapMemo objArapMemo = (ArapMemo) vListDed.get(i);
                            int chk = JSPRequestValue.requestInt(request, "dchk_" + objArapMemo.getOID());

                            if (chk == 1) {

                                double payment = JSPRequestValue.requestDouble(request, "dpayment_amount_" + objArapMemo.getOID());
                                double invPayment = JSPRequestValue.requestDouble(request, "dinv_curr_amount_" + objArapMemo.getOID());
                                String memo = JSPRequestValue.requestString(request, "dmemo_" + objArapMemo.getOID());
                                double excrate = JSPRequestValue.requestDouble(request, "dexc_rate_" + objArapMemo.getOID());
                                long locId = JSPRequestValue.requestLong(request, "dlokasi_" + objArapMemo.getOID());
                                long vendId = JSPRequestValue.requestLong(request, "dvendor_" + objArapMemo.getOID());
                                double deduction = JSPRequestValue.requestDouble(request, "deduction_" + objArapMemo.getOID());
                                ...
                                ii.setVendorId(vendId);
                                ii.setInvoiceId(0);
                                ii.setArapMemoId(objArapMemo.getOID());
                                ii.setCurrencyId(objArapMemo.getCurrencyId());
                                ii.setBookedRate(excrate);
                                ii.setPaymentByInvCurrencyAmount(invPayment);
                                ii.setDeduction(deduction);
                                ii.setPaymentAmount(payment);
                                ii.setSegment1Id(segment1_id);
                                vBPD.add(ii);
                            }
            ...
            if (bankpoPayment.getOID() != 0 && iJSPCommand == JSPCommand.SAVE) {
    ...
    <script language="JavaScript">     
        ...
        <%}%>
         ...
         function cmdSearchJurnal(){
                window.open("<%=approot%>/transactionact/nomorjurnalpo.jsp", null, "height=400,width=600, status=yes,toolbar=no,menubar=no,location=no, scrollbars=yes");
         }
         ...
         %>		
             if(oid=='<%=invoice.getOID()%>'){                 
                 if(document.frmbankpopayment.chk_<%=invoice.getOID()%>.checked){
                     ...
                     var t = parseFloat(total) - parseFloat(paymentIdr);
                     if( t <0 ){
                         alert("Total tidak boleh minus");
                         document.frmbankpopayment.chk_<%=invoice.getOID()%>.checked = true;
                     }else{    
        ...
        function cmdDeductionCheckIt(oid){
            
         <%if (vListDed != null && vListDed.size() > 0) {
                for (int i = 0; i < vListDed.size(); i++) {
                    ArapMemo arapMemo = (ArapMemo) vListDed.get(i);

         %>		
             if(oid=='<%=arapMemo.getOID()%>'){
                 
                 if(document.frmbankpopayment.dchk_<%=arapMemo.getOID()%>.checked){
                     
                     var bal = document.frmbankpopayment.dinv_balance_<%=arapMemo.getOID()%>.value;                     
                     var balidr = document.frmbankpopayment.dbalance_idr_<%=arapMemo.getOID()%>.value;   
                     var total = document.frmbankpopayment.<%=JspBankpoPayment.colNames[JspBankpoPayment.JSP_AMOUNT]%>.value;
                     total = cleanNumberFloat(total, sysDecSymbol, usrDigitGroup, usrDecSymbol);                     
                     balidr = cleanNumberFloat(balidr, sysDecSymbol, usrDigitGroup, usrDecSymbol);                     
                     var tot = parseFloat(total) - parseFloat(balidr);
                     
                     if(tot > 0){
                         
                         document.frmbankpopayment.dinv_curr_amount_<%=arapMemo.getOID()%>.value=bal;
                         document.frmbankpopayment.dpayment_amount_<%=arapMemo.getOID()%>.value=balidr;
                         
                         document.frmbankpopayment.dinv_curr_amount_<%=arapMemo.getOID()%>.value="0.00";
                         document.frmbankpopayment.dh_inv_curr_amount_<%=arapMemo.getOID()%>.value="0.00";
                         
                         document.frmbankpopayment.dpayment_amount_<%=arapMemo.getOID()%>.value="0.00";					
                         document.frmbankpopayment.dh_payment_amount_<%=arapMemo.getOID()%>.value="0.00";
                         
                         document.frmbankpopayment.<%=JspBankpoPayment.colNames[JspBankpoPayment.JSP_AMOUNT] %>.value = formatFloat(''+(parseFloat(total) - parseFloat(balidr)), '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace); 
                         document.frmbankpopayment.tot.value = formatFloat(''+(parseFloat(total) - parseFloat(balidr)), '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace); 
                         
                         document.frmbankpopayment.deduction_<%=arapMemo.getOID()%>.value = formatFloat(''+ (parseFloat(balidr)), '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);
                         document.frmbankpopayment.hdeduction_<%=arapMemo.getOID()%>.value = formatFloat(''+ (parseFloat(balidr)), '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);
                         
                     }else{
                        alert("Total tidak boleh minus");
                        document.frmbankpopayment.dchk_<%=arapMemo.getOID()%>.checked = false;
                 }
                 
             }else{	
             
             var paymentIdr = document.frmbankpopayment.deduction_<%=arapMemo.getOID()%>.value;					
             paymentIdr = cleanNumberFloat(paymentIdr, sysDecSymbol, usrDigitGroup, usrDecSymbol);
             
             document.frmbankpopayment.dinv_curr_amount_<%=arapMemo.getOID()%>.value="0.00";
             document.frmbankpopayment.dpayment_amount_<%=arapMemo.getOID()%>.value="0.00";	
             
             document.frmbankpopayment.dinv_curr_amount_<%=arapMemo.getOID()%>.disabled=true;
             document.frmbankpopayment.dpayment_amount_<%=arapMemo.getOID()%>.disabled=true;					
             
             document.frmbankpopayment.dh_inv_curr_amount_<%=arapMemo.getOID()%>.value="0.00";
             document.frmbankpopayment.dh_payment_amount_<%=arapMemo.getOID()%>.value="0.00";
             document.frmbankpopayment.deduction_<%=arapMemo.getOID()%>.value = "0.00";
             document.frmbankpopayment.hdeduction_<%=arapMemo.getOID()%>.value = "0.00";
        ...
        function getDeductionAmount(oid){
         ...
         %>	
             if(oid=='<%=arapMemo.getOID()%>'){
                 ...
                 var deduction = document.frmbankpopayment.deduction_<%=arapMemo.getOID()%>.value;
                 deduction = cleanNumberFloat(deduction, sysDecSymbol, usrDigitGroup, usrDecSymbol);	
                 
                 var currpayment = document.frmbankpopayment.dinv_curr_amount_<%=arapMemo.getOID()%>.value;
                 currpayment = cleanNumberFloat(currpayment, sysDecSymbol, usrDigitGroup, usrDecSymbol);	
                 
                 var hdeduction = document.frmbankpopayment.hdeduction_<%=arapMemo.getOID()%>.value;
                 hdeduction = cleanNumberFloat(hdeduction, sysDecSymbol, usrDigitGroup, usrDecSymbol);	
                 
                 var totAp = document.frmbankpopayment.dbalance_idr_<%=arapMemo.getOID()%>.value;
                 totAp = cleanNumberFloat(totAp, sysDecSymbol, usrDigitGroup, usrDecSymbol);
                 
                 if(totAp < 0){
                     if(parseFloat(totAp)>parseFloat(deduction) || parseFloat(deduction) > 0){
                         alert("Payment amount is over the account payable,\nSystem will edit the data");
                         deduction = totAp;
                     }
                 }else{
                 if(parseFloat(totAp)<parseFloat(deduction)){
                     alert("Payment amount is over the account payable,\nSystem will edit the data");
                     deduction = totAp;
                 }					
             }
             ...
             document.frmbankpopayment.<%=JspBankpoPayment.colNames[JspBankpoPayment.JSP_AMOUNT] %>.value = formatFloat(''+(parseFloat(total) + parseFloat(hdeduction) - parseFloat(deduction)), '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace); 
             document.frmbankpopayment.tot.value = formatFloat(''+(parseFloat(total) + parseFloat(hdeduction) - parseFloat(deduction)), '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace); 
             document.frmbankpopayment.deduction_<%=arapMemo.getOID()%>.value = formatFloat(''+(deduction), '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);              
             document.frmbankpopayment.hdeduction_<%=arapMemo.getOID()%>.value = formatFloat(''+(deduction), '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);              
         }
         <%}
            }%>
        }
        ...
        function cmdGetAmountDeduction(){
         <%if (vListDed != null && vListDed.size() > 0) {
                for (int i = 0; i < vListDed.size(); i++) {
                    ArapMemo arapMemo = (ArapMemo) vListDed.get(i);
         %>
             var balance = document.frmbankpopayment.dinv_balance_<%=arapMemo.getOID()%>.value;
             var erate = document.frmbankpopayment.dexc_rate_<%=arapMemo.getOID()%>.value;
             ...
             document.frmbankpopayment.dbalance_idr_<%=arapMemo.getOID()%>.value = formatFloat(balance * erate, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace); 
                        ...
                        <tr align="left" valign="top"> 
                            <td height="8" valign="top" colspan="3" class="container"> 
                                <table width="100%" border="0" cellspacing="1" cellpadding="0">
                                    ...
                                    <tr>
                                        <td colspan="5">
                                            <table border="0" cellspacing="3" cellpadding="3" bgcolor="#F3F3F3">                                                                                                                                                           
                                                <tr>
                                                    <td><font face="arial"><b><%=langCT[9]%></b></font></td>
                                                    <td>&nbsp;</td>
                                                    <td>&nbsp;</td>
                                                </tr>
                                                <tr>
                                                    <td><%=langCT[10]%></td>
                                                    <td><input size="25" readonly type="text" name="jurnal_number" value=""></td>
                                    ...
                                    <tr> 
                                        <td colspan="5" width="100%"><table width="800" border="0" cellspacing="0" cellpadding="0"><tr><td background="../images/line.gif"><img src="../images/line.gif"></td></tr></table></td>
                                    </tr>  
                                    <%}%>
                                    <tr> 
                                        <td colspan="5" width="100%" height="5"></td>
        ...
        for (int t = 0; t < periods.size(); t++) {
            ...
            Vector vpm = DbPaymentMethod.list(0, 0, "", "");
                                                                ...
                                                                <td width="100%" class="page"> 
                                                                    <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                                                        <tr> 
                                                                            ...
                                                                            <td class="tablehdr" width="10%"><font size="1"><%=langNav[14]%></font></td>
                ...
                for (int i = 0; i < invoices.size(); i++) {
                    ...
                    Vector apMemo = new Vector();
                    if (tmpVendorId != vxxx.getOID()) {
                        if (tmpVendorId != 0) {

                            String where = DbArapMemo.fieldNames[DbArapMemo.COL_VENDOR_ID] + " = " + tmpVendorId +  " and " + DbArapMemo.fieldNames[DbArapMemo.COL_TYPE] + " = " + DbArapMemo.TYPE_AP;
                            if(iJSPCommand != JSPCommand.REFRESH){
                                where = where+" and " + DbArapMemo.fieldNames[DbArapMemo.COL_PAYMENT_STATUS] + " != " + DbArapMemo.MEMO_STATUS_FULL_PAID;
                            }
                            apMemo = DbArapMemo.list(0, 0, where, DbArapMemo.fieldNames[DbArapMemo.COL_NUMBER]);
                        }
                    }
                    ...
                    BankpoPaymentDetail bpd = getBankpoPaymentDetail(invoice, vBPD);
                                                                        ...
                                                                        %>
                                                                        <%
                                                                            if (apMemo != null && apMemo.size() > 0) {

                                                                                for (int x = 0; x < apMemo.size(); x++) {

                                                                                    ArapMemo arapMemo = (ArapMemo) apMemo.get(x);

                                                                                    BankpoPaymentDetail arbd = getBankpoPaymentDetail(arapMemo, vBPD);

                                                                                    Vendor dVendor = new Vendor();
                                                                                    try {
                                                                                        dVendor = DbVendor.fetchExc(arapMemo.getVendorId());
                                                                                    } catch (Exception e) {
                                                                                    }
                                                                        %>
                                                                        <tr> 
                                                                            <td class="tablecell" width="2%"> 
                                                                                <div align="center"> 
                                                                                <input type="checkbox" name="dchk_<%=arapMemo.getOID()%>" value="1" onClick="javascript:cmdDeductionCheckIt('<%=arapMemo.getOID()%>')" <%if (arbd.getDeduction() != 0) {%>checked<%}%>>
                                                                                       </div>
                                                                            </td>
                                                                            <td class="tablecell" width="8%"> 
                                                                                <div align="left"><font size="1"><%=arapMemo.getNumber()%></font></div>
                                                                            </td>
                                                                            <td width="8%" class="tablecell" nowrap><font size="1"><%=dVendor.getName()%></font></td>
                                                                            <td width="7%" class="tablecell"> 
                                                                                <div align="center"> <font size="1"> 
                                                                                        <%
                                                                                Location lok = new Location();
                                                                                try {
                                                                                    lok = DbLocation.fetchExc(arapMemo.getLocationId());
                                                                                } catch (Exception e) {
                                                                                }

                                                                                Currency c = new Currency();
                                                                                try {
                                                                                    c = DbCurrency.fetchExc(arapMemo.getCurrencyId());
                                                                                } catch (Exception e) {
                                                                                }
                                                                                        %>
                                                                                <%=c.getCurrencyCode()%> </font></div>
                                                                            </td>
                                                                            <td width="9%" class="tablecell"><font size="1"><%=lok.getName()%></font>
                                                                                <input type="hidden" name="dlokasi_<%=arapMemo.getOID()%>" value ="<%=arapMemo.getLocationId()%>">
                                                                                <input type="hidden" name="dvendor_<%=arapMemo.getOID()%>" value ="<%=arapMemo.getVendorId()%>">
                                                                            </td>
                                                                            <td width="9%" class="tablecell"> 
                                                                                <div align="center">
                                                                                    <input type="text" name="dinv_balance_<%=arapMemo.getOID()%>" value="<%=JSPFormater.formatNumber(DbArapMemo.getInvoiceBalance(arapMemo.getOID()), "#,###.##")%>"   style="text-align:right" size="15" class="readonly" readOnly>
                                                                                </div>
                                                                            </td>
                                                                            <td width="4%" class="tablecell"> 
                                                                                ...
                                                                                double balanceIDR = exRateValue * arapMemo.getAmount();
                                                                                    %>
                                                                                    <input type="text" name="dexc_rate_<%=arapMemo.getOID()%>" value="<%=JSPFormater.formatNumber(exRateValue, "#,###.##")%>"   style="text-align:right" size="5" class="readonly" readOnly>
                                                                                </div>
                                                                            </td>
                                                                            <td width="9%" class="tablecell"> 
                                                                                <div align="center"> 
                                                                                    <input type="text" name="dbalance_idr_<%=arapMemo.getOID()%>" value=""   style="text-align:right" size="15" class="readonly" readOnly>
                                                                                </div>
                                                                            </td>
                                                                            <td width="9%" class="tablecell"> 
                                                                                <div align="center"> 
                                                                                    <input type="hidden" name="dh_payment_amount_<%=arapMemo.getOID()%>" value="<%=arbd.getPaymentAmount()%>">
                                                                                    <input type="text" name="dpayment_amount_<%=arapMemo.getOID()%>" value="<%=JSPFormater.formatNumber(arbd.getPaymentAmount(), "#,###.##")%>"   style="text-align:right" size="15" onClick="this.select()" class="readonly" readOnly>
                                                                                </div>
                                                                            </td>
                                                                            <td width="11%" class="tablecell" nowrap> 
                                                                                <div align="center"> 
                                                                                    <input type="hidden" name="dh_inv_curr_amount_<%=arapMemo.getOID()%>" value="<%=arbd.getPaymentByInvCurrencyAmount()%>">
                                                                                    <%=c.getCurrencyCode()%> 
                                                                                    <input type="text" name="dinv_curr_amount_<%=arapMemo.getOID()%>" value="<%=JSPFormater.formatNumber(arbd.getPaymentByInvCurrencyAmount(), "#,###.##")%>"   style="text-align:right" size="15" onClick="this.select()">
                                                                                </div>
                                                                            </td>
                                                                            <td width="9%" class="tablecell"> 
                                                                                <div align="center">                                                                                     
                                                                                    <input type="hidden" name="hdeduction_<%=arapMemo.getOID()%>" value="<%=JSPFormater.formatNumber(arbd.getDeduction(), "#,###.##")%>" >
                                                                                    <input type="text" name="deduction_<%=arapMemo.getOID()%>" value="<%=JSPFormater.formatNumber(arbd.getDeduction(), "#,###.##")%>"   style="text-align:right" size="15" onClick="this.select()" onBlur="javascript:getDeductionAmount('<%=arapMemo.getOID()%>')">
                                                                                </div>
                                                                            </td>
                                                                            <td width="33%" class="tablecell"> 
                                                                                <div align="left"> 
                                                                                    <input type="text" name="dmemo_<%=arapMemo.getOID()%>" size="25" value="<%=arbd.getMemo()%>">
                                                                                </div>
                                                                            </td>
                                                                        </tr>
                                                                        <%}%>
                                                                        ...
                                                                        <tr> 
                                                                            ...
                                                                            <td width="9%" class="tablecell"><font size="1"><%=loc.getName()%></font>
                                                                                ...
                                                                                <input type="hidden" name="vendor_<%=invoice.getOID()%>" value ="<%=invoice.getVendorId()%>">
                                                                            </td>
                                                                            <td width="9%" class="tablecell"> 
                                                                                <div align="center">
                                                                            ...
                                                                            <td width="9%" class="tablecell"> 
                                                                                <div align="center">                                                                                     
                                                                                    <input type="text" name="" value="<%=JSPFormater.formatNumber(0.0, "#,###.##")%>"   style="text-align:right" size="15" class="readonly" readOnly>
                                                                                </div>
                                                                            </td>
                                                                            <td width="33%" class="tablecell"> 
                                                                                <div align="left"> 
