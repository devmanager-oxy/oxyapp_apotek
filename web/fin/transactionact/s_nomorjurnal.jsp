...
<%-- 
    Document   : s_nomorjurnal
    Created on : May 26, 2011, 10:13:43 AM
    Author     : Roy Andika
--%>
...
<%
            ...
            Vector listCashReceive = new Vector();

            if (iJSPCommand == JSPCommand.SEARCH) {
                String where = "";
                if (v_txt_jurnal != null && v_txt_jurnal.length() > 0) {
                    where = DbCashReceive.colNames[DbCashReceive.COL_JOURNAL_NUMBER] + " like '%" + v_txt_jurnal + "%'";
                }
                listCashReceive = DbCashReceive.list(0, 0, where, DbCashReceive.colNames[DbCashReceive.COL_JOURNAL_PREFIX]+","+DbCashReceive.colNames[DbCashReceive.COL_JOURNAL_NUMBER]);
            }
    ...
    <head>
        ...
        <script language="JavaScript">
            function cmdSelect(jurnalname,cashOid){      
                self.opener.document.frmcashreceivedetail.jurnal_number.value = jurnalname;
                self.opener.document.frmcashreceivedetail.cash_id.value = cashOid;
                self.opener.document.frmcashreceivedetail.command.value="<%=JSPCommand.REFRESH%>";    
                self.opener.document.frmcashreceivedetail.submit();                
                self.close();
            }
            
            function cmdSearch(){   
                document.frm_nomorjurnal.command.value="<%=JSPCommand.SEARCH%>";
                document.frm_nomorjurnal.action="s_nomorjurnal.jsp";
                                        ...
                                        <tr height="100%">
                                            ...
                                            <td  valign="top" >
                                                <table border="0" width="100%" cellpadding="0" cellspacing="0">                                                                                                                                        
                                                    ...
                                                    <%
            if (listCashReceive != null && listCashReceive.size() > 0) {
                                                    %>    
                                                    <tr>                                                        
                                                        <td>&nbsp;</td>
                                                    </tr>
                                                    <tr>
                                                        <td >
                                                            <table width="100%" border=0 cellpadding="1" cellspacing="1">
                                                                ...
                                                                <%
                                                        for (int i = 0; i < listCashReceive.size(); i++) {

                                                            CashReceive cashReceive = (CashReceive) listCashReceive.get(i);
                                                            ...
                                                            if (i % 2 == 0) {
                                                                ...
                                                                <tr height="21">
                                                                    <td class="<%=style%>" align="center">
                                                                        <%
                                                                    out.println("<a style='color:blue' href=\"javascript:cmdSelect('" + cashReceive.getJournalNumber() + "','" + cashReceive.getOID() + "')\">" + cashReceive.getJournalNumber() + "</a>");
                                                                        %>
                                                                    </td>
                                                                    <td class="<%=style%>" align="center">&nbsp;<%=JSPFormater.formatDate(cashReceive.getTransDate(),"dd MMM yyyy")%></td>
                                                                    <td class="<%=style%>" align="left">&nbsp;<%=cashReceive.getMemo()%></td>
                                                                    <td class="<%=style%>" align="right"><%=JSPFormater.formatNumber(cashReceive.getAmount(), "#,###.##")%>&nbsp;</td>
