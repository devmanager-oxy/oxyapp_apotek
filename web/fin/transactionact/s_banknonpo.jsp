...
<%-- 
    Document   : s_banknonpo
    Created on : Feb 28, 2012, 10:23:04 AM
    Author     : Roy Andika
--%>
...
<%
            ...
            Vector listBank = new Vector();

            if (iJSPCommand == JSPCommand.SEARCH) {
                String where = "";
                if (v_txt_jurnal != null && v_txt_jurnal.length() > 0) {
                    where = DbBanknonpoPayment.colNames[DbBanknonpoPayment.COL_JOURNAL_NUMBER] + " like '%" + v_txt_jurnal + "%' ";
                }
                listBank = DbBanknonpoPayment.list(0, 0, where, DbBanknonpoPayment.colNames[DbBanknonpoPayment.COL_JOURNAL_PREFIX] + "," + DbBanknonpoPayment.colNames[DbBanknonpoPayment.COL_JOURNAL_NUMBER]);
            }
%>
    ...
    <head>
        ...
        <script language="JavaScript">
            function cmdSelect(jurnalname,bankOid){      
                self.opener.document.<%=formName%>.<%=txt_Id%>.value = bankOid;
            ...
            function cmdSearch(){           
                ...
                document.frm_nomorjurnal.action="s_banknonpo.jsp";
                document.frm_nomorjurnal.submit();
            }
            
            function cmdDelPict(oidbank){
                document.frmimage.hidden_banknonpopayment_id.value=oidbank;
                document.frmimage.command.value="<%=JSPCommand.POST%>";
                document.frmimage.action="s_banknonpo.jsp";
            ...
            if (listBank != null && listBank.size() > 0) {
                ...
                <tr>
                    ...
                    for (int i = 0; i < listBank.size(); i++) {
                        ...
                        BanknonpoPayment banknonpoPayment = (BanknonpoPayment) listBank.get(i);
                        String style = "";
                        if (i % 2 == 0) {
                            ...
                            <tr>
                                <td class="<%=style%>" align="center">
                                    <%
                                out.println("<a style='color:blue' href=\"javascript:cmdSelect('" + banknonpoPayment.getJournalNumber() + "','" + banknonpoPayment.getOID() + "')\">" + banknonpoPayment.getJournalNumber() + "</a>");
                                    %>
                                </td>
                                <td class="<%=style%>" align="center">&nbsp;<%=JSPFormater.formatDate(banknonpoPayment.getTransDate(), "dd MMM yyyy")%></td>
                                <td class="<%=style%>" align="left">&nbsp;<%=banknonpoPayment.getMemo()%></td>
                                <td class="<%=style%>" align="right"><%=JSPFormater.formatNumber(banknonpoPayment.getAmount(), "#,###.##")%>&nbsp;</td>
