...
<%
	...
	if(session.getValue("DONOR_IFC_CATEGORY")!=null){
		session.removeValue("DONOR_IFC_CATEGORY");
	}
...
function cmdChangeParam(){
	document.form1.action="expensecategory.jsp";
	document.form1.submit();
}

function cmdPrintJournal(){	 
	window.open("<%=printroot%>.report.RptDonorIFCCategoryPDF?oid=<%=appSessUser.getLoginId()%>&reportType=<%=reportType%>&amountType=<%=amountType%>&act=<%=periodId%>");
}

function cmdPrintJournalXLS(){	 
	window.open("<%=printroot%>.report.RptDonorIFCCategoryXLS?oid=<%=appSessUser.getLoginId()%>&reportType=<%=reportType%>&amountType=<%=amountType%>&act=<%=periodId%>");
}
                ...
                <td width="100%" valign="top"> 
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr> 
                                           ...
                                           <%
            String navigator = "<font class=\"lvl1\">Donor Report</font><font class=\"tit1\">&nbsp;&raquo;&nbsp;<span class=\"lvl2\">Expense Category</span></font>";
                                           %>
                          ...
                          <tr> 
                            <td class="container"> 
                              <table width="70%" border="0" cellspacing="0" cellpadding="0" align="center">
                                ...
                                <tr> 
                                  <td> 
                                    <div align="center"><b><font size="3">By Expenses 
                                      Categories</font></b></div>
                                  </td>
                                </tr>
                                <tr> 
                                  <td> 
                                ...
                                <tr> 
                                  <td> 
                                      <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
                                        <tr> 
                                        <td width="56%" class="tablehdr" height="23">Expense 
                                          Category</td>
                                        <td width="28%" class="tablehdr" height="23">Total 
                                          Expenditure</td>
                                        <td width="15%" class="tablehdr" height="23">% 
                                          to Total</td>
                                      </tr>
                                      <%
								  Vector v123 = DbCoaCategory.list(0,0, "", "");
								  
								  double totalCategory = DbTransactionActivityDetail.getTotalCategoryByPeriod(periodId);
								  ...
								  for(int i=0; i<v123.size(); i++){
								  		CoaCategory ec = (CoaCategory)v123.get(i);
										...
										double amount = DbTransactionActivityDetail.getTotalByCategory(ec.getOID(), periodId);
								  ...
								  %>
                                      <tr> 
                                          <td width="56%" class="<%=css%>" height="17"><%=ec.getName()%></td>
                                          <td width="28%" class="<%=css%>" height="17"> 
                                      ...
                                      <!--tr> 
                                    <td colspan="4" height="1" background="<%=approot%>/images/line1.gif"><img src="<%=approot%>/images/line1.gif"></td>
                                  </tr-->
                                      <%}}%>
											<%
													//add detail
							...
							<%
								session.putValue("DONOR_IFC_CATEGORY", listReport);							
							%>								 						
						<%}catch(Exception e){
