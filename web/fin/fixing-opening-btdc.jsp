...
if(iCommand==JSPCommand.SUBMIT){
	...
	out.println("start --- processing opening");
	//System.out.println("start --- processing fixing data .. titipan, bymhd");
	
	Vector v = DbCoa.list(0,0, "opening_balance!=0", "");
	if(v!=null && v.size()>0){
		...
		for(int i=0; i<v.size(); i++){
			...
			try{
				DbCoaOpeningBalance.updateOpeningBalanceRecursif(coa.getOID(), coa.getOpeningBalance(), period.getOID());
				out.println("<br>----> processing idx :"+i+",      coaid : "+coa.getOID()+",   "+coa.getCode()+", amount : "+coa.getOpeningBalance()+" ====== success");			
			}
			catch(Exception e){
				out.println("<br>----> processing idx :"+i+", "+coa.getCode()+" ====== failed");			
			}
			
		}
		
		//DbCoaOpeningBalance.updateOpeningBalanceRecursif(5004112l, -841471267.70, period.getOID());
	}
	
	out.println("<br>end --- processing coa opening");
...
function cmdFix(){
	...
	document.form1.action="fixing-opening-btdc.jsp";
