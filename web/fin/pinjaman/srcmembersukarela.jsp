...
<%!
...

	{
		...
		ctrlist.addHeader("NIK","10%");
		ctrlist.addHeader("Nama","20%");
		ctrlist.addHeader("Alamat","20%");
		ctrlist.addHeader("J.Kelamin","10%");
		ctrlist.addHeader("Dinas/Unit","20%");
		ctrlist.addHeader("Status Kepeg.","10%");
		ctrlist.addHeader("Status Anggota","10%");
		...
		for (int i = 0; i < objectClass.size(); i++) {
			...
			Dinas din = new Dinas();
			try{
				din = DbDinas.fetchExc(member.getDinasId());
			}
			catch(Exception e){
			}
			DinasUnit du = new DinasUnit();
			try{
				du = DbDinasUnit.fetchExc(member.getDinasUnitId());
			}
			catch(Exception e){
			}
			
			rowx.add(din.getNama()+"/"+du.getNama());
			dtC.setAgama(din.getNama()+"/"+du.getNama());
			...
			*/
			...
			rowx.add(DbMember.statusPegawai[member.getStatusPegawai()]);
			dtC.setKecamatan(DbMember.statusPegawai[member.getStatusPegawai()]);
			...
			*/
			...
			rowx.add(DbMember.status[member.getStatus()]);
			dtC.setDesa(DbMember.status[member.getStatus()]);
...
//out.println("listMember.size() : "+listMember.size());
...
<title>Sipadu - Cari Member</title>
...
function cmdSearch(){
	...
	document.frmmember.action="srcmembersukarela.jsp";
	document.frmmember.submit();
}
...
function cmdEdit(oidMember){
	...
	document.frmmember.prev_command.value="<%=prevJSPCommand%>";
	document.frmmember.action="tabungansukarela.jsp";
	document.frmmember.submit();
	}
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
                      <td class="title"><!-- #BeginEditable "title" --><span class="level1">Keanggotaan</span> 
                        &raquo; <span class="level1">Tabungan Sukarela</span> 
                        &raquo; <span class="level2">Pencarian Anggota<br>
                        </span><!-- #EndEditable --></td>
                    </tr>
                    <!--tr> 
