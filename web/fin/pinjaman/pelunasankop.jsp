...
<%
	...
	if(iJSPCommand==JSPCommand.ADD){
		...
		if(bayarType==0){
			...
			if(pd.getCicilanKe()==1){
				saldo = pinjaman.getTotalPinjaman();
			}
			else{
				Vector v = DbPinjamanDetail.list(0,0, "pinjaman_id = "+pd.getPinjamanId()+" and cicilan_ke="+(pd.getCicilanKe()-1), "");
				if(v!=null && v.size()>0){
					PinjamanDetail pdx = (PinjamanDetail)v.get(0);
					saldo = pdx.getSaldoKoperasi();
				}
			}
			bayarPinjaman.setAmount(saldo);
		}
	}
	...
	if(iJSPCommand==JSPCommand.SAVE && bayarPinjaman.getOID()!=0 && iErrCode==0){
		...
		try{
			pinjaman.setStatus(DbPinjaman.STATUS_LUNAS);
			DbPinjaman.updateExc(pinjaman);
		}
		catch(Exception e){
		}
...
<script language="JavaScript">
	...
	function cmdTotal(){
		...
		var pinalti = document.frm_bayarpinjaman.<%=JspBayarPinjaman.colNames[JspBayarPinjaman.JSP_PINALTI]%>.value;
		pinalti = cleanNumberFloat(pinalti, sysDecSymbol, usrDigitGroup, usrDecSymbol);
		...
		total = parseFloat(pokok) + parseFloat(bunga) + parseFloat(denda) + parseFloat(pinalti);
	...
	function cmdAdd(){
		...
		document.frm_bayarpinjaman.action="pelunasankop.jsp";
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
                      <td class="title"><!-- #BeginEditable "title" --><span class="level1">Keanggotaan</span> 
                        ...
                        <span class="level2">Pelunasan Pinjaman 
                        <%if(pinjaman.getType()==DbPinjaman.TYPE_PINJAMAN_KOPERASI){%>
                        Koperasi 
                        <%}else{%>
                        Bank 
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frm_bayarpinjaman" method="post" action="">
                          ...
                          <input type="hidden" name="<%=JspBayarPinjaman.colNames[JspBayarPinjaman.JSP_TYPE]%>" value="<%=DbBayarPinjaman.TYPE_PELUNASAN%>">
                            ...
                            <tr> 
                              <td class="container"> 
                                <table width="100%" cellspacing="1" cellpadding="1" >
                                  ...
                                  <tr align="left"> 
                                    <td width="11%"  >Pelunasan</td>
                                    <td  width="20%"> 
                                      <input type="text" name="<%=JspBayarPinjaman.colNames[JspBayarPinjaman.JSP_AMOUNT]%>" value="<%=JSPFormater.formatNumber(bayarPinjaman.getAmount(),"#,###.##")%>" class="formElemen" style="text-align:right" readOnly>
                                  ...
                                  <tr align="left"> 
                                    ...
                                    <td  width="20%"> 
                                      <input type="text" name="<%=JspBayarPinjaman.colNames[JspBayarPinjaman.JSP_BUNGA]%>" value="<%=JSPFormater.formatNumber(bayarPinjaman.getAmount()*pinjaman.getBunga()/100,"#,###.##")%>" class="formElemen" style="text-align:right" onBlur="javascript:checkNumber(this)" onClick="this.select()">
                                    </td>
                                    <td  width="13%">&nbsp;</td>
                                    <td  width="56%">&nbsp;</td>
                                  </tr>
                                    ...
                                    <td colspan="2" rowspan="6"> 
                                      <table width="85%" border="0" cellspacing="1" cellpadding="1">
                                        ...
                                        <tr bgcolor="#E1E1E1"> 
                                          ...
                                          <td width="34%" nowrap> 
                                            <%	
											Vector temp = DbAccLink.list(0,0, DbAccLink.colNames[DbAccLink.COL_TYPE]+"='"+I_Project.ACC_LINK_BIAYA_PINJAMAN_CREDIT+"'", "");											
											if(temp!=null && temp.size()>0){
												if(temp.size()>0){
												%>
                                        ...
                                        <tr bgcolor="#E1E1E1"> 
                                          <td width="30%" nowrap>Biaya Bunga </td>
                                          <td width="34%"> 
                                            <%	
                                        ...
                                        <tr bgcolor="#E1E1E1"> 
                                          <td width="30%" nowrap>Biaya Denda </td>
                                          <td width="34%"> 
                                            <%	
                                        ...
                                        <tr bgcolor="#E1E1E1"> 
                                          <td width="30%">Biaya Pinalti </td>
                                          <td width="34%"> 
                                            <%	
											temp = DbAccLink.list(0,0, DbAccLink.colNames[DbAccLink.COL_TYPE]+"='"+I_Project.ACC_LINK_BIAYA_PINJAMAN_PINALTI_DEBET+"'", "");											
											if(temp!=null && temp.size()>0){
												if(temp.size()>0){
												%>
                                            <select name="<%=JspBayarPinjaman.colNames[JspBayarPinjaman.JSP_COA_PINALTI_DEBET_ID] %>">
                                              <%for(int i=0; i<temp.size(); i++){
													  		AccLink al = (AccLink)temp.get(i);
															Coa cx = new Coa();
															try{
													  ...
													  %>
                                              <option value="<%=cx.getOID()%>" <%if(bayarPinjaman.getCoaPinaltiDebetId()==cx.getOID()){%>selected<%}%>><%=cx.getCode()+" - "+cx.getName()%></option>
                                              <%}%>
                                            </select>
                                            <%}else{
													AccLink al = new AccLink();													
												...
												%>
                                            <input type="hidden" name="<%=JspBayarPinjaman.colNames[JspBayarPinjaman.JSP_COA_PINALTI_DEBET_ID] %>" value="<%=al.getCoaId()%>">
                                            <%=cx.getCode()+" - "+cx.getName()%> 
                                            <%}}%>
                                          </td>
                                          <td width="36%">&nbsp;</td>
                                        ...
                                        <tr bgcolor="#E1E1E1"> 
                                          <td width="30%">Pengurangan Hutang (AP)</td>
                                          <td width="34%">&nbsp; </td>
                                          <td width="36%"> 
                                            <%	
                                        ...
                                        <tr bgcolor="#E1E1E1"> 
                                          <td width="30%">Bunga Dibayar Dengan</td>
                                          <td width="34%">&nbsp; </td>
                                          <td width="36%"> 
                                            <%	
                                        ...
                                        <tr bgcolor="#E1E1E1"> 
                                          <td width="30%"> Denda Dibayar Dengan</td>
                                          <td width="34%">&nbsp; </td>
                                          <td width="36%"> 
                                            <%	
										 ...
										 <tr bgcolor="#E1E1E1"> 
                                          <td width="30%"> Pinalti Dibayar Dengan</td>
                                          <td width="34%">&nbsp; </td>
                                          <td width="36%"> 
                                            <%	
											temp = DbAccLink.list(0,0, DbAccLink.colNames[DbAccLink.COL_TYPE]+"='"+I_Project.ACC_LINK_BIAYA_PINJAMAN_PINALTI_CREDIT+"'", "");											
											if(temp!=null && temp.size()>0){
												if(temp.size()>0){
												%>
                                            <select name="<%=JspBayarPinjaman.colNames[JspBayarPinjaman.JSP_COA_PINALTI_CREDIT_ID] %>">
                                              <%for(int i=0; i<temp.size(); i++){
													  		AccLink al = (AccLink)temp.get(i);
															Coa cx = new Coa();
															try{
													  ...
													  %>
                                              <option value="<%=cx.getOID()%>" <%if(bayarPinjaman.getCoaPinaltiCreditId()==cx.getOID()){%>selected<%}%>><%=cx.getCode()+" - "+cx.getName()%></option>
                                              <%}%>
                                            </select>
                                            <%}else{
													AccLink al = new AccLink();													
												...
												%>
                                            <input type="hidden" name="<%=JspBayarPinjaman.colNames[JspBayarPinjaman.JSP_COA_PINALTI_CREDIT_ID] %>" value="<%=al.getCoaId()%>">
                                            <%=cx.getCode()+" - "+cx.getName()%> 
                                            <%}}%>
                                          </td>
                                        </tr>
