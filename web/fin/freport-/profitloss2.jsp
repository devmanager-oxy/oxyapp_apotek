...
%>
<%
	if(session.getValue("PROFIT2")!=null){
		session.removeValue("PROFIT2");
	}
	...
	if(strShowLevel.equals("")){valShowLevel=2;};
	...
	if(vDep!=null && vDep.size()>0){
		...
		colSpan = loopingDep+loopingSec+loopingSubSec;
	}
...
function cmdChangeList(){
	document.frmcoa.action="profitloss2.jsp";
	document.frmcoa.submit();
}

function cmdPrintJournal(){	 
	window.open("<%=printroot%>.report.RptProfitLoss2XLS?oid=<%=appSessUser.getLoginId()%>");
}
              ...
              <tr> 
                ...
                <td width="100%" valign="top"> 
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
                      <td class="title"><!-- #BeginEditable "title" --><span class="level1">Financial Report </span> &raquo; <span class="level2">Profit & Loss<br>
                        </span><!-- #EndEditable --></td>
                    </tr>
                    <!--tr> 
                                  ...
                                  <tr align="left" valign="top"> 
                                    <td height="22" valign="middle" colspan="3" class="page"> 
                                      <table width="100%" border="0" cellpadding="0" height="20" cellspacing="1">                                        
                                        <tr> 
                                          <td width="400" class="tablehdr" height="22" rowspan="3" nowrap><div align="center"><b><font color="#FFFFFF">Description</font></b></div></td>
										  <%
											  	if(vDep!=null && vDep.size()>0){
													for(int i=0; i<vDep.size(); i++){
														Department dep = (Department)vDep.get(i);												
										  ...
										  %>
										  			<td width="150" class="tablehdr" height="22" colspan="<%=vSec.size()+looping%>" nowrap><%=dep.getName()%></td>
										  <%					
										  				} else
														{
										  %>
										  ...
										  <%	}	}	}	%>										  
										  <td width="150" class="tablehdr" height="22" rowspan="3" nowrap>Total Corporate</td>
                                        </tr>
										<tr>											
										  <%
											  	if(vDep!=null && vDep.size()>0){
										  ...
										  %>
													<td width="140" class="tablehdr" height="20" colspan="<%=vSubSec.size()%>" nowrap><div align="center"><b><font color="#FFFFFF"><%=depSec.getName()%></font></b></div></td>										  
										  <%				}					
										  				} else
														{
										  %>
										...
										<tr>											
										  ...
										  %>
													<td width="140" class="tablehdr" height="20" nowrap><div align="center"><b><font color="#FFFFFF"><%=depSubSec.getName()%></font></b></div></td>										  
										  <%						}					
																} else
																{
										  %>
															...
															for(int ia=0; ia<vSec.size(); ia++){
																...
																if(vSubSec!=null && vSubSec.size()>0){															
																	for(int ib=0; ib<vSubSec.size(); ib++){
																		...
																		amount = DbCoa.getCoaBalance(coa.getOID(),dep.getOID(),depSec.getOID(),depSubSec.getOID(),"CD");
															...
															for(int ia=0; ia<vSec.size(); ia++){
																...
																if(vSubSec!=null && vSubSec.size()>0){															
																	for(int ib=0; ib<vSubSec.size(); ib++){
																		...
																		double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_REVENUE,"CD",dep.getOID(),depSec.getOID(),depSubSec.getOID());																
															...
															for(int ia=0; ia<vSec.size(); ia++){
																...
																if(vSubSec!=null && vSubSec.size()>0){															
																	for(int ib=0; ib<vSubSec.size(); ib++){
																		...
																		amount = DbCoa.getCoaBalance(coa.getOID(),dep.getOID(),depSec.getOID(),depSubSec.getOID(),"DC");
																		displayStr1 = strDisplay(amount, coa.getStatus());
																		vSesDep.add(""+amount);
																		//total by subSection
										  %>
															...
															for(int ia=0; ia<vSec.size(); ia++){
																...
																if(vSubSec!=null && vSubSec.size()>0){															
																	for(int ib=0; ib<vSubSec.size(); ib++){
																		...
																		double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_COST_OF_SALES,"DC",dep.getOID(),depSec.getOID(),depSubSec.getOID());																
																		displayStr1 = strDisplay(amount, "");
																		vSesDep.add(""+amount);
																		//total by subSection
										  %>
															...
															for(int ia=0; ia<vSec.size(); ia++){
																...
																if(vSubSec!=null && vSubSec.size()>0){															
																	for(int ib=0; ib<vSubSec.size(); ib++){
																		...
																		double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_EXPENSE,"DC",dep.getOID(),depSec.getOID(),depSubSec.getOID());																
																		displayStr1 = strDisplay(amount, "");
																		vSesDep.add(""+amount);
																		//total by subSection
										  %>
															...
															for(int ia=0; ia<vSec.size(); ia++){
																...
																if(vSubSec!=null && vSubSec.size()>0){															
																	for(int ib=0; ib<vSubSec.size(); ib++){
																		...
																		double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_OTHER_REVENUE,"CD",dep.getOID(),depSec.getOID(),depSubSec.getOID());																
																		displayStr1 = strDisplay(amount, "");
																		vSesDep.add(""+amount);
																		//total by subSection
										  %>
...
<!--level ACC_GROUP_OTHER_EXPENSE-->
                                        ...
                                        <tr> 
															...
															for(int ia=0; ia<vSec.size(); ia++){
																...
																if(vSubSec!=null && vSubSec.size()>0){															
																	for(int ib=0; ib<vSubSec.size(); ib++){
																		...
																		double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_OTHER_EXPENSE,"DC",dep.getOID(),depSec.getOID(),depSubSec.getOID());																
																		displayStr1 = strDisplay(amount, "");
																		vSesDep.add(""+amount);
																		//total by subSection
										  %>
										  <td class="tablecell2">
										    <table width="100%" border="0" cellpadding="0" cellspacing="1">
											  <tr>
											  	<td width="5%"></td>													
												<td width="90%" class="tablecell2"><div align="right"><%=displayStr1%></div></td>
												<td width="5%"></td>
										  	  </tr>
										  	</table>
										  </td>
					...
					%>
															...
															for(int ia=0; ia<vSec.size(); ia++){
																...
																if(vSubSec!=null && vSubSec.size()>0){															
																	for(int ib=0; ib<vSubSec.size(); ib++){
																		Department depSubSec = (Department)vSubSec.get(ib);												
																		double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_REVENUE,"CD",dep.getOID(),depSec.getOID(),depSubSec.getOID())+
																				DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_COST_OF_SALES,"DC",dep.getOID(),depSec.getOID(),depSubSec.getOID())-
																				DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_EXPENSE,"DC",dep.getOID(),depSec.getOID(),depSubSec.getOID())+
																				DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_OTHER_REVENUE,"CD",dep.getOID(),depSec.getOID(),depSubSec.getOID())-
																				DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_OTHER_EXPENSE,"DC",dep.getOID(),depSec.getOID(),depSubSec.getOID());																
																		//double amount = DbCoa.getCoaBalance(listCoa,I_Project.ACC_GROUP_EXPENSE,"DC",dep.getOID(),depSec.getOID(),depSubSec.getOID());																
																		displayStr1 = strDisplay(amount, "");
																		vSesDep.add(""+amount);
																		//total by subSection
							...
							<%
								session.putValue("PROFIT2", listReport);							
