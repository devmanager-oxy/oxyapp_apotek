...
<%!
	...
	public String drawList(int iJSPCommand,JspSubLocation frmObject, SubLocation objEntity, Vector objectClass,  long stockMinId, long subLocationId)

	{
		...
		jsplist.addHeader("Sub Location","35%");
		jsplist.addHeader("Location","20%");
		...
		Vector loc_key = new Vector(1,1);
                    ...
                    if(vloc!=null && vloc.size()>0)
                    {
                            for(int i=0; i<vloc.size(); i++)
                            {
                                    ...
                                    if(subLocationId==0 && i==0){
                                            subLocationId = loc.getOID();
                                    }
		...
		for (int i = 0; i < objectClass.size(); i++) {
			 SubLocation subLocation = (SubLocation)objectClass.get(i);
			 rowx = new Vector();
			 if(stockMinId == subLocation.getOID())
				 index = i; 

			 if(index == i && (iJSPCommand == JSPCommand.EDIT || iJSPCommand == JSPCommand.ASK)){
							
				rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"35\" name=\""+frmObject.colNames[JspSubLocation.JSP_NAME] +"\" value=\""+subLocation.getName()+"\" class=\"formElemen\">"+"</div>");
				rowx.add("<div align=\"left\">" + JSPCombo.draw(frmObject.colNames[JspSubLocation.JSP_LOCATION_ID], null, "" + objEntity.getLocation_id(), loc_value, loc_key, "onchange=\"javascript:parserCurrency()\"", "formElemen") + frmObject.getErrorMsg(frmObject.JSP_LOCATION_ID) + "</div>");    
				
                                rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"35\" name=\""+frmObject.colNames[JspSubLocation.JSP_DESCRIPTION] +"\" value=\""+subLocation.getDescription()+"\" class=\"formElemen\">"+"</div>");
                                				
			}else{
                                Location loc = new Location();
                                try{
                                    loc = DbLocation.fetchExc(subLocation.getLocation_id());
                                }catch(Exception e){
                                    
                                }
				rowx.add("<a href=\"javascript:cmdEdit('"+String.valueOf(subLocation.getOID())+"')\">"+subLocation.getName()+"</a>");

				
				rowx.add(loc.getName()+"");
                                rowx.add(subLocation.getDescription()+"");
		...
		if(iJSPCommand == JSPCommand.ADD || (iJSPCommand == JSPCommand.SAVE && frmObject.errorSize() > 0)){ 
                    rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"35\" name=\""+frmObject.colNames[JspSubLocation.JSP_NAME] +"\" value=\"\" class=\"formElemen\">"+"</div>");
                    rowx.add("<div align=\"left\">" + JSPCombo.draw(frmObject.colNames[JspSubLocation.JSP_LOCATION_ID], null, "" + objEntity.getLocation_id(), loc_value, loc_key, "onchange=\"javascript:parserCurrency()\"", "formElemen") + frmObject.getErrorMsg(frmObject.JSP_LOCATION_ID) + "</div>");    
                    
                    rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"35\" name=\""+frmObject.colNames[JspSubLocation.JSP_DESCRIPTION] +"\" value=\"\" class=\"formElemen\">"+"</div>");
		}
		lstData.add(rowx);
...
long oidSubLocation = JSPRequestValue.requestLong(request, "hidden_sub_location_id");
...
CmdSubLocation ctrlSubLocation = new CmdSubLocation(request);
JSPLine ctrLine = new JSPLine();
Vector listStockMin = new Vector(1,1);

/*switch statement */

iErrCode = ctrlSubLocation.action(iJSPCommand , oidSubLocation);
/* end switch*/

JspSubLocation jspSubLocation = ctrlSubLocation.getForm();

/*count list All Shift*/
int vectSize = DbSubLocation.getCount(whereClause);

/*switch list Shift*/
if((iJSPCommand == JSPCommand.FIRST || iJSPCommand == JSPCommand.PREV )||
  (iJSPCommand == JSPCommand.NEXT || iJSPCommand == JSPCommand.LAST)){
		start = ctrlSubLocation.actionList(iJSPCommand, start, vectSize, recordToGet);
 } 
/* end switch list*/

SubLocation subLocation = ctrlSubLocation.getSubLocation();
msgString =  ctrlSubLocation.getMessage();

/* get record to display */
//if(oidItemMaster!=0){
//    whereClause= DbSubLocation.colNames[DbSubLocation.COL_ITEM_MASTER_ID]+  "=" + oidItemMaster;
//}

listStockMin = DbSubLocation.list(start,recordToGet, whereClause , orderClause);
...
function cmdAdd(){
	document.frmitemmaster.hidden_sub_location_id.value="0";
        document.frmitemmaster.command.value="<%=JSPCommand.ADD%>";
	document.frmitemmaster.prev_command.value="<%=prevJSPCommand%>";
	document.frmitemmaster.action="subLocation.jsp";
	document.frmitemmaster.submit();
}

function cmdAsk(oidSubLocation){
	document.frmitemmaster.hidden_sub_location_id.value=oidSubLocation;
...
function cmdConfirmDelete(oidSubLocation){
	document.frmitemmaster.hidden_sub_location_id.value=oidSubLocation;
...
function cmdEdit(oidSubLocation){
	document.frmitemmaster.hidden_sub_location_id.value=oidSubLocation;
...
function cmdCancel(oidSubLocation){
	document.frmitemmaster.hidden_sub_location_id.value=oidSubLocation;
	document.frmitemmaster.command.value="<%=JSPCommand.EDIT%>";
	document.frmitemmaster.prev_command.value="<%=prevJSPCommand%>";
	document.frmitemmaster.action="subLocation.jsp";
...
function cmdDelPict(oidSubLocation){
	document.frmimage.hidden_sub_location_id.value=oidSubLocation;
	document.frmimage.command.value="<%=JSPCommand.POST%>";
	document.frmimage.action="subLocation.jsp";
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmitemmaster" method ="post" action="">
                          ...
                          <input type="hidden" name="hidden_sub_location_id" value="<%=oidSubLocation%>">
                                            ...
                                            <table width="100%" border="0" cellspacing="1" cellpadding="1" height="17">
                                              <tr valign="bottom"> 
                                                <td width="60%" height="23"><b><font color="#990000" class="lvl1">Master 
                                                  ...
                                                  </font><font class="tit1"><span class="lvl2">Sub Location</span> 
                                                  </font> <span class="lvl2"> </span></b></td>
							...
							%>
                                        <tr align="left" valign="top"> 
                                          <td height="22" valign="middle" colspan="3"> 
                                            <%= drawList(iJSPCommand,jspSubLocation, subLocation,listStockMin,oidSubLocation, subLocationId)%> </td>
                                        </tr>
                                        <% 
						  }catch(Exception exc){ 
						  }%>
                            ...
                            <tr align="left" valign="top" > 
                              <td colspan="3" class="command"> 
                                <%
									...
									String scomDel = "javascript:cmdAsk('"+oidSubLocation+"')";
									String sconDelCom = "javascript:cmdConfirmDelete('"+oidSubLocation+"')";
									String scancel = "javascript:cmdEdit('"+oidSubLocation+"')";
