...
<%!
...

	{
		...
		for (int i = 0; i < objectClass.size(); i++) {
			ReceiveReport purchaseOrderReport = (ReceiveReport)objectClass.get(i);
			RptIRSubCategoryL detail = new RptIRSubCategoryL();
...
RptIRSubCategory rptKonstan = new RptIRSubCategory();
  
SrcReceiveReport srcReceiveReport = new SrcReceiveReport();
srcReceiveReport.setTypeReport(SrcReceiveReport.GROUP_BY_SUB_CATEGORY);
...
function cmdPrintXLS(){	 
	window.open("<%=printroot%>.report.RptIRSubCategoryXLS?idx=<%=System.currentTimeMillis()%>");
}

function cmdSearch(){
	document.frmadjusment.command.value="<%=JSPCommand.LIST%>";
	document.frmadjusment.action="recreport-subcategory.jsp";
