...
<%
            boolean priv = QrUserSession.isHavePriviledge(appSessUser.getUserOID(), AppMenu.M1_RETAIL_REPORT, AppMenu.M2_COSTING_REPORT);
            boolean privView = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_RETAIL_REPORT, AppMenu.M2_COSTING_REPORT, AppMenu.PRIV_VIEW);
            boolean privPrint = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_RETAIL_REPORT, AppMenu.M2_COSTING_REPORT, AppMenu.PRIV_PRINT);
%>
<!-- Jsp Block -->
<%
...
<html ><!-- #BeginTemplate "/Templates/index.dwt" -->
    ...
    <body onLoad="MM_preloadImages('<%=approot%>/images/home2.gif','<%=approot%>/images/logout2.gif','../images/search2.gif','../images/new2.gif','../images/print2.gif')">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" height="100%">
            ...
            Vector locationsx = DbLocation.list(0, 0, "", DbLocation.colNames[DbLocation.COL_NAME]);
                                                                                                        %>
                                                                                                        <td > 
            ...
            if (locationsx != null && locationsx.size() > 0) {
                for (int i = 0; i < locationsx.size(); i++) {
                    Location d = (Location) locationsx.get(i);
                                                                                                                %>
            ...
            if (iJSPCommand == JSPCommand.LIST) {
                ...
                try {
                    ...
                    String where2 = "";
                    if (srcLocationId != 0) {
                        where = where + " and c." + DbCosting.colNames[DbCosting.COL_LOCATION_ID] + " = " + srcLocationId;
                        where2 = where2 + " and c." + DbCosting.colNames[DbCosting.COL_LOCATION_ID] + " = " + srcLocationId;
                    }

                    if (srcCostId != 0) {
                        where = where + " and c." + DbCosting.colNames[DbCosting.COL_LOCATION_POST_ID] + " = " + srcCostId;
                        where2 = where2 + " and c." + DbCosting.colNames[DbCosting.COL_LOCATION_ID] + " = " + srcCostId;
                    }

                    if (srcStatus.compareTo("") != 0) {
                        where = where + " and c." + DbCosting.colNames[DbCosting.COL_STATUS] + " = '" + srcStatus + "' ";
                        where2 = where2 + " and c." + DbCosting.colNames[DbCosting.COL_STATUS] + " = '" + srcStatus + "' ";
                    }

                    if (srcIgnore == 0) {
                        where = where + " and to_days(c." + DbCosting.colNames[DbCosting.COL_DATE] + ") >= to_days('" + JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd") + "') and to_days(c." + DbCosting.colNames[DbCosting.COL_DATE] + ") <= to_days('" + JSPFormater.formatDate(srcEndDate, "yyyy-MM-dd") + "') ";
                        where2 = where2 + " and to_days(c." + DbCosting.colNames[DbCosting.COL_DATE] + ") >= to_days('" + JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd") + "') and to_days(c." + DbCosting.colNames[DbCosting.COL_DATE] + ") <= to_days('" + JSPFormater.formatDate(srcEndDate, "yyyy-MM-dd") + "') ";
                    }
                    ...
                    String sql = "select loc_name,code,barcode,item_name,date,number,status,location_id,costing_item_id,costing_id,item_master_id,qty,price,amount from (" +
                            "select l.name as loc_name,m.code,m.barcode,m.name as item_name,c.date,c.number,c.status,c.location_id,ci.* from pos_costing c inner join pos_costing_item ci on c.costing_id = ci.costing_id inner join pos_item_master m on ci.item_master_id = m.item_master_id inner join pos_location l on c.location_post_id = l.location_id where c." + DbCosting.colNames[DbCosting.COL_LOCATION_POST_ID] + " != 0 ";

                    if (where != null && where.length() > 0) {
                        sql = sql + where;
                    }

                    sql = sql + " group by ci.costing_item_id union ";

                    sql = sql + " select l.name as loc_name,m.code,m.barcode,m.name as item_name,c.date,c.number,c.status,c.location_id,ci.* from pos_costing c inner join pos_costing_item ci on c.costing_id = ci.costing_id inner join pos_item_master m on ci.item_master_id = m.item_master_id inner join pos_location l on c.location_id = l.location_id where c." + DbCosting.colNames[DbCosting.COL_LOCATION_POST_ID] + " = 0 ";

                    if (where2 != null && where2.length() > 0) {
                        sql = sql + where2;
                    }

                    sql = sql + " group by ci.costing_item_id )as x group by costing_item_id ";
