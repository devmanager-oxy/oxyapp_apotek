...
<%!
...

	{
		...
		for (int i = 0; i < objectClass.size(); i++) {
			...
			RptPoSubCategoryL detail = new RptPoSubCategoryL();
...
RptPoSubCategory rptKonstan = new RptPoSubCategory();

SrcPurchaseOrderReport srcPurchaseOrderReport = new SrcPurchaseOrderReport();
srcPurchaseOrderReport.setTypeReport(SrcPurchaseOrderReport.GROUP_BY_SUB_CATEGORY);
...
function cmdPrintXLS(){	 
	window.open("<%=printroot%>.report.RptPoSubCategoryXLS?idx=<%=System.currentTimeMillis()%>");
}

function cmdSearch(){
	document.frmadjusment.command.value="<%=JSPCommand.LIST%>";
	document.frmadjusment.action="poreport-subcategory.jsp";
