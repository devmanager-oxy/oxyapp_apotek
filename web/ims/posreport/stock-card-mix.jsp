...
<%!
    public Vector drawList(Vector objectClass, int start, long locationId) {
        ...
        for (int i = 0; i < objectClass.size(); i++){
            ...
            rowx.add("" +  DbStock.getItemTotalStock(locationId, itemMaster.getOID())) ;
...
%>
<%
            if (session.getValue("KONSTAN") != null) { session.removeValue("KONSTAN"); }
            if (session.getValue("DETAIL") != null) { session.removeValue("DETAIL"); }            
            if (session.getValue("RPT_PARAMETER") != null) { session.removeValue("RPT_PARAMETER"); }
            if (session.getValue("SRC_VENDOR") != null) { session.removeValue("SRC_VENDOR"); }
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdPrintXLS(){	 
                window.open("<%=printroot%>.report.ReportStockCardMixXLS?idx=<%=System.currentTimeMillis()%>");
                }
                
                function cmdEdit(oidItemMaster){                    
                    window.open("<%=approot%>/posreport/stock-card-detail_mix.jsp?locationId=<%=srcLocationId%>&src_start_date=<%=startDate%>&src_end_date=<%=endDate%>&itemMasterId="+oidItemMaster+"&type_item=<%=DbStockCode.TYPE_NON_CONSIGMENT%>", null, "height=800,width=1200, status=yes,toolbar=no,menubar=no,location=no, scrollbars=yes"); 
                    }    
                    function cmdSearch(){
                        document.frmadjusment.command.value="<%=JSPCommand.LIST%>";
                        document.frmadjusment.start.value=0; 
                        document.frmadjusment.action="stock-card-mix.jsp";
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" height="100%">
            ...
            if (locations != null && locations.size() > 0) {
                if(user.getSegment1Id() != 0){
                    sd = DbSegmentDetail.fetchExc(user.getSegment1Id());
                }
                if(sd.getLocationId()==0){
