...
<%!
	public Vector drawList(Vector objectClass, int start){
		...
		for (int i = 0; i < objectClass.size(); i++) {
			SrcStockCardL stockCardtL = (SrcStockCardL)objectClass.get(i);
			
			Vector rowx = new Vector();

                        rowx.add("<div align=\"center\">"+(i+1)+"</div>");
			
			rowx.add(""+stockCardtL.getCode());
				
			rowx.add(""+stockCardtL.getItemName());
			
			rowx.add(""+stockCardtL.getLocationName());
			rowx.add(""+stockCardtL.getOpening());
				
			rowx.add("<div align=\"right\">"+stockCardtL.getQtyIn()+"</div>");
                        
                        if(stockCardtL.getQtyOut()<0){
                            rowx.add("<div align=\"right\">"+stockCardtL.getQtyOut()* DbStock.STOCK_OUT+"</div>");	
                        }else{
                            rowx.add("<div align=\"right\">"+stockCardtL.getQtyOut()+"</div>");	
                        }
			rowx.add("<div align=\"right\">"+stockCardtL.getSaldo()+"</div>");
			...
			lstData.add(rowx);
			temp.add(stockCardtL);
			lstLinkData.add(String.valueOf(stockCardtL.getItemMasterId()));
		}
...
if(iJSPCommand!=JSPCommand.NONE){
	listReport = SessStockReport.getItemStockCard(srcCode, srcName, srcLocationId, DbStock.TYPE_CONSIGMENT, JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd"), JSPFormater.formatDate(srcEndDate, "yyyy-MM-dd"));
}
...
function cmdEdit(oidItemMaster){
         ...
         window.open("<%=approot%>/posreport/stock-card-detail-Consigment.jsp?locationId=<%=srcLocationId%>&itemMasterId="+oidItemMaster+"&type_item=<%=DbStockCode.TYPE_CONSIGMENT%>", null, "height=400,width=600, status=yes,toolbar=no,menubar=no,location=no, scrollbars=yes"); 
}    
function cmdSearch(){
	document.frmadjusment.command.value="<%=JSPCommand.LIST%>";
	document.frmadjusment.action="stock-card-consigment.jsp";
