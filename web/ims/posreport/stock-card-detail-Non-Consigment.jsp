            ...
            switch (st.getType()) {
                ...
                case 7:
                    try {
                        ...
                        rowx.add("" + getNumber(st.getOpnameId()));
                ...
                case 11:
                    try {
                        ...
                        try {
                            ga = DbGeneralAffair.fetchExc(st.getCostingId());
                        } catch (Exception ex) {

                        }
                        rowx.add("" + ga.getNumber());
                        if ((st.getQty() * st.getInOut()) < 0) {
                            rowx.add("GA out");
                        } else {
                            rowx.add("GA in");
...
<%
            ...
            double stockPrev = 0;
            try {                
                stockPrev = getTotalStockPrev(locationId, itemMasterId, srcStartDate);
            } catch (Exception e) {
            }
            ...
            vstockDetail = DbStock.list(0, 0, " to_days(date) between to_days('" + JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd") + "') and to_days('" + JSPFormater.formatDate(srcEndDate, "yyyy-MM-dd") + "') and item_master_id=" + itemMasterId + " and location_id=" + locationId + " and status = 'APPROVED' ", "date");
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdPrintXLS(){	 
                window.open("<%=printroot%>.report.ReportStockCardDetailXLS1?idx=<%=System.currentTimeMillis()%>");
                }
