...
<%!
	...
	public String drawList(int iJSPCommand,JspShift frmObject, Shift objEntity, Vector objectClass,  long shiftId)

	{
		...
		jsplist.addHeader("Start Time","33%");
		jsplist.addHeader("End Time","33%");
		...
		String startHour = "<select name=\""+frmObject.colNames[JspShift.JSP_START_HOURS]+"\"> ";
		String endHour = "<select name=\""+frmObject.colNames[JspShift.JSP_END_HOURS]+"\"> ";
		String startMinutes = "<select name=\""+frmObject.colNames[JspShift.JSP_START_MINUTES]+"\"> ";
		String endMinutes = "<select name=\""+frmObject.colNames[JspShift.JSP_END_MINUTES]+"\"> ";
		
		String ending = "</select>";
		
		for (int i = 0; i < objectClass.size(); i++) {
			 Shift shift = (Shift)objectClass.get(i);
			 rowx = new Vector();
			 if(shiftId == shift.getOID())
				 index = i; 

			 if(index == i && (iJSPCommand == JSPCommand.EDIT || iJSPCommand == JSPCommand.ASK)){
					
				rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"35\" name=\""+frmObject.colNames[JspShift.JSP_NAME] +"\" value=\""+shift.getName()+"\" class=\"formElemen\">"+"</div>");
				//rowx.add("<div align=\"center\">"+"<input type=\"text\" name=\""+frmObject.colNames[JspShift.JSP_START_TIME] +"\" value=\""+shift.getStartTime()+"\" class=\"formElemen\">"+"</div>");
				//rowx.add("<div align=\"center\">"+"<input type=\"text\" name=\""+frmObject.colNames[JspShift.JSP_END_TIME] +"\" value=\""+shift.getEndTime()+"\" class=\"formElemen\">"+"</div>");
				
				rowx.add("<div align=\"center\">"+startHour+getHourOptions(shift.getStartHours())+ending+" : "+startMinutes+getMinutesOptions(shift.getStartMinutes())+ending+"</div>");
				rowx.add("<div align=\"center\">"+endHour+getHourOptions(shift.getEndHours())+ending+" : "+endMinutes+getMinutesOptions(shift.getEndMinutes())+ending+"</div>");
				
			}else{
				rowx.add("<a href=\"javascript:cmdEdit('"+String.valueOf(shift.getOID())+"')\">"+shift.getName()+"</a>");
		...
		if(iJSPCommand == JSPCommand.ADD || (iJSPCommand == JSPCommand.SAVE && frmObject.errorSize() > 0)){ 
				rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"35\" name=\""+frmObject.colNames[JspShift.JSP_NAME] +"\" value=\""+objEntity.getName()+"\" class=\"formElemen\">"+"</div>");
	...
	public static String getHourOptions(int slc){
		String s = "";
		for(int i=0; i<24; i++){
			s = s + "<option value=\""+i+"\"";
			if(i==slc){
	...
	public static String getMinutesOptions(int slc){
		String s = "";
		for(int i=0; i<60; i++){
			s = s + "<option value=\""+i+"\"";
			if(i==slc){
...
long oidShift = JSPRequestValue.requestLong(request, "hidden_shift_id");
...
CmdShift ctrlShift = new CmdShift(request);
JSPLine ctrLine = new JSPLine();
Vector listShift = new Vector(1,1);

/*switch statement */
iErrCode = ctrlShift.action(iJSPCommand , oidShift);
/* end switch*/
JspShift jspShift = ctrlShift.getForm();

/*count list All Shift*/
int vectSize = DbShift.getCount(whereClause);

/*switch list Shift*/
if((iJSPCommand == JSPCommand.FIRST || iJSPCommand == JSPCommand.PREV )||
  (iJSPCommand == JSPCommand.NEXT || iJSPCommand == JSPCommand.LAST)){
		start = ctrlShift.actionList(iJSPCommand, start, vectSize, recordToGet);
 } 
/* end switch list*/

Shift shift = ctrlShift.getShift();
msgString =  ctrlShift.getMessage();

/* get record to display */
listShift = DbShift.list(start,recordToGet, whereClause , orderClause);
...
function cmdDelPict(oidShift){
	...
	document.frmimage.action="shift.jsp";
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmshift" method ="post" action="">
                          ...
                          <input type="hidden" name="hidden_shift_id" value="<%=oidShift%>">
                                            ...
                                            <table width="100%" border="0" cellspacing="1" cellpadding="1" height="17">
                                              <tr valign="bottom"> 
                                                <td width="60%" height="23"><b><font color="#990000" class="lvl1">Master 
                                                  ...
                                                  </font><font class="tit1"><span class="lvl2">Master Cashier</span> 
                                                  &raquo;</font> <span class="lvl2">Cashier 
                                                  Shift </span></b></td>
							...
							%>
                                        <tr align="left" valign="top"> 
                                          <td height="22" valign="middle" colspan="3"> 
                                            <%= drawList(iJSPCommand,jspShift, shift,listShift,oidShift)%> </td>
                                        </tr>
                                        <% 
						  }catch(Exception exc){ 
						  }%>
