...
<%
            ...
            if (iJSPCommand != JSPCommand.NONE) {
                if (whereClause.length() > 0) {
                    whereClause = whereClause + " and (to_days(" + DbPriceType.colNames[DbPriceType.COL_CHANGE_DATE] + ")>=to_days('" + JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd") + "')" +
                            " and to_days(" + DbPriceType.colNames[DbPriceType.COL_CHANGE_DATE] + ")<=to_days('" + JSPFormater.formatDate(srcEndDate, "yyyy-MM-dd") + "'))";
                } else {
                    whereClause = "(to_days(" + DbPriceType.colNames[DbPriceType.COL_CHANGE_DATE] + ")>=to_days('" + JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd") + "')" +
                            " and to_days(" + DbPriceType.colNames[DbPriceType.COL_CHANGE_DATE] + ")<=to_days('" + JSPFormater.formatDate(srcEndDate, "yyyy-MM-dd") + "'))";
                }
            }
            ...
            /*switch statement */
            iErrCode = cmdPriceType.action(iJSPCommand, oidReceive);
            /* end switch*/
            ...
            int vectSize = 0;
            if (iJSPCommand != JSPCommand.NONE) {
                vectSize = DbPriceType.getCount(whereClause);
            }
            ...
            if (iJSPCommand != JSPCommand.NONE) {
                listChangePrice = DbPriceType.list(start, recordToGet, whereClause, orderClause);
            }

            if (iJSPCommand != JSPCommand.NONE) {
                Vector vlist = DbPriceType.list(0, 0, whereClause, orderClause);
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdPrintXLS(){	 
                window.open("<%=printroot%>.report.RptChangePriceXLS?idx=<%=System.currentTimeMillis()%>");
                }
