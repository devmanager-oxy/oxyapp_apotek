...
<%!
	...
	public String drawList(int iJSPCommand,JspCashMaster frmObject, CashMaster objEntity, Vector objectClass,  long cashMasterId, long locationId)

	{
		...
		jsplist.addHeader("Location","33%");
		jsplist.addHeader("Cashier Number","33%");
		...
		for (int i = 0; i < objectClass.size(); i++) {
			 ...
			 if(index == i && (iJSPCommand == JSPCommand.EDIT || iJSPCommand == JSPCommand.ASK)){
				...
				rowx.add("<div align=\"left\">" + JSPCombo.draw(frmObject.colNames[JspCashMaster.JSP_LOCATION_ID], null, "" + objEntity.getLocationId(), loc_value, loc_key, "onchange=\"javascript:parserCurrency()\"", "formElemen") + frmObject.getErrorMsg(frmObject.JSP_LOCATION_ID) + "</div>");    
				rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"35\" name=\""+frmObject.colNames[JspCashMaster.JSP_CASHIER_NUMBER] +"\" value=\""+cashMaster.getCashierNumber()+"\" class=\"formElemen\">"+"</div>");
				...
				rowx.add("<a href=\"javascript:cmdEdit('"+String.valueOf(cashMaster.getOID())+"')\">"+loc.getName()+"</a>");
		...
		if(iJSPCommand == JSPCommand.ADD || (iJSPCommand == JSPCommand.SAVE && frmObject.errorSize() > 0)){ 
                    rowx.add("<div align=\"left\">" + JSPCombo.draw(frmObject.colNames[JspCashMaster.JSP_LOCATION_ID], null, "" + objEntity.getLocationId(), loc_value, loc_key, "onchange=\"javascript:parserCurrency()\"", "formElemen") + frmObject.getErrorMsg(frmObject.JSP_LOCATION_ID) + "</div>");    
                    rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"35\" name=\""+frmObject.colNames[JspCashMaster.JSP_CASHIER_NUMBER] +"\" value=\"0\" class=\"formElemen\">"+"</div>");
...
long oidCashMaster = JSPRequestValue.requestLong(request, "hidden_cash_master_id");
long locationId=0;
...
CmdCashMaster ctrlcashMaster = new CmdCashMaster(request);
JSPLine ctrLine = new JSPLine();
Vector listcashMaster = new Vector(1,1);

/*switch statement */
iErrCode = ctrlcashMaster.action(iJSPCommand , oidCashMaster);
/* end switch*/
JspCashMaster jspCashMaster = ctrlcashMaster.getForm();

/*count list All Shift*/
int vectSize = DbCashMaster.getCount(whereClause);

/*switch list Shift*/
if((iJSPCommand == JSPCommand.FIRST || iJSPCommand == JSPCommand.PREV )||
  (iJSPCommand == JSPCommand.NEXT || iJSPCommand == JSPCommand.LAST)){
		start = ctrlcashMaster.actionList(iJSPCommand, start, vectSize, recordToGet);
 } 
/* end switch list*/
...
listcashMaster = DbCashMaster.list(start,recordToGet, whereClause , orderClause);
...
function cmdDelPict(oidCashMaster){
	document.frmimage.hidden_cash_master_id.value=oidCashMaster;
	document.frmimage.command.value="<%=JSPCommand.POST%>";
	document.frmimage.action="cashMaster.jsp";
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmshift" method ="post" action="">
                          ...
                          <input type="hidden" name="hidden_cash_master_id" value="<%=oidCashMaster%>">
							...
							%>
                                        <tr align="left" valign="top"> 
                                          <td height="22" valign="middle" colspan="3"> 
                                            <%= drawList(iJSPCommand,jspCashMaster, cashMaster,listcashMaster,oidCashMaster, locationId)%> </td>
                                        </tr>
                                        <% 
						  }catch(Exception exc){ 
						  }%>
