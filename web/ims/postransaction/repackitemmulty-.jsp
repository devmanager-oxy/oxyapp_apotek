...
<%!
    ...
    public boolean statusApproved(long repackId) {
        CONResultSet dbrs = null;
        try {
            String sql = "select sum(qty_in) as qty_in,sum(qty_out) as qty_out from ( " +
                    " select 0 as id,0 as qty_in,sum(qty) as qty_out from pos_repack_item where repack_id = " + repackId + " and type = 0 union " +
                    " select 1 as id,sum(qty) as qty_in,0 as qty_out from pos_repack_item where repack_id = " + repackId + " and type = 1 ) as tbl ";

            dbrs = CONHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                double in = rs.getDouble("qty_in");
                double out = rs.getDouble("qty_out");
                if (in != 0 && out != 0) {
                    return true;
                } else {
                    return false;
    ...
    if (repack.getOID() != 0) {
        ...
        if (repack.getOID() != 0) {
            session.putValue("KONSTAN", rptRepack);
                                                                            ...
                                                                            <tr align="left" > 
                                                                                <td colspan="5" <%if (!jspRepack.getErrorMsg(jspRepack.JSP_APPROVAL_1).equals("")) {%> bgcolor="yellow"<%}%> height="24" >
                                                                                    <font color="#FF0000"><%=jspRepack.getErrorMsg(jspRepack.JSP_APPROVAL_1)%></font><input type="hidden" name="sembunyi">
                                                                                </td>
                                                                            </tr>
                                                                            <tr align="left" > 
                                                                                <td colspan="5" valign="top">&nbsp;</td>
