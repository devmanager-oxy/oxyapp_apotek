...
<%!
				...
				int iErrCode2, String status)
	{
        ...
        for (int i = 0; i < objectClass.size(); i++) {
			 ...
			 SessIncomingGoodL iGL = new SessIncomingGoodL();
...
<%
	...
	if(session.getValue("INCOMING_DETAIL")!=null){
		session.removeValue("INCOMING_DETAIL");
    }

	if(session.getValue("INCOMING_TITTLE")!=null){
		session.removeValue("INCOMING_TITTLE");
    }
...
SessIncomingGood iG = new SessIncomingGood();
...
function cmdPrintPdf(){
	//window.open();
	window.open("<%=printroot%>.report.RptIncomingGoodPdf?mis=<%=System.currentTimeMillis()%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
}
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                                          ...
                                          <td height="8" valign="middle" colspan="3"> 
											...
											%>
                                              ...
                                              <tr align="left" > 
												...
												%>  
												<%=strList%>
												<% session.putValue("INCOMING_DETAIL",objRpt); %>
												</td>
                                              </tr>
                                              <tr align="left" > 
                                                <td colspan="5" valign="top"> 
						...
						//if(receive.getOID()!=0 && !receive.getStatus().equals(I_Project.DOC_STATUS_CLOSE)){%>
							...
							<% session.putValue("INCOMING_TITTLE",iG); %>
                        </form>
