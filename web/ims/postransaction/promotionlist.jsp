...
<%!
...

	{
		...
		cmdist.addHeader("Description","45%");
		...
		for (int i = 0; i < objectClass.size(); i++) {
			Promotion promotion = (Promotion)objectClass.get(i);
			 Vector rowx = new Vector();

                        rowx.add("<div align=\"center\">"+(start+i+1)+"</div>");         
			rowx.add("<div align=\"center\">"+JSPFormater.formatDate(promotion.getStartDate(),"dd-MMM-yyyy")+"</div>");
                        rowx.add("<div align=\"center\">"+JSPFormater.formatDate(promotion.getEndDate(),"dd-MMM-yyyy")+"</div>");
				       
				
                        //rowx.add(promotion.getUserId());

			rowx.add("<div align=\"center\">"+promotion.getPromoDesc()+"</div>");
			
			//rowx.add(promotion.getUserId());

                        lstData.add(rowx);
			lstLinkData.add(String.valueOf(promotion.getOID()));
		}
...
long oidTransfer = JSPRequestValue.requestLong(request, "hidden_promotion_id");
...
if(srcIgnore==0){
	if(whereClause.length()>0){
		whereClause = whereClause + " and (to_days("+DbPromotion.colNames[DbPromotion.COL_START_DATE]+")>=to_days('"+JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd")+"')"+
				" and to_days("+DbPromotion.colNames[DbPromotion.COL_END_DATE]+")<=to_days('"+JSPFormater.formatDate(srcEndDate, "yyyy-MM-dd")+"'))";	
	}
	else{
		whereClause = "(to_days("+DbPromotion.colNames[DbPromotion.COL_START_DATE]+")>=to_days('"+JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd")+"')"+
				" and to_days("+DbPromotion.colNames[DbPromotion.COL_END_DATE]+")<=to_days('"+JSPFormater.formatDate(srcEndDate, "yyyy-MM-dd")+"'))";	
	}
}
...
Vector listPromotion = new Vector(1,1);

/*switch statement */
//iErrCode = cmdPromotion.action(iJSPCommand , oidPromotion, );
/* end switch*/
JspPromotion jspTransfer = cmdPromotion.getForm();

/*count list All Transfer*/
int vectSize = DbPromotion.getCount(whereClause);

Promotion vendor = cmdPromotion.getPromotion();
msgString =  cmdPromotion.getMessage();


if((iJSPCommand == JSPCommand.FIRST || iJSPCommand == JSPCommand.PREV )||
  (iJSPCommand == JSPCommand.NEXT || iJSPCommand == JSPCommand.LAST)){
	start = cmdPromotion.actionList(iJSPCommand, start, vectSize, recordToGet);
 } 
/* end switch list*/

/* get record to display */
orderClause = DbPromotion.colNames[DbPromotion.COL_START_DATE];
listPromotion = DbPromotion.list(start,recordToGet, whereClause , orderClause);

/*handle condition if size of record to display = 0 and start > 0 	after delete*/
if (listPromotion.size() < 1 && start > 0)
{
...
function cmdSearch(){
	...
	document.frmtransfer.action="promotionlist.jsp";
	document.frmtransfer.submit();
}


function cmdEdit(oid){
	document.frmtransfer.hidden_promotion_id.value=oid;
	document.frmtransfer.command.value="<%=JSPCommand.EDIT%>";
	document.frmtransfer.prev_command.value="<%=prevJSPCommand%>";
	document.frmtransfer.action="promotionitem.jsp";
	document.frmtransfer.submit();
}

function cmdAdd(){
	document.frmtransfer.hidden_promotion_id.value="0";
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmtransfer" method ="post" action="">
                          ...
                          <input type="hidden" name="hidden_promotion_id" value="<%=oidTransfer%>">
                                ...
                                <table width="100%" border="0" cellspacing="1" cellpadding="1" height="17">
                                  <tr valign="bottom"> 
                                    <td width="60%" height="23"><b><font color="#990000" class="lvl1">Promotion
                                      </font><font class="tit1">&raquo; <span class="lvl2">Promotion List </span></font></b></td>
                                      ...
                                      <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                        <tr > 
                                          ...
                                          <td class="tabin" nowrap> 
                                            <div align="center">&nbsp;&nbsp;<a href="javascript:cmdAdd()" class="tablink">Promotion Item</a>&nbsp;&nbsp;</div>
                                          </td>
                                          <td class="tabheader"><img src="<%=approot%>/images/spacer.gif" width="3" height="10"></td>
                                        ...
                                        <tr align="left" valign="top"> 
                                          <td height="22" valign="middle" colspan="3"> 
                                            <table width="100%" border="0" cellspacing="2" cellpadding="1">
                                              <tr> 
                                                <td width="10%"><b><i>Search Parameters 
                                                  :</i></b></td>
                                                <td width="90%">&nbsp;</td>
                                              </tr>
							...
							%>
                                        <tr align="left" valign="top"> 
                                          <td class="boxed1" height="22" valign="middle" colspan="3"> 
                                            <%= drawList(listPromotion,start)%> </td>
                                        </tr>
                                        <tr align="left" valign="top"> 
