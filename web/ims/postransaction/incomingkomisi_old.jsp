...
<%!
	...
	public Vector drawList(Vector objectClass, long vendorId){
		...
		for (int i = 0; i < objectClass.size(); i++){
			...
			*/
			...
			rowx.add("<div align=\"center\">" + "<input type=\"checkbox\" name=\"chk_"+vid+"\" value=\"1\">" +"</div>");
    ...
    public Vector getKomisiSales(long locationId, Date startDate, Date endDate){
            ...
            /*
            String sql = "select v.vendor_id, v.code as vcode, v.name as vname, v.komisi_margin, m.item_master_id, " +
                " m.code as code, m.name as name, sum(psd.qty) as qty, "+
                ...
                " and  psd.status_komisi=0 and v.is_komisi=1 "+
                " group by m.item_master_id, v.vendor_id order by v.name, m.name";
            */ 
            
            String sql = "select vid, vcode, vname, vkomisi, mitem_id, "+
                " mcode, mname, sum(qty) as xqty, "+
                " sum(amount) as xamount from ( "+
                " select v.vendor_id as vid, v.code as vcode, v.name as vname, v.komisi_margin as vkomisi, m.item_master_id as mitem_id, "+
                ...
                " and  to_days(ps.date) >= to_days('"+JSPFormater.formatDate(startDate, "yyyy-MM-dd")+"') "+
                " and to_days(ps.date) <= to_days('"+JSPFormater.formatDate(endDate, "yyyy-MM-dd")+"') "+
                " and  psd.status_komisi=0 and m.type_item=2 and ps.type in (0,1) "+
                ...
                " and  to_days(ps.date) >= to_days('"+JSPFormater.formatDate(startDate, "yyyy-MM-dd")+"') "+
                " and to_days(ps.date) <= to_days('"+JSPFormater.formatDate(endDate, "yyyy-MM-dd")+"') "+
                " and  psd.status_komisi=0 and m.type_item=2 and ps.type in (2,3) "+
                " group by m.item_master_id, v.vendor_id "+
...
if(iJSPCommand==JSPCommand.SEARCH || iJSPCommand==JSPCommand.SAVE ){
    listKomisiItem = getKomisiSales(srcLocationId, srcStartDate, srcEndDate);//DbReceiveItem.getReceiveKomisi(whereClause);
}
...
if (iJSPCommand == JSPCommand.SAVE){
       ...
       for(int i=0;i<listKomisiItem.size();i++){
             ...
             int chk = JSPRequestValue.requestInt(request, "chk_" + vid);   
             ...
             if(chk==1){                 
                   ...
                   rec.setNote("AUTO GENERATED - Incoming Komisi");
                       ...
                       if(oidx!=0){
                           ...
                           try{
                               ...
                               if(oid!=0){
                                    ...
                                    //update status sales
                                    String sql = "update pos_sales_detail set status_komisi=1 where sales_id in (select sales_id from pos_sales " +
                                            "where to_days(date) >= to_days('"+ JSPFormater.formatDate(srcStartDate , "yyyy-MM-dd")+"') " +
                                            "and to_days(date) <= to_days('"+ JSPFormater.formatDate(srcEndDate, "yyyy-MM-dd") + "') " +
                                            "and location_id="+ srcLocationId + " ) and product_master_id = " + ri.getItemMasterId();
                                    try{
                                        DbSalesDetail.execUpdate(sql);
                                    }catch(Exception ex){} 
                               }
                           }catch(Exception ex){
                                                ...
                                                <td height="8" valign="middle" colspan="3"> 
                                                  <table width="100%" border="0" cellspacing="1" cellpadding="0">
                                                    ...
                                                    <tr> 
                                                      ...
                                                      <td width="6%"  height="22">Sales Form</td>
                                                      <td width="15%">
                                                  ...
                                                  <select name="src_location_id">
                                                    ...
                                                    <tr> 
                                                <td width="9%"  height="22">Date Between</td>
                                                <td width="91%"> 
                                                  ...
                                                  <a href="javascript:void(0)" onClick="if(self.gfPop)gfPop.fPopCalendar(document.frmorder.src_end_date);return false;" ><img class="PopcalTrigger" align="absmiddle" src="<%=approot%>/calendar/calbtn.gif" height="19" border="0" alt=""></a> 
                                                    ...
                                                    <tr>
                                                      <td width="14%" height="22"><a href="javascript:cmdSearch()"  onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('search','','../images/search2.gif',1)"><img src="../images/search.gif" name="search"  border="0"></a></td>
                                                    </tr>    
                                                    <tr> 
