...
<%!
    public Vector drawList(int iJSPCommand, JspReceiveItem frmObject,
        ...
        jsplist.addHeader("Transport", "10%");
        jsplist.addHeader("Bea", "10%");
        jsplist.addHeader("Komisi", "10%");
        jsplist.addHeader("Lain-lain", "10%");
        jsplist.addHeader("Harga Perolehan", "10%");
       ...
       /* if (vectVendMaster != null && vectVendMaster.size() > 0) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
			...
			try{
                ...
                if (true) {
                    rowx.add("<div align=\"right\">" + "<input type=\"text\" size=\"15\" name=\"" + frmObject.colNames[JspReceiveItem.JSP_PRICE_IMPORT] + "\" value=\"" + objEntity.getPriceImport() + "\" class=\"formElemen\" style=\"text-align:right\" onClick=\"this.select()\" onBlur=\"javascript:calculaterate()\">" + "</div>");
                    rowx.add("<div align=\"right\">" + "<input type=\"text\" size=\"15\" name=\"" + frmObject.colNames[JspReceiveItem.JSP_TRANSPORT] + "\" value=\"" + objEntity.getTransport() + "\" class=\"formElemen\" style=\"text-align:right\" onClick=\"this.select()\" onBlur=\"javascript:calculateSubTotal()\">" + "</div>");
                    rowx.add("<div align=\"right\">" + "<input type=\"text\" size=\"15\" name=\"" + frmObject.colNames[JspReceiveItem.JSP_BEA] + "\" value=\"" + objEntity.getBea() + "\" class=\"formElemen\" style=\"text-align:right\" onClick=\"this.select()\" onBlur=\"javascript:calculateSubTotal()\">" + "</div>");
                    rowx.add("<div align=\"right\">" + "<input type=\"text\" size=\"15\" name=\"" + frmObject.colNames[JspReceiveItem.JSP_KOMISI] + "\" value=\"" + objEntity.getKomisi() + "\" class=\"formElemen\" style=\"text-align:right\" onClick=\"this.select()\" onBlur=\"javascript:calculatekomisi()\">" + "</div>");
                    rowx.add("<div align=\"right\">" + "<input type=\"text\" size=\"15\" name=\"" + frmObject.colNames[JspReceiveItem.JSP_LAIN_LAIN] + "\" value=\"" + objEntity.getLainLain() + "\" class=\"formElemen\" style=\"text-align:right\" onClick=\"this.select()\" onBlur=\"javascript:calculateSubTotal()\">" + "</div>");
                ...
                rowx.add("<div align=\"right\">" + JSPFormater.formatNumber(receiveItem.getPriceImport(), "#,###.##") + "</div>");
                rowx.add("<div align=\"right\">" + JSPFormater.formatNumber(receiveItem.getTransport(), "#,###.##") + "</div>");
                rowx.add("<div align=\"right\">" + JSPFormater.formatNumber(receiveItem.getBea(), "#,###.##") + "</div>");
                rowx.add("<div align=\"right\">" + JSPFormater.formatNumber(receiveItem.getKomisi(), "#,###.##") + "</div>");
                rowx.add("<div align=\"right\">" + JSPFormater.formatNumber(receiveItem.getLainLain(), "#,###.##") + "</div>");
...
<%
            ...
            double rate = JSPRequestValue.requestDouble(request, "rate");
...
<%
            ...
            //update/insert vendor item otomatis
            if(iJSPCommand== JSPCommand.SAVE){
                Vector vvi = new Vector();
                vvi = DbVendorItem.list(0, 0, "item_master_id="+receiveItem.getItemMasterId()+" and vendor_id="+receive.getVendorId(), "") ;
                VendorItem vi = new VendorItem();
                if(vvi.size()>0){
                    vi = (VendorItem)vvi.get(0);
                    vi.setLastPrice(receiveItem.getAmount());
                    DbVendorItem.updateExc(vi);
                }else{
                    vi.setItemMasterId(receiveItem.getItemMasterId());
                    vi.setLastPrice(receiveItem.getAmount());
                    vi.setVendorId(receive.getVendorId());
    ...
    <head>
        ...
        <script language="JavaScript">
         ...
         <%}
            ...
            document.frmreceive.action="receiveitemimport.jsp";
...
function calculaterate(){
     ...
     var ratedolar = document.frmreceive.rate.value;
     var priceImport = document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_PRICE_IMPORT]%>.value;
     
     
     if(ratedolar>0){
         priceImport = removeChar(priceImport);
         priceImport = cleanNumberFloat(priceImport, sysDecSymbol, usrDigitGroup, usrDecSymbol);
        
         priceImport = priceImport * ratedolar;
     }    
     
     document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_PRICE_IMPORT]%>.value = formatFloat(''+priceImport, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);	
     calculateSubTotal();
    
}
function calculatekomisi(){
     
     var ratedolar = document.frmreceive.rate.value;
     var komisi = document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_KOMISI]%>.value;
     
     
     if(ratedolar>0){
         komisi = removeChar(komisi);
         komisi = cleanNumberFloat(komisi, sysDecSymbol, usrDigitGroup, usrDecSymbol);
        
         komisi = komisi * ratedolar;
     }    
     
     document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_KOMISI]%>.value = formatFloat(''+komisi, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);	
     calculateSubTotal();
...
function calculateSubTotal(){   
    ...
    var priceImport = document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_PRICE_IMPORT]%>.value;
    var transport = document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_TRANSPORT]%>.value;
    var bea = document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_BEA]%>.value;
    var komisi = document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_KOMISI]%>.value;
    var lain = document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_LAIN_LAIN]%>.value;
    ...
    transport = removeChar(transport);
    transport = cleanNumberFloat(transport, sysDecSymbol, usrDigitGroup, usrDecSymbol);	
    document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_TRANSPORT]%>.value = formatFloat(''+transport, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);	
    
    bea = removeChar(bea);
    bea = cleanNumberFloat(bea, sysDecSymbol, usrDigitGroup, usrDecSymbol);	
    document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_BEA]%>.value = formatFloat(''+bea, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);	
    
    komisi = removeChar(komisi);
    komisi = cleanNumberFloat(komisi, sysDecSymbol, usrDigitGroup, usrDecSymbol);	
    document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_KOMISI]%>.value = formatFloat(''+komisi, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);	
    
    lain = removeChar(lain);
    lain = cleanNumberFloat(lain, sysDecSymbol, usrDigitGroup, usrDecSymbol);	
    document.frmreceive.<%=JspReceiveItem.colNames[JspReceiveItem.JSP_LAIN_LAIN]%>.value = formatFloat(''+lain, '', sysDecSymbol, usrDigitGroup, usrDecSymbol, decPlace);	
    
    amount = parseFloat(priceImport) + parseFloat(transport) + parseFloat(bea) + parseFloat(komisi) + parseFloat(lain);
    ...
    subtot = cleanNumberFloat(subtot, sysDecSymbol, usrDigitGroup, usrDecSymbol);
     ...
     function cmdAddItemMaster(){  
        ...
        function cmdAddItemMasterNew(){  
            var vendorId=document.frmreceive.JSP_VENDOR_ID.value; 
            window.open("<%=approot%>/posmaster/additemmaster.jsp?vendor_id=" + vendorId, null, "height=1000,width=1000, status=yes,toolbar=no,menubar=no,location=no, scrollbars=yes");
 ...
 function cmdToRecord(){
     ...
     document.frmreceive.action="receivelistimport.jsp";
     document.frmreceive.submit();
 }
                                                ...
                                                <tr> 
                                                    <td><!-- #BeginEditable "content" --> 
                                                        <form name="frmreceive" method ="post" action="">
                                                            ...
                                                            <input type="hidden" name="JSP_TYPE_INC" value="1">
                                                            <input type="hidden" name="hidden_rate" value="<%=rate%>">
    ...
    try {
        us = DbUser.fetch(receive.getUserId());
           ...
           <% }%>
            ...
            payment_value.add(I_Project.PAYMENT_TYPE_CREDIT);
                                                ...
                                                <td width="42%" colspan="2" class="comment"> 
                                                  <input name="<%=JspReceive.colNames[JspReceive.JSP_DUE_DATE]%>" value="<%=JSPFormater.formatDate((receive.getDueDate() == null) ? new Date() : receive.getDueDate(), "dd/MM/yyyy")%>" size="11" readonly>
                                                                                                    ...
                                                                                                    <td width="9%">No Pajak</td>
                                                                                                        
                                                                                                        <td width="42%" colspan="2" class="comment"><input type="text" value="<%=receive.getNoPajak()%>" name="<%=JspReceive.colNames[JspReceive.JSP_NO_PAJAK]%>" > </td>
                                                                                                    <tr align="left" > 
                                                                                                        <td height="21" width="12%" valign="top">&nbsp;&nbsp;Nilai $</td>
                                                                                                        <td width="42%" colspan="2" class="comment"><input type="text" value="<%=rate%>" name="rate" onchange="calculaterate()" > </td>
                                                                                                    </tr>    
                                                                                                    <tr align="left" > 
            ...
            if((receive.getStatus().equals(I_Project.DOC_STATUS_DRAFT) && DbSystemProperty.getValueByName("APPLY_DOC_APPROVAL").equalsIgnoreCase("YES")) || DbSystemProperty.getValueByName("APPLY_DOC_APPROVAL").equalsIgnoreCase("NO") ) {%>
