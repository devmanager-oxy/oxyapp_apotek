...
<%!
...

	{
		...
		for (int i = 0; i < objectClass.size(); i++) {
                     itemMaster = (ItemMaster) objectClass.get(i);
                    //try{
...
//int vectSize = DbStock.getStockItemCount(location_id, DbStock.TYPE_NON_CONSIGMENT, srcGroupId, srcCategoryId, srcName, srcCode);
...
//listItemMaster = DbStock.getStockList(start, recordToGet, location_id, DbStock.TYPE_NON_CONSIGMENT, srcGroupId, srcCategoryId, srcName, srcCode );
...
if (listItemMaster.size() < 1 && start > 0)
{
	 ...
	 //listItemMaster = DbStock.getStock(location_id, DbStock.TYPE_NON_CONSIGMENT);
         listItemMaster = DbItemMaster.list(start,recordToGet, whereClause , orderClause);
}
...
function cmdSearch(){
	...
	document.frmitemmaster.action="addItemTransfer.jsp";
...
function cmdEdit(oidItemMaster){
	//self.opener.document.frmsalesproductdetail.hidden_item_master_id.value=oidItemMaster  
        ...
        self.opener.document.frmtransfer.JSP_ITEM_MASTER_ID.value=oidItemMaster  
