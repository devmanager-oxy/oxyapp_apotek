...
String srcReturNumber = JSPRequestValue.requestString(request, "src_number");
...
if(srcVendorId!=0){
	whereClause = DbRetur.colNames[DbRetur.COL_VENDOR_ID]+"="+srcVendorId;	
}
...
if(srcLocationId!=0){
	if(whereClause.length()>0){
		whereClause = whereClause + " and "+DbRetur.colNames[DbRetur.COL_LOCATION_ID]+"="+srcLocationId+"";	
	}
	else{
    ...
    if(userLocations!=null && userLocations.size()>0){
        for(int x=0;x<userLocations.size();x++){
            ...
            whereLoc = whereLoc + DbRetur.colNames[DbRetur.COL_LOCATION_ID]+"="+loc.getOID()+" or ";
        }
...
if(srcReturNumber!=null && srcReturNumber.length()>0){
	if(whereClause.length()>0){
		whereClause = whereClause + " and "+DbRetur.colNames[DbRetur.COL_NUMBER]+" like '%"+srcReturNumber+"%'";	
	}
	else{
		whereClause = DbRetur.colNames[DbRetur.COL_NUMBER]+" like '%"+srcReturNumber+"%'";	
	}
}
...
if((iJSPCommand == JSPCommand.FIRST || iJSPCommand == JSPCommand.PREV )||
  (iJSPCommand == JSPCommand.NEXT || iJSPCommand == JSPCommand.LAST)){
	start = cmdDirectRetur.actionList(iJSPCommand, start, vectSize, recordToGet);
 } 
/* end switch list*/

/* get record to display */
orderClause = DbRetur.colNames[DbRetur.COL_NUMBER];
...
function cmdSearch(){
	...
	document.frmpurchase.action="returtovendorlist.jsp";
	document.frmpurchase.submit();
}


function cmdEdit(oid){
	document.frmpurchase.hidden_retur_id.value=oid;
	
	document.frmpurchase.command.value="<%=JSPCommand.EDIT%>";
	document.frmpurchase.prev_command.value="<%=prevJSPCommand%>";
	document.frmpurchase.action="returtovendor.jsp";
	document.frmpurchase.submit();
}

function cmdAdd(){
	document.frmpurchase.hidden_retur_id.value="0";
                                      ...
                                      <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                        <tr > 
                                          ...
                                          <td class="tabin" nowrap> 
                                            <div align="center">&nbsp;&nbsp;<a href="javascript:cmdAdd()" class="tablink">Direct 
                                              Retur</a>&nbsp;&nbsp;</div>
                                          </td>
                                          <td class="tabheader"><img src="<%=approot%>/images/spacer.gif" width="3" height="10"></td>
                                        ...
                                        <tr align="left" valign="top"> 
                                          <td height="22" valign="middle" colspan="3"> 
                                            <table width="100%" border="0" cellspacing="2" cellpadding="1">
                                              ...
                                              <tr> 
                                                <td width="12%">Retur Number</td>
                                                <td width="88%"> 
                                                  <input type="text" name="src_number" value="<%=srcReturNumber%>" align="left" size="25">
                                                </td>
                                              </tr>
