...
long oidTransfer = JSPRequestValue.requestLong(request, "hidden_repack_id");
...
/* end switch*/
JspRepack jspTransfer = cmdRepack.getForm();

/*count list All Transfer*/
int vectSize = DbRepack.getCount(whereClause);

Repack vendor = cmdRepack.getRepack();
...
orderClause = DbRepack.colNames[DbRepack.COL_DATE];
listCosting = DbRepack.list(start,recordToGet, whereClause , orderClause);
...
function cmdSearch(){
	...
	document.frmtransfer.action="repacklist.jsp";
	document.frmtransfer.submit();
}


function cmdEdit(oid,cnt){
	document.frmtransfer.hidden_repack_id.value=oid;
	document.frmtransfer.command.value="<%=JSPCommand.EDIT%>";
	document.frmtransfer.prev_command.value="<%=prevJSPCommand%>";
        if(parseInt(cnt)>1){ 
            document.frmtransfer.action="repackitemmulty.jsp";
        }
        else{
            document.frmtransfer.action="repackitem.jsp";
        }
	document.frmtransfer.submit();
}

function cmdAdd(){
	document.frmtransfer.hidden_repack_id.value="0";
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmtransfer" method ="post" action="">
                          ...
                          <input type="hidden" name="hidden_repack_id" value="<%=oidTransfer%>">
