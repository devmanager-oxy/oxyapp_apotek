<%-- 
    ...
    Created on : 12 november, 2012, 8:23:41 AM
...
if(itemCode.length()>0 && itemCode!=null){
    srcCode= itemCode;
}
...
if(srcCode!=null && srcCode.length()>0){
	...
	whereClause = whereClause + "(" + DbItemMaster.colNames[DbItemMaster.COL_CODE]+" like '%"+srcCode+"%'" +" || " + DbItemMaster.colNames[DbItemMaster.COL_BARCODE]+" like '%"+srcCode+"%')";
}
...
function cmdSearch(){
	...
	document.frmitemmaster.action="addItemRetur.jsp";
...
function cmdEdit(oidItemMaster){
	//self.opener.document.frmsalesproductdetail.hidden_item_master_id.value=oidItemMaster  
        self.opener.document.frmretur.X_itm_JSP_ITEM_MASTER_ID.value=oidItemMaster  
        self.opener.document.frmretur.submit();                
        self.close();  
	}
