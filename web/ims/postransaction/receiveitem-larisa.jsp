...
<%!
    public Vector drawList(int iJSPCommand, JspReceiveItem frmObject,
        ...
        jsplist.addHeader("Vendor Price", "10%");
       ...
       /* if (vectVendMaster != null && vectVendMaster.size() > 0) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
			...
			try{
				...
				System.out.println(e);
                    ...
                    if(useStockCode){
                        ...
                        strVal += "<tr><td colspan=\"4\"><input type=\"hidden\" name=\""+frmObject.colNames[frmObject.JSP_ITEM_MASTER_ID] +"\" value=\""+colCombo2.getOID()+"\">"+"<input style=\"text-align:left\" type=\"text\" name=\"X_"+frmObject.colNames[frmObject.JSP_ITEM_MASTER_ID] +"\" value=\""+colCombo2.getName()+"\"  size=\"35\" onChange=\"javascript:cmdAddItemMaster()\" >" + "<a href=\"javascript:cmdAddItemMaster()\"height=\"20\" border=\"0\" style=\"padding:0px\"> &nbsp Search</a>"+ "<a href=\"javascript:cmdAddItemMasterNew()\"height=\"20\" border=\"0\" style=\"padding:0px\"> &nbsp Add</a> </td></tr>";                    }
                    if (useStockCode) {
            ...
            rowx.add("<div align=\"right\">"+JSPFormater.formatNumber(vendorItem.getLastPrice(), "###,###.##")+"</div>");
                ...
                Vector vItem = new Vector();
            vItem= DbVendorItem.list(0, 0, "vendor_id=" + oidVendor + " and item_master_id="+ receiveItem.getItemMasterId(), "");
            VendorItem vendorItem = new VendorItem();
            if(vItem.size()>0){
                  vendorItem = (VendorItem)vItem.get(0);
            }
...
<%
	     ...
	     //update/insert vendor item otomatis
            ...
            if(user.getSegment1Id() != 0){
                sd1 = DbSegmentDetail.fetchExc(user.getSegment1Id());
                try{
                    loca = DbLocation.fetchExc(sd1.getLocationId());
                }catch(Exception ex){
                ...
                if(sd1.getLocationId()!=0){
                    vendors = DbVendor.list(0, 0, "type_loc_incoming='"+loca.getType()+"'", "name");
                }else{
                    vendors = DbVendor.list(0, 0, "", "name");
                }
