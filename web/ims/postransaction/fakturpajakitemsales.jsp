...
<%!
    public Vector drawList(int iJSPCommand, JspFakturPajakDetail frmObject,
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            SalesDetail salesDetail = (SalesDetail) objectClass.get(i);

            RptTranferL detail = new RptTranferL();

            rowx = new Vector();

            if (transferItemId == salesDetail.getOID()) {
                index = i;
            }
            ...
            try {
                itemMaster = DbItemMaster.fetchExc(salesDetail.getProductMasterId());
            ...
            detail.setPrice(salesDetail.getSellingPrice());
            rowx.add("<div align=\"left\">" + itemMaster.getCode() + "</div>");
            rowx.add("<div align=\"left\">" + ig.getName() + "</div>");
            detail.setQty(salesDetail.getQty());

            detail.setTotal(salesDetail.getTotal());
            rowx.add("<div align=\"center\">" + uom.getUnit() + "</div>");
            rowx.add("<div align=\"center\">" + (salesDetail.getTotal() + salesDetail.getDiscountAmount()) + "</div>");
...
<%
            ...
            String salesNumber = JSPRequestValue.requestString(request, JspFakturPajak.colNames[JspFakturPajak.JSP_SALES_NUMBER]);
            ...
            if (oidFakturPajak != 0) {
                salesNumber = fakturPajak.getSalesNumber();
            }
            oidSales = DbTransfer.getOidSalesByNumber(salesNumber);
            //}
            Vector vfakturItem = DbSalesDetail.list(0, 0, " sales_id=" + oidSales, " sales_detail_id");
            ...
            if (iJSPCommand == JSPCommand.POST) {
                ...
                if (vfakturItem != null && vfakturItem.size() > 0) {
                    for (int i = 0; i < vfakturItem.size(); i++) {
                        SalesDetail ti = (SalesDetail) vfakturItem.get(i);
                        ...
                        fakDetail.setItemMasterId(ti.getProductMasterId());
                        fakDetail.setTotal(ti.getTotal() + ti.getDiscountAmount());
                        fakDetail.setDiscount(ti.getDiscountAmount());
                        ItemMaster im = new ItemMaster();
                        try {
                            im = DbItemMaster.fetchExc(ti.getProductMasterId());
                        } catch (Exception e) {
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdPrintXLS(){	 
                window.open("<%=printroot%>.report.RptFakturPajakSalesXls?idx=<%=System.currentTimeMillis()%>");
                }
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" height="100%">
            ...
            if (fakturPajak.getOID() == 0) {
                ...
                rptKonstan.setNumberFaktur(number);
                                                                                                    ...
                                                                                                    <tr align="left"> 
                                                                                                        <td height="21" width="12%" valign="top">Sales Number</td>
                                                                                                        <td height="21" width="27%"> 
                                                                                                            <input type="text" name="<%=JspFakturPajak.colNames[JspFakturPajak.JSP_SALES_NUMBER]%>" value="<%= fakturPajak.getSalesNumber()%>" cols="40" rows="3">
                                                                                                        </td>
