...
<%-- 
    Document   : fakturpajaksales
    Created on : Oct 3, 2012, 3:11:01 PM
    Author     : Roy Andika
--%>
...
<%
            ...
            long oidTmpSalesId = JSPRequestValue.requestLong(request, "hidden_tmp_sales_id");
                ...
                if (fakturPajak.getOID() == 0) {
                    if (vFakturPajakDetail != null && vFakturPajakDetail.size() > 0) {
                        ...
                        for (int t = 0; t < vFakturPajakDetail.size(); t++) {
                            ...
                            int count = 0;
                            if (tmpOid != fd.getSalesId()) {
                                count = DbFakturPajakDetail.getCountSales(fd.getSalesId());
                            }
                            if (count == 0) {
                ...
                if (vFakturPajakDetail != null && vFakturPajakDetail.size() > 0) {
                    ...
                    for (int t = 0; t < vFakturPajakDetail.size(); t++) {
                        ...
                        if (checked == 1) {
                            ...
                            whereFd = whereFd + DbFakturPajakDetail.colNames[DbFakturPajakDetail.COL_SALES_ID] + " = " + fd.getSalesId();
                            tmpSales.add("" + fd.getSalesId());
                        }
                    }
                    if (whereFd.length() > 0) {
            ...
            if (iJSPCommand == JSPCommand.REFRESH){
                if (vFakturPajakDetail != null && vFakturPajakDetail.size() > 0) {
                    for (int t = 0; t < vFakturPajakDetail.size(); t++){
                        FakturPajakDetail fd = (FakturPajakDetail) vFakturPajakDetail.get(t);
                        if (fd.getSalesId() == oidTmpSalesId) {
                            trasferIdExist = true;
                        }
                    }
                }
                ...
                if (trasferIdExist == false) {                    
                    String whereFD = DbSalesDetail.colNames[DbSalesDetail.COL_SALES_ID] + " = " + oidTmpSalesId;
                    //String orderFD = " im."+DbItemMaster.colNames[DbItemMaster.COL_NAME]+" asc ";
                    //Vector vItem = DbFakturPajakDetail.getSalesDetail(whereFD,orderFD);
                    Vector vItem = DbSalesDetail.list(0,0,whereFD,DbSalesDetail.colNames[DbSalesDetail.COL_SALES_DETAIL_ID]+" asc ");
                    
                    if (vItem != null && vItem.size() > 0) {
                        for (int iItem = 0; iItem < vItem.size(); iItem++) {
                            SalesDetail sd = (SalesDetail) vItem.get(iItem);

                            ItemMaster itemMaster = new ItemMaster();
                            try {
                                itemMaster = DbItemMaster.fetchExc(sd.getProductMasterId());
                            } catch (Exception e) {
                            }
                            
                                FakturPajakDetail tmpFPD = new FakturPajakDetail();
                                tmpFPD.setSalesId(oidTmpSalesId);
                                tmpFPD.setItemMasterId(itemMaster.getOID());
                                tmpFPD.setItemName(itemMaster.getName());
                                tmpFPD.setPrice(sd.getSellingPrice());
                                tmpFPD.setDiscount(sd.getDiscountAmount());
                                tmpFPD.setQty(sd.getQty());
                                double total = (sd.getSellingPrice() * sd.getQty()) - sd.getDiscountAmount();
            ...
            if (iJSPCommand == JSPCommand.RESET){                
                ...
                if(fakturPajak.getOID() != 0){
                    DbFakturPajak.updateLocation(fakturPajak.getOID(),oidLocationTo);
                }
    ...
    <head>
        ...
        <script language="JavaScript"> 
            function cmdPrintDocXLS(){                
                window.open("<%=printroot%>.report.ReportPajakSalesXls?oid=<%=fakturPajak.getOID()%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                } 
                ...
                function cmdOpenSales(){
                    window.open("<%=approot%>/postransaction/getsaleslist.jsp?location=<%=fakturPajak.getLocationToId()%>", null, "height=400,width=800, status=yes,toolbar=no,menubar=no,location=no, scrollbars=yes");
                    }
        ...
        <!-- #EndEditable -->
                                                ...
                                                <tr> 
                                                    <td><!-- #BeginEditable "content" --> 
                                                        <form name="frmfakturpajak" method ="post" action="">
                                                            ...
                                                            <input type="hidden" name="<%=JspFakturPajak.colNames[JspFakturPajak.JSP_TYPE_FAKTUR]%>" value="<%=DbFakturPajak.TYPE_FAKTUR_SALES%>">   
            ...
            if (fakturPajak.getLocationToId() == 0) {
                if (vLocation != null && vLocation.size() > 0) {
                    ...
                    strNumber = DbFakturPajak.getNextNumberFp(counterJournal, objLocation.getOID());
                }
            } else {
                counterJournal = DbFakturPajak.getNextCounterFp(fakturPajak.getLocationToId());
                strNumber = DbFakturPajak.getNextNumberFp(counterJournal, fakturPajak.getLocationToId());
            ...
            if (vFakturPajakDetail != null && vFakturPajakDetail.size() > 0) {
                for (int i = 0; i < vFakturPajakDetail.size(); i++) {
                    ...
                    try {
                        sales = DbSales.fetchExc(fpd.getSalesId());
                    } catch (Exception e) {
                    }
                    
                    double total = (fpd.getQty() * fpd.getPrice()) - fpd.getDiscount();
                                                                                                    %>
                                                                                                    <tr>
                                                                                                        <%
                                                                                                            if (sales.getOID() != salesId) {
                    ...
                    try {
                        sales = DbSales.fetchExc(fpd.getSalesId());
                                                                                                    ...
                                                                                                    <tr>
                                                                                                        ...
                                                                                                        <td bgcolor="#F4F673" align="center"><a href="javascript:cmdOpenSales()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('new21','','../images/search.jpg',1)"><img src="../images/search2.jpg" name="new211" height="17" border="0" style="padding:0px"></a></td>
                                                                                                    </tr>
                                                                                                    <%}%> 
                                                                                                    ...
                                                                                                    <tr height="20" align="left" valign="top" > 
                                                                                                        <td colspan="3" bgcolor="#F15858" valign="middle"><font face="arial" color="FFFFFF">&nbsp;Data sales already exist</font></td>                                                                                                        
                                                                                                    </tr>
                                                                                                    <%}%>
                                                                                                    <%if (dataExist) {%>
                                                                                                    <tr height="20" align="left" valign="top" > 
