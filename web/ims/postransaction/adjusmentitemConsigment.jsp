...
Vector stockItems = DbStock.getStock(adjusment.getLocationId(),DbStock.TYPE_CONSIGMENT);
...
function cmdAddItemMaster(){   
            ...
            window.open("<%=approot%>/postransaction/addItemAdjusmentConsigment.jsp?location_id=" + oidLoc , null, "height=400,width=600, status=yes,toolbar=no,menubar=no,location=no, scrollbars=yes");
...
function cmdLocation(){
	<%if(adjusment.getOID()!=0 && purchItems!=null && purchItems.size()>0){%>
		if(confirm('Warning !!\nChange the vendor could effect to deletion of some or all adjusment item based on vendor item master. ')){
			document.frmadjusment.command.value="<%=JSPCommand.LOAD%>";
			document.frmadjusment.action="adjusmentitemConsigment.jsp";
			document.frmadjusment.submit();
		}
	<%}else{%>
			document.frmadjusment.command.value="<%=JSPCommand.LOAD%>";
...
function cmdToRecord(){
	...
	document.frmadjusment.action="adjusmentlistConsigment.jsp";
	document.frmadjusment.submit();
}
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmadjusment" method ="post" action="">
                          ...
                          <input type="hidden" name="<%=JspAdjusmentItem.colNames[JspAdjusmentItem.JSP_TYPE]%>" value="<%=DbAdjusmentItem.TYPE_CONSIGMENT%>">
                          <input type="hidden" name="<%=JspAdjusment.colNames[JspAdjusment.JSP_TYPE]%>" value="<%=DbAdjusment.TYPE_CONSIGMENT%>">
                          <% if(DbSystemProperty.getValueByName("APPLY_DOC_APPROVAL").equalsIgnoreCase("NO")){%>
											...
											%>
                                              ...
                                              <tr align="left" > 
                                                <td colspan="5" valign="top"> 
                                                  &nbsp; 
                                                  <%
													Vector x = drawList(iJSPCommand,jspAdjusmentItem, adjusmentItem, purchItems, oidAdjusmentItem, approot, adjusment.getLocationId(), iErrCode2, adjusment.getStatus(), itemMasterId);
                          ...
                          <script language="JavaScript">
							...
							<%
							if(stockItems!=null && stockItems.size()>0){
							if(((iErrCode2==0 &&(adjusment.getStatus().equals(I_Project.DOC_STATUS_DRAFT) && (iJSPCommand==JSPCommand.NONE || iJSPCommand==JSPCommand.ADD || (iJSPCommand==JSPCommand.EDIT && adjusmentItem.getOID()!=0) || iErrCode!=0))) && DbSystemProperty.getValueByName("APPLY_DOC_APPROVAL").equalsIgnoreCase("YES")) || DbSystemProperty.getValueByName("APPLY_DOC_APPROVAL").equalsIgnoreCase("NO")){%>
                            	parserMaster();
							<%}
							}%>
                          </script>
