...
<%!
...

	{
		...
		cmdist.addHeader("From","10%");
		//cmdist.addHeader("Currency","5%");
		cmdist.addHeader("To","10%");
...
if(srclocFromId!=0){
    ...
    if(userLocations!=null && userLocations.size()>0){ 
        for(int x=0;x<userLocations.size();x++){
            ...
            whereLoc = whereLoc + DbTransfer.colNames[DbTransfer.COL_FROM_LOCATION_ID]+"="+loc.getOID()+" or ";
        }        
...
if(srclocToId!=0){
    ...
    if(srclocFromId==0){
        ...
        if(!inside){
            ...
            whereClause = whereClause + DbTransfer.colNames[DbTransfer.COL_TO_LOCATION_ID]+"="+srclocToId;
    ...
    if(userLocations!=null && userLocations.size()>0){
        for(int x=0;x<userLocations.size();x++){
            Location loc = (Location)userLocations.get(x);
            whereLoc = whereLoc + DbTransfer.colNames[DbTransfer.COL_TO_LOCATION_ID]+"="+loc.getOID()+" or ";
        }
        whereLoc = "("+whereLoc.substring(0,whereLoc.length()-4)+")";
    }
...
if(whereClause.length()>0){
	if(whereClause.length()>0){
		whereClause = whereClause + " and "+DbTransfer.colNames[DbTransfer.COL_STATUS]+"!='REQUEST'";	
	}
	else{
		whereClause = DbTransfer.colNames[DbTransfer.COL_STATUS]+"='REQUEST'";	
	}
}
if(lokId!=0){
    if(whereClause.length()>0){
            if(whereClause.length()>0){
                    whereClause = whereClause + " and ("+DbTransfer.colNames[DbTransfer.COL_FROM_LOCATION_ID]+"=" + lokId + " or " + DbTransfer.colNames[DbTransfer.COL_TO_LOCATION_ID]+"=" + lokId + ")";	
            }
            else{
                    whereClause = DbTransfer.colNames[DbTransfer.COL_FROM_LOCATION_ID]+"=" + lokId + " or " + DbTransfer.colNames[DbTransfer.COL_TO_LOCATION_ID]+"=" + lokId ;	
            }
    }
}
                                ...
                                <table width="100%" border="0" cellspacing="1" cellpadding="1" height="17">
                                  <tr valign="bottom"> 
                                    <td width="60%" height="23"><b><font color="#990000" class="lvl1">Transaction 
                                      </font><font class="tit1">&raquo; <span class="lvl2">Transfer Stock </span></font></b></td>
                                      ...
                                      <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                        <tr > 
                                          ...
                                          <td class="tabin" nowrap> 
                                            <div align="center">&nbsp;&nbsp;<a href="javascript:cmdAdd()" class="tablink">Transfer Stock</a>&nbsp;&nbsp;</div>
                                          </td>
                                          <td class="tabheader"><img src="<%=approot%>/images/spacer.gif" width="3" height="10"></td>
                                              ...
                                              <tr> 
                                                ...
                                                <td width="90%" nowrap> 
                                                  <select name="src_loc_from_id">
                                                    <option value="0" <%if(srclocFromId==0){%>selected<%}%>>- 
                                                    All -</option>
                                                    <%
													
													//Vector locs = userLocations;//DbLocation.list(0,0, "", "name");
                                                    Vector locs = DbLocation.list(0,0, "", "name");
