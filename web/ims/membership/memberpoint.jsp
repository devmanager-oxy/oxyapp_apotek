...
if(iJSPCommand==JSPCommand.POST){
	try{
		...
		if(temp!=null && temp.size()>0){
			...
			try {
				String sql = "select sum(point*in_out) from pos_member_point where customer_id="+cus.getOID()+" and to_days(date) < to_days('"+JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd")+"')";
				
				dbrs = CONHandler.execQueryResult(sql);
				ResultSet rs = dbrs.getResultSet();
			
				int count = 0;
				while(rs.next()) { pointx = rs.getDouble(1); }
			...
			result = DbMemberPoint.list(0,0, "to_days(date) >= to_days('"+JSPFormater.formatDate(srcStartDate, "yyyy-MM-dd")+"') and customer_id="+cus.getOID(), "");
                                          ...
                                          <td height="22" valign="middle" colspan="3"> 
                                            <table width="70%" border="0" cellspacing="1" cellpadding="1">
                                              <tr> 
                                                ...
                                                <td class="tablehdr">Date</td>
                                                <td class="tablehdr">Note</td>
                                                <td class="tablehdr">Point</td>
                                                <td class="tablehdr">Saldo</td>
                                              </tr>
                                              <tr> 
                                                <td class="tablearialcell1">&nbsp;</td>
                                                <td class="tablearialcell1">&nbsp;</td>
                                                <td class="tablearialcell1">Saldo 
                                                  Awal </td>
                                                <td class="tablearialcell1"> 
                                                  <div align="right"><%=JSPFormater.formatNumber(pointx, "#,###")%></div>
                                                </td>
                                                <td class="tablearialcell1"> 
                                                  <div align="right"><%=JSPFormater.formatNumber(pointx, "#,###")%></div>
                                                </td>
											  ...
											  for(int i=0; i<result.size(); i++){ 
											  		MemberPoint mp = (MemberPoint)result.get(i);
													pointx = pointx + (mp.getPoint() * mp.getInOut());
													...
													if(mp.getType()==0){
														note = "Point sales";
                                                                                                                ...
                                                                                                                try{
                                                                                                                    sales = DbSales.fetchExc(mp.getSalesId());
                                                                                                                    note = note +" #"+sales.getNumber()+"-"+JSPFormater.formatDate(sales.getDate(),"dd/MM/yy");
                                                                                                                }
                                                                                                                catch(Exception e){
                                                                                                                    
                                                                                                                }
                                                                                                                if(mp.getItemGroupId()!=0){
                                                                                                                    ItemGroup ig = new ItemGroup();
                                                                                                                    try{
                                                                                                                        ig = DbItemGroup.fetchExc(mp.getItemGroupId());
                                                                                                                        note = note +"-"+ig.getName();
                                                                                                                    }
                                                                                                                    catch(Exception e){
                                                                                                                    ...
                                                                                                                    note = note +"-"+I_Ccs.strCategoryType[mp.getGroupType()];
                                                                                                                }
													}
                                                                                                        else{
                                                                                                            note = "Point released";
											  ...
											  %>
                                              <tr> 
                                                <td class="tablearialcell"><%=i+1%></td>
                                                <td class="tablearialcell"><%=JSPFormater.formatDate(mp.getDate(), "dd/MM/yyyy")%></td>
                                                <td class="tablearialcell"><%=note%></td>
                                                <td class="tablearialcell"> 
                                                  <div align="right"><%=JSPFormater.formatNumber(mp.getPoint() * mp.getInOut(), "#,###")%></div>
                                                </td>
                                                <td class="tablearialcell"> 
                                                  <div align="right"><%=JSPFormater.formatNumber(pointx, "#,###")%></div>
                                                </td>
