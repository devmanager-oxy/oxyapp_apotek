...

	{
		...
		for (int i = 0; i < objectClass.size(); i++) {
			try{
				...
				String str = customer.getOID()+"','"+customer.getName()+"','"+customer.getAddress()+"','"+customer.getContactPerson()+"','"+customer.getPhone()+"','"+customer.getContactPosition();
...
function cmdAsk(oidCustomer){
	...
	document.frmcustomer.prev_command.value="<%=prevJSPCommand%>";
	document.frmcustomer.action="customerpglist.jsp";
	document.frmcustomer.submit();
}
...
function cmdEdit(oidCus, cName, cAddress, cPerson, cPhone, cPosition){

	self.opener.frmproject.<%=JspProject.colNames[JspProject.JSP_CUSTOMER_ID] %>.value=oidCus;
	self.opener.frmproject.customer_name.value=cName;
	self.opener.frmproject.<%=JspProject.colNames[JspProject.JSP_CUSTOMER_ADDRESS] %>.value=cAddress;
	self.opener.frmproject.<%=JspProject.colNames[JspProject.JSP_CUSTOMER_PIC] %>.value=cPerson;
	self.opener.frmproject.<%=JspProject.colNames[JspProject.JSP_CUSTOMER_PIC_PHONE] %>.value=cPhone;
	self.opener.frmproject.<%=JspProject.colNames[JspProject.JSP_CUSTOMER_PIC_POSITION] %>.value=cPosition;			
	self.close();
}
