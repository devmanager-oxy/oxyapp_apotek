...
<%!
	...
	public String drawList(int iJSPCommand,JspDeseas frmObject, Deseas objEntity, Vector objectClass,  long deseaseId)

	{
		...
		ctrlist.addHeader("Nama Penyakit","100%");
		...
		for (int i = 0; i < objectClass.size(); i++) {
			 Deseas deseas = (Deseas)objectClass.get(i);
			 rowx = new Vector();
			 if(deseaseId == deseas.getOID())
				 index = i; 

			 if(index == i && (iJSPCommand == JSPCommand.EDIT || iJSPCommand == JSPCommand.ASK)){
					
				rowx.add("<input type=\"text\" name=\""+frmObject.colNames[JspDeseas.JSP_FIELD_NAME] +"\" value=\""+deseas.getName()+"\" class=\"formElemen\">");
			}else{
				rowx.add("<a href=\"javascript:cmdEdit('"+String.valueOf(deseas.getOID())+"')\">"+deseas.getName()+"</a>");
			} 
		...
		if(iJSPCommand == JSPCommand.ADD || (iJSPCommand == JSPCommand.SAVE && frmObject.errorSize() > 0)){ 
				rowx.add("<input type=\"text\" name=\""+frmObject.colNames[JspDeseas.JSP_FIELD_NAME] +"\" value=\""+objEntity.getName()+"\" class=\"formElemen\">");
...
long oidDeseas = JSPRequestValue.requestLong(request, "hidden_desease_id");
...
CmdDeseas ctrlDeseas = new CmdDeseas(request);
JSPLine ctrLine = new JSPLine();
Vector listDeseas = new Vector(1,1);

/*switch statement */
iErrCode = ctrlDeseas.action(iJSPCommand , oidDeseas);
/* end switch*/
JspDeseas jspDeseas = ctrlDeseas.getForm();

/*count list All Deseas*/
int vectSize = DbDeseas.getCount(whereClause);

/*switch list Deseas*/
if((iJSPCommand == JSPCommand.FIRST || iJSPCommand == JSPCommand.PREV )||
  (iJSPCommand == JSPCommand.NEXT || iJSPCommand == JSPCommand.LAST)){
		start = ctrlDeseas.actionList(iJSPCommand, start, vectSize, recordToGet);
 } 
/* end switch list*/

Deseas deseas = ctrlDeseas.getDeseas();
msgString =  ctrlDeseas.getMessage();

/* get record to display */
listDeseas = DbDeseas.list(start,recordToGet, whereClause , orderClause);

/*handle condition if size of record to display = 0 and start > 0 	after delete*/
if (listDeseas.size() < 1 && start > 0)
{
...
function cmdAdd(){
	document.frmdeseas.hidden_desease_id.value="0";
	document.frmdeseas.command.value="<%=JSPCommand.ADD%>";
	document.frmdeseas.prev_command.value="<%=prevJSPCommand%>";
	document.frmdeseas.action="deseas.jsp";
	document.frmdeseas.submit();
}

function cmdAsk(oidDeseas){
	document.frmdeseas.hidden_desease_id.value=oidDeseas;
...
function cmdConfirmDelete(oidDeseas){
	document.frmdeseas.hidden_desease_id.value=oidDeseas;
...
function cmdDelPict(oidDeseas){
	document.frmimage.hidden_desease_id.value=oidDeseas;
	document.frmimage.command.value="<%=JSPCommand.POST%>";
	document.frmimage.action="deseas.jsp";
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmdeseas" method ="post" action="">
                          ...
                          <input type="hidden" name="hidden_desease_id" value="<%=oidDeseas%>">
							...
							%>
                                  <tr align="left" valign="top"> 
                                    <td height="22" valign="middle" colspan="3"> 
                                      <%= drawList(iJSPCommand,jspDeseas, deseas,listDeseas,oidDeseas)%> </td>
                                  </tr>
                                  <% 
						  }catch(Exception exc){ 
						  }%>
                            ...
                            <tr align="left" valign="top" > 
                              <td colspan="3" class="command"> 
                                <%
									...
									String scomDel = "javascript:cmdAsk('"+oidDeseas+"')";
									String sconDelCom = "javascript:cmdConfirmDelete('"+oidDeseas+"')";
									String scancel = "javascript:cmdEdit('"+oidDeseas+"')";
